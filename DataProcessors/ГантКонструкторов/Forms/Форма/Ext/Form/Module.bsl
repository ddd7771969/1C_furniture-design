...
Процедура ОбновитьГантНаСервере()
	...
	Если Сортировка = 0 Тогда
		тзДанныеГанта.Сортировать("ПроектНаименование, 		ДатаНачалаПлан");
	ИначеЕсли Сортировка = 1 Тогда
		тзДанныеГанта.Сортировать("ИсполнительНаименование, ДатаНачалаПлан");
	Иначе
		тзДанныеГанта.Сортировать("ОбъектТЗНаименование, 	ДатаНачалаПлан");
	КонецЕсли; 
...
Функция ПолучитьДанныеДиаграммы()
	...
	Запрос.Текст = 
		...
		|	ТаймингЗадачНачалоВыполненияПлан.НачалоВыполненияПлан КАК НачалоВыполненияПлан
		|ПОМЕСТИТЬ втТайминг
		|ИЗ
		|	вт КАК вт
		...
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТаймингЗадачНачалоВыполненияПлан.СрезПоследних(
		|					,
		|					Задача В
		|						(ВЫБРАТЬ
		|							вт.Задача
		|						ИЗ
		|							вт)) КАК ТаймингЗадачНачалоВыполненияПлан
		|			ПО ТаймингЗадач.Задача = ТаймингЗадачНачалоВыполненияПлан.Задача
		|		ПО вт.Задача = ТаймингЗадач.Задача
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	вт.ОбъектТЗ КАК ОбъектТЗ,
		|	ЕСТЬNULL(втТайминг.Исполнитель, вт.Задача.Исполнитель) КАК Исполнитель,
		...
		|	ЕСТЬNULL(втТайминг.НачалоВыполненияПлан, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаНачалаПлан,
		|	вт.Проект КАК Проект,
		|	вт.Проект.Наименование КАК ПроектНаименование,
		|	ВЫБОР
		|		КОГДА втТайминг.Исполнитель ЕСТЬ NULL
		|			ТОГДА вт.Задача.Исполнитель.Наименование
		|		ИНАЧЕ втТайминг.Исполнитель.Наименование
		|	КОНЕЦ КАК ИсполнительНаименование,
		|	вт.Подразделение.Наименование КАК ПодразделениеНаименование,
		|	вт.ОбъектТЗ.Наименование КАК ОбъектТЗНаименование,
		|	""ПКД"" КАК Этап,
		|	вт.Задача КАК Задача,
		|	ЕСТЬNULL(вт.Задача.ОбобщеннаяЗадача, ЛОЖЬ) КАК ОбобщеннаяЗадача
		|ИЗ
		|	вт КАК вт
		|		ЛЕВОЕ СОЕДИНЕНИЕ втТайминг КАК втТайминг
		|		ПО вт.Задача = втТайминг.Задача";
