&НаКлиенте
Перем СтруктураДанных, ИзмененыНастройки;
...
Процедура ОбновитьИсполнителяНаСервере(ТекущийИсполнитель, ТекущийПроект, мДанныеГантаИсполнитель)
	...
	Диаграмма.Обновление = Ложь;
	омГантКонструкторовНаСервере.ВывестиИсполнитель(стПараметры, ТекущийИсполнитель, Строка(ТекущийИсполнитель));
	Диаграмма.Обновление = Истина;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	стПараметры = омНастройкиПолучитьЗаписать.ПолучитьНастройкиГантКонструктора(); 
...
Процедура ДиаграммаПриОкончанииРедактированияИнтервала(Элемент, Интервал, ОтменаРедактирования)
	...
	Если Интервал.Расшифровка.РежимРаботы = ПредопределенноеЗначение("Перечисление.РежимыРаботы.Пятидневка") Тогда
		Продолжительность 	= Окр(омДаты.КоличествоРабочихДней(Интервал.Начало, Интервал.Конец), 0);
		Продолжительность 	= ?(Продолжительность = 0, 1, Продолжительность);
		Интервал.Начало 	= НачалоДня(Интервал.Начало);
		Интервал.Конец 		= КонецДня(Интервал.Конец + омДаты.ДобавитьДниСУчетомВыходных(Интервал.Начало, Продолжительность) - 1);
	Иначе
		Продолжительность 	= (Интервал.Конец - Интервал.Начало) / (24 * 3600);
		Продолжительность 	= ?(Продолжительность = 0, 1, Продолжительность);
		Интервал.Начало 	= НачалоДня(Интервал.Начало);
		Интервал.Конец 		= КонецДня(Интервал.Начало + (Продолжительность * 24 * 3600) - 1);
	КонецЕсли;
	...
	Если Интервал.Расшифровка.Свойство("Задача") Тогда
		...
		ЗаписьПлановыхПараметров(Интервал.Расшифровка.Задача, Интервал.Начало, Продолжительность); 
...
Процедура ЗаписьПлановыхПараметров(Задача, Начало, Продолжительность)
	
	РегистрыСведений.ПлановоеЗначениеПроектирования.ЗаписатьПлановоеЗначениеЗадачи(Задача, Начало, Продолжительность); 
	
КонецПроцедуры // ЗаписьПлановыхПараметров()
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	Если ИзмененыНастройки И НЕ ЗавершениеРаботы Тогда
		...
		стПараметры = омНастройкиПолучитьЗаписать.ПолучитьСтруктуруГантКонструктора();
		стПараметры.Проект 			= Проект;
		стПараметры.Конструктор 	= Конструктор;
		стПараметры.Сортировка 		= Сортировка; 
		стПараметры.Подразделение 	= Подразделение; 
		
		омНастройкиПолучитьЗаписать.ЗаписатьНастройкиГантКонструктора(стПараметры);
...
Процедура Легенда(Команда)
	ОткрытьФорму("Обработка.ГантКонструкторов.Форма.ФормаЛегендаГанта", , ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
