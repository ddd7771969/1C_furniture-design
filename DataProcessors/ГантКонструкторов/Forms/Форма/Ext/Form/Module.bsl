...
Функция ПолучитьДанныеДиаграммы()
	...
	Запрос.Текст = 
		...
		|		ПО Задачи_ПКД.Исполнитель.Сотрудник = Подразделения.Сотрудник
		|			И (Подразделения.НомерСтрокиТаблицы = 1)
		|ГДЕ
		|	ВЫБОР
		...
		|	ДанныеКомплекта.ВнешняяДатаОкончаниеПроектирования КАК ОкончаниеПКД,
		...
		|		ПО втПКД.ОбъектТЗ = ДанныеКомплекта.Комплект
		|ГДЕ
