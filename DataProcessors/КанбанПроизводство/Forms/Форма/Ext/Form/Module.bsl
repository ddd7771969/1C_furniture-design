...
Процедура ПриОткрытии(Отказ)
	...
	Для каждого текПроект Из стПараметры.ПроектыКанбанТЗ Цикл
		Проект.Добавить(текПроект);	
	КонецЦикла;

	Если НЕ стПараметры.Свойство("Сотрудник") Тогда
		стПараметры.Вставить("Сотрудник", ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка"));
	КонецЕсли;
	...
	стНастройки.Вставить("Сотрудник", 				стПараметры.Сотрудник);
	...
	стДанныеHTML 		= омКанбанПроизводство.ПолучитьHTMLПроизводство(стСтруктураКанбана, стНастройки);
...
Процедура ПолучитьИОбработатьДанные() Экспорт
	...
	ОкноДокумента = Элементы.ПолеHTML.Документ.parentWindow;
	Если ОкноДокумента = Неопределено  Тогда   
		ОкноДокумента = Элементы.ПолеHTML.Документ.defaultView;
	КонецЕсли;

	ДанныеВозврата = омКанбанНаКлиенте.ПолучитьИОбработатьДанные(ОкноДокумента, стСтруктураКанбана, СчетчикОбращений, Неопределено, Проект, стНастройки, "Производство");
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ИзмененыНастройки И НЕ ЗавершениеРаботы Тогда
		...
		Для каждого х Из Проект Цикл
			стПараметры.ПроектыКанбанТЗ.Добавить(х.Значение);		
		КонецЦикла;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновитьКанбанПроизводства" Тогда
...
Процедура КомплектыНаСтопе()

	КомплектыНаСтопе.ЗагрузитьЗначения(омКанбанДоскиНаСервере.ПолучитьКомплектыНаСтопе(омКанбанДоскиНаСервереПовтор.ПолучитьУИДДоски("УИДДоскиПроизводство")));
	
	КоличествоЭлементов = КомплектыНаСтопе.Количество();
	Если КоличествоЭлементов = 0  Тогда
	
		Элементы.ГруппаНаСтопе.Видимость = Ложь;	
	
	Иначе
	
		Элементы.ГруппаНаСтопе.Видимость = Истина;	
		Элементы.ГруппаНаСтопе.Заголовок = "На стопе (" + КоличествоЭлементов + ")";	
