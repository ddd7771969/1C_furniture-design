...
КонецПроцедуры // ПослеВыбораКудаКолонки()
...
Процедура ПослеВводаНаименованиеКамбана(НаименованиеДоски, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Элементы.Доска.СписокВыбора.Очистить();
		ЗаполнитьСписокДоск();
	...
	Пока Выборка.Следующий() Цикл
		...
		Для х = 0 По МаксимальныИндекс Цикл
			...
			Если текСтрока.Наименование = Выборка.Наименование И х + 1 = Выборка.НомерПоПорядку И текСтрока.ИмяСобытия = Выборка.ИмяСобытия
				...
				И текСтрока.Окончание = Выборка.Окончание И текСтрока.ЗапрещенИнтерактивныйПереход = Выборка.ЗапрещенИнтерактивныйПереход 
				И текСтрока.Статус = Выборка.Статус И текСтрока.ЗапрещенИнтерактивныйУход = Выборка.ЗапрещенИнтерактивныйУход 
				И текСтрока.ДействиеПриПопадании = Выборка.ДействиеПриПопадании И текСтрока.ВРаботе = Выборка.ВРаботе 
				И текСтрока.ТребуетсяКомментарий = Выборка.ТребуетсяКомментарий 
				 Тогда
			...
			МенеджерЗаписей.Наименование 					= текСтрока.Наименование;
			МенеджерЗаписей.ИмяСобытия 						= текСтрока.ИмяСобытия;
			МенеджерЗаписей.ЗапрещенИнтерактивныйПереход 	= текСтрока.ЗапрещенИнтерактивныйПереход;
			МенеджерЗаписей.ЗапрещенИнтерактивныйУход 		= текСтрока.ЗапрещенИнтерактивныйУход;
			МенеджерЗаписей.ДействиеПриПопадании 			= текСтрока.ДействиеПриПопадании;
			МенеджерЗаписей.ЛимитКарточек 					= текСтрока.ЛимитКарточек;
			МенеджерЗаписей.ЛимитЧасов 						= текСтрока.ЛимитЧасов;
			МенеджерЗаписей.Окончание 						= текСтрока.Окончание;
			МенеджерЗаписей.Статус 							= текСтрока.Статус;
			МенеджерЗаписей.Цвет 							= текСтрока.Цвет;
			МенеджерЗаписей.ВРаботе 						= текСтрока.ВРаботе;
			МенеджерЗаписей.ТребуетсяКомментарий 			= текСтрока.ТребуетсяКомментарий;
