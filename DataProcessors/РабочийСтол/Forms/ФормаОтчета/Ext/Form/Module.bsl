...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Элементы.ФормаНастройкаРабочегоСтола.Видимость = РольДоступна("Программист");
...
Процедура ЗаполнитьЗадачиВРаботеВедущийКонструктор()
	...
	Запрос.Текст = 
		...
		|	И Задачи_РКД.ТипЗадачи В(&ТипыЗадачКонструктора)
		|	И ВЫБОР
		|			КОГДА &Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|				ТОГДА ИСТИНА
...
&НаКлиенте
Процедура КорректировкаДизайнером(Команда)
	ОткрытьФорму("Обработка.Дополнение_ТЗ_ПКД.Форма", Новый Структура("ОтЗаказчика", Ложь), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура КорректировкаЗаказчиком(Команда)
	ОткрытьФорму("Обработка.Дополнение_ТЗ_ПКД.Форма", Новый Структура("ОтЗаказчика", Истина), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
