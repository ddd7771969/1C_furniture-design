...
Процедура ПолучитьИОбработатьДанные() Экспорт
	...
	Если омКанбанНаКлиенте.ПолучитьИОбработатьДанные(ОкноДокумента, стСтруктураКанбана, СчетчикОбращений, Сотрудник, Проект, стНастройки) Тогда
...
Процедура ПоискОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, ВыборДобавлением, СтандартнаяОбработка)
	...
	id = омКанбанТЗ.ПолучитУИДРесурса(ВыбранноеЗначение);	
	
	ОкноДокумента = ЭтаФорма.Элементы.ПолеHTML.Документ.defaultView;
	ОкноДокумента.scrollIntoView(id);
КонецПроцедуры
