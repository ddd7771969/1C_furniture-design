&НаКлиенте
Перем ПодчиненыеЭлеметыСтрокиДерева, СоответствиеСотрудниковПоДолжностям;
...
Процедура УстановитьСотрудников(ОбъектИзменения, Автор, строкаДанных)
	...
	Если ЗначениеЗаполнено(строкаДанных.Замерщик) Тогда
 	   МенеджерЗаписи.Замерщик = строкаДанных.Замерщик;
	ИначеЕсли НЕ ЗначениеЗаполнено(МенеджерЗаписи.Замерщик) Тогда
		МенеджерЗаписи.Замерщик = Замерщик;
	КонецЕсли;
	...
	омКанбанТЗ.УстановитьРазделительСотрудникТЗ(ОбъектИзменения, МенеджерЗаписи.Дизайнер);
	
	СтруктуруДляИзмененияИсполнителя = омЗадачаОбщаяНаСервере.ПолучитьСтруктуруДляИзмененияИсполнителя();
	ЗаполнитьЗначенияСвойств(СтруктуруДляИзмененияИсполнителя, строкаДанных);
...
Процедура ПроектПриИзмененииНаСервере()
	...
	|	ВЫБОР
	|		КОГДА ИзменениеСотрудникаСрезПоследних.Замерщик ЕСТЬ NULL
	|			ТОГДА ИзделияКомплект.Проект.Замерщик
	|		ИНАЧЕ ИзменениеСотрудникаСрезПоследних.Замерщик
	|	КОНЕЦ КАК Замерщик,
	|	ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ИзменениеСотрудникаСрезПоследних.Замерщик ЕСТЬ NULL
	|			ТОГДА Композиции.Проект.Замерщик
	|		ИНАЧЕ ИзменениеСотрудникаСрезПоследних.Замерщик
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ИзменениеСотрудникаСрезПоследних.Дизайнер ЕСТЬ NULL
...
Процедура ДеревоСотрудникиПередНачаломИзменения(Элемент, Отказ)
	...
	Если ПоказатьВсехСотрудников Тогда
	...
	Иначе
		...
		Отбор = ПредопределенноеЗначение("Перечисление.Должности." + ИмяКолонки);	
		
		Элемент.ТекущийЭлемент.СписокВыбора.Очистить();
		Для каждого СтруктураСотрудник Из СоответствиеСотрудниковПоДолжностям.Получить(Отбор) Цикл
		
			Если ИспользоватьФамилии И НЕ ПустаяСтрока(СтруктураСотрудник.Фамилия) Тогда
			
				Представление = СтруктураСотрудник.Фамилия;	
			
			Иначе
			
				Представление = СтруктураСотрудник.Наименование;	
			
			КонецЕсли;
			
			Элемент.ТекущийЭлемент.СписокВыбора.Добавить(СтруктураСотрудник.Сотрудник, Представление);	
...
Процедура ПриОткрытии(Отказ)
	СоответствиеСотрудниковПоДолжностям = омСотрудники.ЗаполнитьСписокСотрудниковПоДолжностям(ТолькоРаботающие);
КонецПроцедуры
