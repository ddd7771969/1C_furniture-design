...
Процедура Принять(Команда)
	...
	//Сообщить(Формат(ТекущаяДата(), "ДЛФ=DT") + " Начало создание задач ПКД");
	//
	КоличествоЗадач = СоздатьЗадачиПКД();
	//
	//ПараметрыПредметаИсчисления = "задача, задачи, задач, ж, , , , , 0";

	//Сообщить(Формат(ТекущаяДата(), "ДЛФ=DT") + " Создано " + НРег(ЧислоПрописью(КоличествоЗадач,, ПараметрыПредметаИсчисления)));
	//
	//Сообщить(Формат(ТекущаяДата(), "ДЛФ=DT") + " Начало переноса файла ТЗ");
	//
	//стРезультат = омТЗКлиент.СохранитьФайлТЗВоВременномКаталоге(Объект.ОбъектТЗ, Объект.Версия, ЭтаФорма.УникальныйИдентификатор, "Выгрузить");
	//
...
Процедура НазначитьОсновногоКонструктораНаСервере(КонструкторКомпозиции, мКомплекты)
	...
	Для каждого Комплект Из мКомплекты Цикл
		...
		Иначе	
			...
			ТекущаяСтадия = НайденаяСтрока.ТекущаяСтадия;
