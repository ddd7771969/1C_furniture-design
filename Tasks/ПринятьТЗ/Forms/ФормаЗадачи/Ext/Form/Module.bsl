...
Функция СоздатьЗадачиПКД()
	...
	Если НЕ ТипыВнешнегоПроизводства.ПКД Тогда
		...
		Если мСуществующиеЗадачи.Найти(Объект.ОбъектТЗ) = Неопределено Тогда
			...
			РегистрыСведений.ПлановоеЗначениеПроектирования.ЗаписатьПлановоеЗначениеЗадачи(НоваяЗадача, ТДНД, Объект.ПланРазработкиВДнях); 
		...
		Для каждого х Из Комплекты Цикл
			...
			РегистрыСведений.ПлановоеЗначениеПроектирования.ЗаписатьПлановоеЗначениеЗадачи(НоваяЗадача, ТДНД, х.ПланРазработкиВДнях); 
...
Процедура КомплектыПередНачаломИзменения(Элемент, Отказ)
	...
	Если НЕ Элемент.ТекущийЭлемент.Имя = "КомплектыПланРазработкиВДнях" Тогда
		Отказ = Истина;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Объект.ОбъектТЗ) = Тип("СправочникСсылка.Композиции") Тогда
		...
		Если Параметры.Ключ.Пустая() Тогда
			Комплекты.Загрузить(ЗаполнитьКомплекты(Объект.Ссылка, Объект.ОбъектТЗ, Истина));
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЗаполнитьКомплекты(ПринятьТЗ, Композиция, ВозвратВТаблице)
	...
	|		ПО (ПлановоеЗначениеПроектированияСрезПоследних.Комплект = ИзделияКомплект.Ссылка)
	|			И (ПлановоеЗначениеПроектированияСрезПоследних.Задача = &ПринятьТЗ)
	|ГДЕ
	...
	Запрос.УстановитьПараметр("ПринятьТЗ", 	ПринятьТЗ);
...
Процедура Обновить(Команда)
	...
	мКомплекты = ЗаполнитьКомплекты(Объект.Ссылка, Объект.ОбъектТЗ, Ложь);
