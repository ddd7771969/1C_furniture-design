...
Функция СоздатьЗадачиПКД()
	...
	Если НЕ ТипыВнешнегоПроизводства.ПКД Тогда
		...
		тзСотрудникПользователь = Запрос.Выполнить().Выгрузить();
		...
		Для каждого х Из Объект.Комплекты Цикл
			...
			стПараметры.ОбобщеннаяЗадача	= ЗначениеЗаполнено(стПараметры.Композиция) И КонструкторКомпозиции = х.Конструктор;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Элементы.ПланРазработкиВДнях.ТолькоПросмотр = Объект.Выполнена И Элементы.ОбъектТЗ.ТолькоПросмотр;
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ГлобальныйСтатус = Перечисления.СтатусыЗадач.ГлобальнаяПриостановкаРаботы Тогда
			ТолькоПросмотр 						= НЕ РольДоступна("Программист");	
...
Процедура ПриОткрытии(Отказ)
	...
	ПутьКФайлуПДМ = омФайлы.ПолучитьПутьКФайлуТЗ(Объект.ОбъектТЗ, , , Истина);

	Если ПустаяСтрока(ПутьКФайлуТЗ) Тогда
		Элементы.ГруппаКнопкиФайлаТЗ.Видимость = Ложь;
	Иначе
		
		Файл = Новый Файл(ПутьКФайлуТЗ);
		
		Если Файл.Существует() Тогда
			Элементы.ГруппаКнопкиФайлаТЗ.Видимость = Истина;
			омPDF_Клиент.ПоказатьФайл(ПутьКФайлуТЗ, ФайлТЗ);	
		Иначе
			Элементы.ГруппаКнопкиФайлаТЗ.Видимость = Ложь;
		КонецЕсли;
...
Процедура ЗаполнитьНаСервере()
	...
	Объект.Проект 		= Объект.ОбъектТЗ.Проект;
	Объект.Наименование = Объект.ОбъектТЗ.Наименование + " проверить ТЗ";
	
	Объект.Комплекты.Очистить();
	
	Если ТипЗнч(Объект.ОбъектТЗ) = Тип("СправочникСсылка.Композиции") Тогда
	
		Для каждого х Из омКомпозиции.ПолучитьКомплектыКомпозиции(Объект.ОбъектТЗ).ДанныеКомплектов Цикл
		
			НС = Объект.Комплекты.Добавить();
			ЗаполнитьЗначенияСвойств(НС, х);
			
			НС.ВыгруженВPDM = х.Код_в_PDM > 0;
			НС.ПланРазработкиВДнях = 1;
		
		КонецЦикла;
		
		Объект.ПланРазработкиВДнях = Объект.Комплекты.Количество();
...
Процедура ТаймингЗадачи(Команда)
	ОткрытьФорму("ОбщаяФорма.ТаймигОбъекта", Новый Структура("ОбъектТайминга", Объект.ОбъектТЗ), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
