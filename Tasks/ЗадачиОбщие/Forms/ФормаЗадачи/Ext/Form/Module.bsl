&НаКлиенте                                                 
Перем ВидимостьКнопкиНаблюдатели, стСотрудники ,НЕЗакрывать, КартинкаДобавлена;
...
Процедура ТипАдресацииПриИзменении(Элемент)
	...
	Иначе
		...
		ИначеЕсли ТипАдресации = "Проверить габаритные чертежи" Тогда
		
			Элементы.ОбъектАдресации.ОграничениеТипа = Новый ОписаниеТипов("ЗадачаСсылка.ПроверитьГабаритныеЧертежи");	
			ВидимостьКнопкиНаблюдатели 				 = Истина;
		
		ИначеЕсли ТипАдресации = "Создать модель габаритного чертежа" Тогда
			
			Элементы.ОбъектАдресации.ОграничениеТипа = Новый ОписаниеТипов("ЗадачаСсылка.СоздатьМодельГабаритногоЧертежа");	
			ВидимостьКнопкиНаблюдатели 				 = Истина;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если КартинкаДобавлена Тогда
		ЗаписатьФайлКартинкиНаСервере(АдресКартинки, Объект.Ссылка);
		КартинкаДобавлена = Ложь;
	КонецЕсли; 
	
	Если НЕ Объект.Выполнена Тогда
		омВзаимодействиеСервер.ОтправитьОповещениевФоне(Объект.Ссылка);
	КонецЕсли;
