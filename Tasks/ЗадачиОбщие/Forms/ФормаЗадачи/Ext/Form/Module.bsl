...
Процедура ОбновитьКартинки()	
	КлючКартинки = РегистрыСведений.КартинкиОбщихЗадач.СоздатьКлючЗаписи(Новый Структура("Задача", Объект.Ссылка));
...
Асинх Процедура ЗагрузитьКартинку()
	...
	Если ПомещенныйФайл <> Неопределено И Не ПомещенныйФайл.ПомещениеФайлаОтменено Тогда 
		...
		ЗаписатьФайлКартинкиНаСервере(ПомещенныйФайл.Адрес);
...
&НаСервере
Функция ЗаписатьФайлКартинкиНаСервере(АдресВХ)
//	
//	Объект..КартинкаКЗадаче = Новый ХранилищеЗначения(ПолучитьИзВременногоХранилища(АдресВХ));;	
//	
КонецФункции	
