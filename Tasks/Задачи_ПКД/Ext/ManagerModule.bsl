...
Функция ДействияНаСервере(Действие, Знач мКомплектыДляАналогичногоДействия, Знач Задача, ТипЗадачи, Исполнитель = Неопределено, Параметр = Неопределено) Экспорт
	...
	ТипыЗадачДляКонструктора 	= омЗадачиПКДПовтор.ТипыЗадачКонструктора(); 
	...
	ИначеЕсли НЕ ТипыЗадачДляКонструктора.Найти(ТипЗадачи) = Неопределено Тогда
	...
	ИначеЕсли ТипЗадачи = Перечисления.ТипыЗадачКонструирования.ПодтверждениеВедущимКонструктором ИЛИ ТипЗадачи = Перечисления.ТипыЗадачКонструирования.КонтрольДизайнера Тогда
...
Процедура СоздатьЗадачуРКД(ОбъектЗадача, Конструктор, СтатусЗадачи = Неопределено, ТипЗадачи = Неопределено, СостоянияЗадач = Неопределено) Экспорт
	...
	ЗадачаРКД.Дата 				= ТекущаяДата();
	ЗадачаРКД.Наименование		= Строка(ОбъектТЗ) + ".РКД";
	...
	ЗадачаРКД.Наименование		= Строка(ОбъектТЗ) + СуфиксНаименованияЗадачиРКД(стДанные);
