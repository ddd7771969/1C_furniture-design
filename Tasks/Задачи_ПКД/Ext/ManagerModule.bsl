...
Процедура ДействиеСогласовано(Задача)
	...
	ОбъектЗадача = ЗавершитьЗадачуПКД(Задача);
	...
	Иначе
		...
		спКомплекты = Новый СписокЗначений;
		омКомпозицииСервер.ПолучитьКомплектыКомпозиции(ОбъектЗадача.ОбъектТЗ, спКомплекты);
		...
		Запрос.УстановитьПараметр("Комплект", спКомплекты);
		
		Выборка = Запрос.Выполнить().Выбрать();
		
		Пока Выборка.Следующий() Цикл
			ОбъектЗадача = ЗавершитьЗадачуПКД(Выборка.Задача);
...
Функция ЗавершитьЗадачуПКД(СсылкаНаЗадачу)
