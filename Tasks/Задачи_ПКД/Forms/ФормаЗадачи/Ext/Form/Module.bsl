...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПроверитьДополнительноеТЗ(Объект.Ссылка);
...
Функция ПроверитьДополнительноеТЗ(Ссылка)
	...
	Запрос.Текст = 
		...
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Дополнение_к_ТЗ.Комплект) КАК Комплект
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		стрДополнениеТЗ = СтрШаблон("Всего %1. Ознакомлено %2. С файлами %3"
