...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если КонтрольДлительнойОперации(НаименованиеДействия) Тогда
...
Функция КонтрольДлительнойОперации(НаименованиеДействия)
...
Процедура НачатьВыполнение(Команда)
	...
	Если НуженЗапускСкрипта Тогда
		...
		Иначе	
			...
			ДанныеОперации = омОчередьПДМНаСервере.ПоставитьОбъектВОчередь(Объект.Ссылка, ПредопределенноеЗначение("Перечисление.ТипыОбменаPDM.НазначениеПрав"));
			
			Если НЕ (ПустаяСтрока(ДанныеОперации.ОписаниеОшибки) ИЛИ ПустаяСтрока(ДанныеОперации.УИД)) Тогда
				омОчередьПДМНаСервере.СоздатьЗаписьВЛог(ДанныеОперации.УИД, ДанныеОперации.ОписаниеОшибки, Ложь);
			КонецЕсли;
			
			ЗапустьФоновоеЗаданиеДляДействия();
			
			ПодключитьОбработчикОжидания("ЗаполнитьСостояниеДействияПДМ", 1);
		КонецЕсли;
...
Процедура ЗавершитьВыполнение(Команда)
	...
	Если типЗнч(Объект.ОбъектТЗ) = Тип("СправочникСсылка.ИзделияКомплект") Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ФормаРазрегестрированыеФайлы", Новый Структура("Задача, мКомплектыДляАналогичногоДействия", Объект.Ссылка, мКомплектыДляАналогичногоДействия)); 
...
Процедура СбросКонструктором(Команда)
	...
	ДействияНаСервере("ИзменениеКонструктора", мКомплектыДляАналогичногоДействия, Объект.Ссылка, Объект.ТипЗадачи, Объект.Исполнитель);
