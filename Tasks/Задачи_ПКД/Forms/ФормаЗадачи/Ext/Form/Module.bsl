...
Процедура НачатьВыполнение(Команда)
	...
	Если НуженЗапускСкрипта Тогда
		...
		Если ДоступенПрограммист() Тогда
			омОчередьПДМНаСервере.ПоставитьОбъектВОчередь(Объект.ОбъектТЗ, ПредопределенноеЗначение("Перечисление.ТипыОбменаPDM.НазначениеПрав"));
...
Функция ДоступенПрограммист()
	Возврат РольДоступна("Программист");	
КонецФункции
