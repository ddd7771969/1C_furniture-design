...
Процедура ЗавершитьВыполнение(Команда)
	...
	Если типЗнч(Объект.ОбъектТЗ) = Тип("СправочникСсылка.ИзделияКомплект") Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ФормаСтруктураКаталоговPDM", Новый Структура("ЗадачаПКД", Объект.Ссылка)); 
...
Процедура ДоработкаВедущийКонструктор(Команда)
	...
	ДействияНаСервере("ДоработкаВедущийКонструктор", мКомплектыДляАналогичногоДействия, Объект.Ссылка, Объект.ТипЗадачи, Объект.Исполнитель);
	Закрыть();
