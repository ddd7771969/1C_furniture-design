...
Процедура НачатьВыполнение(Команда)
	...
	Если НуженЗапускСкрипта Тогда
		...
		Иначе	
			...
			//Результат = омФайлыPDM.ИзменитьПраваКомплекту(Объект.ОбъектТЗ, Истина); 
			//
			//Если Лев(Результат, 1) = ";" Тогда
			//	
			//	ДействияНаСервере("НачатьВыполнение", мКомплектыДляАналогичногоДействия, Объект.Ссылка, Объект.ТипЗадачи, Объект.Исполнитель);
			//	Закрыть();
			//	
			//Иначе
			//	
...
Процедура ЗавершитьВыполнение(Команда)
	...
	//	
	//	СруктураПараметров = Новый Структура("ЗадачаСсылка, НаименованиеДействия, ИмяДействияДляФайла", Объект.Ссылка, "ЗавершениеРаботы", "");
	//	УИДДействия = омФайлыPDM.СоздатьДействие("ЗавершениеРаботы", СруктураПараметров);
	//	ИзменитьПраваПДМ(УИДДействия, Ложь, "ЗавершениеРаботы", СруктураПараметров)
	//	
	//Иначе
	//	
	//	ДействияНаСервере("ЗавершениеРаботы", мКомплектыДляАналогичногоДействия, Объект.Ссылка, Объект.ТипЗадачи, Объект.Исполнитель, Объект.ОбъектТЗ);
	//	
...
//&НаКлиенте
//Процедура ИзменитьПраваПДМ(УИДДействия, УстановитьПрава, ДействиеПриОкончании, СруктураПараметров)
//	
//	ПараметрыФункции 	= Новый Массив(5);
//	ПараметрыФункции[0] = УИДДействия;	
//	ПараметрыФункции[1] = Объект.ОбъектТЗ;	
//	ПараметрыФункции[2] = УстановитьПрава;	
//	ПараметрыФункции[3] = СруктураПараметров;	
//	ПараметрыФункции[4] = ДействиеПриОкончании;	
//	омФайлыPDM.ЗапуститьФоновыеЗадания("омФайлыPDM.ИзменитьПраваКомплекту", ПараметрыФункции, "ИзменитьПраваКомплекту");  
//	
//    Элементы.Группа2.Доступность = Ложь;
//	ПодключитьОбработчикОжидания("КонтрольДлительнойОперацииНаКлиенте", 0.5, Истина);
//	
//КонецПроцедуры // ИзменитьПраваПДМ() 

//&НаКлиенте
//Процедура КонтрольДлительнойОперацииНаКлиенте() Экспорт
...
//	КонецЕсли;
//	
//КонецПроцедуры // КонтрольДлительнойОперации()
// 
