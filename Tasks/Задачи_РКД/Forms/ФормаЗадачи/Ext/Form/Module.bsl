...
Процедура ИнфрмацияГЧ()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ИзделияКомплект.Родитель.ГабаритныйЧертеж ЕСТЬ NULL
		|				ИЛИ ИзделияКомплект.Родитель.ГабаритныйЧертеж = ЗНАЧЕНИЕ(Справочник.ГабаритныеЧертежи.ПустаяСсылка)
