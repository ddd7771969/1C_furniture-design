...
Процедура ИнфрмацияГЧ()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СостояниеЗамераСрезПоследних.Период КАК Дата_статуса_ГЧ,
		|	СостояниеЗамераСрезПоследних.ОбъектИзменения КАК ОбъектИзмененияГЧ,
		...
		|		ПО ИзделияКомплект.ГабаритныйЧертеж = СостояниеЗамераСрезПоследних.ОбъектЗамера
