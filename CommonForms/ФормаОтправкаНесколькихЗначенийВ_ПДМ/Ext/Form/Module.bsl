&НаКлиенте
Перем ПутьКФайлуEXE, ПутьКФайлуЗагрузки, ПутьКФайлуEXEКомпозиции, ТестКаталог;
...
Процедура ПриОткрытии(Отказ)
	...
	ПутьКФайлуEXEКомпозиции = ом_ПДМ_клиент_сервер.ПолучитьПутьКВыгузке(ТипВыгрузки + "Композиции", стДанныеВыгрузки);
	
	Если ПустаяСтрока(ПутьКФайлуEXE) Тогда       
		ТекущиеДействие = "Не заполнен путь к исполняемому файлу комплекта.";        
		Возврат;	
	КонецЕсли;
	
	Если ПустаяСтрока(ПутьКФайлуEXEКомпозиции) Тогда       
		ТекущиеДействие = "Не заполнен путь к исполняемому файлу композиции.";        
		Возврат;	
	КонецЕсли;
...
Процедура ВыгрузитьДанные()
	...
	Если ТипВыгрузки = "РаздатьПрава" ИЛИ ТипВыгрузки = "ЗабратьПрава"
		ИЛИ ТипВыгрузки = "РаздатьПраваКомпозиции" ИЛИ ТипВыгрузки = "ЗабратьПраваКомпозиции" Тогда     
		...
		ТекстДок = ом_ПДМ_клиент_сервер.СоздатьФайлВыгрузки(мДанныеЭлемента, ТипВыгрузки);
	...
	Если ТипЗнч(тзДанные[НомерСтрокиВыгрузки -1].ОбъектТЗ) = Тип("СправочникСсылка.ИзделияКомплект") Тогда
		ИмяФайла = ТестКаталог.ПолноеИмя + "\" + НаименованиеЭлемента + ".csv";
	Иначе
		ИмяФайла = ТестКаталог.ПолноеИмя + ?(ТипВыгрузки = "РаздатьПрава", "\SetPermissionsComposition.csv", "\MovePermissionsComposition.csv");
	КонецЕсли;
	...
	Попытка
		...
		Если НЕ ПустаяСтрока(ПутьКФайлуEXE) Тогда
			...
			Если ТипЗнч(тзДанные[НомерСтрокиВыгрузки -1].ОбъектТЗ) = Тип("СправочникСсылка.ИзделияКомплект") Тогда
				стрОшибка = ом_ПДМ_клиент_сервер.ВыгрузитьФайлВPDM(ПутьКФайлуEXE);
			Иначе
				стрОшибка = ом_ПДМ_клиент_сервер.ВыгрузитьФайлВPDM(ПутьКФайлуEXEКомпозиции);
			КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменилсяНаборПравPDM" Тогда
		...
		Если НомерСтрокиВыгрузки > тзДанные.Количество() - 1 Тогда
