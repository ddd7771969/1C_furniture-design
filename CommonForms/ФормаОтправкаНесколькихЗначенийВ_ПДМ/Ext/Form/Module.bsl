...
Процедура ВыгрузитьДанные()
	...
	Если ТипВыгрузки = "РаздатьПрава" ИЛИ ТипВыгрузки = "ЗабратьПрава" Тогда     
		...
		мДанныеЭлемента.Добавить(Новый Структура("ID_PDM, UsrName", тзДанные[НомерСтрокиВыгрузки -1].ID_PDM, тзДанные[НомерСтрокиВыгрузки - 1].UsrName));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ИзменилсяНаборПравPDM" Тогда
		...
		Если ТипВыгрузки = "РаздатьПрава" ИЛИ ТипВыгрузки = "ЗабратьПрава" Тогда     
			...
			стОтбор = Новый Структура("ID_PDM", Число(Формат(Параметр.КодЭлемента, "ЧГ=0")));
			НайденыеСтроки = тзДанные.НайтиСтроки(стОтбор);
			...
			мДанныеЭлемента[0] 	= Новый Структура("ID_PDM, UsrName", тзДанные[НомерСтрокиВыгрузки -1].ID_PDM, тзДанные[НомерСтрокиВыгрузки - 1].UsrName);
