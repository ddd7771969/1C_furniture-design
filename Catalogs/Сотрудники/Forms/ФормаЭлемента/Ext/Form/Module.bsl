&НаКлиенте 
Перем ПодразделенияИзменены, PDMИзменены, НачальноеЗначениеUserID, ИзмененыКонтактныеДанные, НачальныйРежимРаботы; 
...
Процедура ПриОткрытии(Отказ)
	...
	НачальныйРежимРаботы 		= ПолучитьРежимРаботыСотрудника(Объект.Ссылка);
	РежимРаботы					= НачальныйРежимРаботы;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НЕ НачальныйРежимРаботы = РежимРаботы Тогда
		
		ЗаписатьРежимРаботыНаСервере(Объект.Ссылка, РежимРаботы);
		НачальныйРежимРаботы = РежимРаботы;
...
&НаСервереБезКонтекста
Функция ПолучитьРежимРаботыСотрудника(Сотрудник)
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.ДанныеСотрудников.СрезПоследних(, Сотрудник = &Сотрудник) КАК ДанныеСотрудниковСрезПоследних";
...
КонецФункции // ПолучитьРежимРаботыСотрудника()
...
&НаСервереБезКонтекста
Процедура ЗаписатьРежимРаботыНаСервере(Сотрудник, РежимРаботы)
	
	ТД = ТекущаяДата();
	
	МенеджерЗаписи = РегистрыСведений.ДанныеСотрудников.СоздатьМенеджерЗаписи();
	...
	МенеджерЗаписи.Период = ТД;
	МенеджерЗаписи.Сотрудник = Сотрудник;
	МенеджерЗаписи.РежимРаботы = РежимРаботы;
...
&НаКлиенте
Процедура ЗаписатьРежимРаботы(Команда)
