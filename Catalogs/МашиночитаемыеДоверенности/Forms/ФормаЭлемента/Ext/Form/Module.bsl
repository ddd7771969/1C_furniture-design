...
Процедура ДатаОкончанияПриИзменении(Элемент)
	...
	Если Просрочена Тогда
		ТехническийСтатус = ЭтотОбъект.ТехническийСтатус_Просрочено;
		ТребуетсяОбновлениеДекораций = Истина;
		ОтобразитьДекорацииСтатуса(ЭтотОбъект);
	Иначе
		ТехническийСтатус = ЭтотОбъект.ТехническийСтатус_Создание;
		ТребуетсяОбновлениеДекораций = Истина;
		ОтобразитьДекорацииСтатуса(ЭтотОбъект);
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыЭлектронныеПодписи
...
#Область ОбработчикиСобытийЭлементовТаблицыТаблицаПредставительНесколько
...
Процедура Подписать(Команда)
	...
	Отказ = Ложь;
	ОбновитьСформироватьФайлДоверенности(Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьПодпись(Команда)
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		...
		ЭлектроннаяПодписьКлиент.ПроверитьПодпись(Новый ОписаниеОповещения("ПослеПроверкиПодписи", ЭтотОбъект, Строка),
			ДанныеФайла.СсылкаНаДвоичныеДанныеФайла, ЭлектронныеПодписи.НайтиПоИдентификатору(Строка).АдресПодписи,,,ПараметрыПроверки);
	КонецЦикла;
...
&НаКлиенте
Процедура ПослеПроверкиПодписи(РезультатПроверки, Строка) Экспорт
	
	Если РезультатПроверки.Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СтрокаПодписи = ЭлектронныеПодписи.НайтиПоИдентификатору(Строка);
	
	ЗаполнитьЗначенияСвойств(СтрокаПодписи, РезультатПроверки);
	Если ТипЗнч(РезультатПроверки.Результат) = Тип("Строка") Тогда
		СтрокаПодписи.ОписаниеОшибки = РезультатПроверки.Результат;
	КонецЕсли;
	
	ДатаСеанса = ОбщегоНазначенияКлиент.ДатаСеанса();
	СтрокаПодписи.ДатаПроверкиПодписи = ДатаСеанса;
	
	СтатусПодписи = СтатусПодписи(СтрокаПодписи, ДатаСеанса);
	Если СтрокаПодписи.Статус <> СтатусПодписи Тогда
		СохранитьРезультатПроверкиПодписей = Истина;
		Модифицированность = Истина;
	КонецЕсли;
	СтрокаПодписи.Статус = СтатусПодписи(СтрокаПодписи, ДатаСеанса);
...
Процедура СохранитьРезультатПроверкиПодписейНаСервере()
	...
	Свойства = Новый Структура("ДатаПроверкиПодписи, ПодписьВерна, ТребуетсяПроверка");
	
	Для Каждого Подпись Из УстановленныеПодписи Цикл
		
		Отбор = Новый Структура("ИдентификаторПодписи", Подпись.ИдентификаторПодписи);
		Найдено = ЭлектронныеПодписи.НайтиСтроки(Отбор)[0];
		
		Если Подпись.ДатаПодписи         <> Найдено.ДатаПодписи
		 Или Подпись.Комментарий         <> Найдено.Комментарий
		 Или Подпись.КомуВыданСертификат <> Найдено.КомуВыданСертификат И ЗначениеЗаполнено(Найдено.КомуВыданСертификат)
		 Или Подпись.Отпечаток           <> Найдено.Отпечаток И ЗначениеЗаполнено(Найдено.Отпечаток)
		 Или Подпись.УстановившийПодпись <> Найдено.УстановившийПодпись Тогда
			Продолжить; // Если объект был изменен, результаты проверки не переносятся.
		КонецЕсли;
		
		ЕстьИзменения = Ложь;
		
		Для Каждого КлючИЗначение Из Свойства Цикл
			Если Подпись[КлючИЗначение.Ключ] <> Найдено[КлючИЗначение.Ключ] Тогда
		...
		ЗаполнитьЗначенияСвойств(Подпись, Найдено, "ДатаПроверкиПодписи, ПодписьВерна, ТребуетсяПроверка");
		ЭлектроннаяПодпись.ОбновитьПодпись(Объект.ФайлДоверенности, Подпись);
...
Функция ЗаполнениеПроверено()
	...
	Для Каждого Строка Из Объект.Представители Цикл
		...
		Если Не МашиночитаемыеДоверенностиФНССлужебный.ПроверитьСведенияОбОрганизации(
			ЭтотОбъект, Поле,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Представитель %1 - %2'"), Строка.Представитель, " %1:"),
...
Процедура ПослеПодписанияДоверенности(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Успех Тогда
		...
		ДобавитьПодпись(СвойстваПодписи, Отказ);
...
&НаСервере
Процедура СформироватьТаблицуНесколькоПредставителей(ИдентификаторРодителя = Неопределено);
...
Процедура ПослеДобавленияПодписи(Результат, Контекст) Экспорт
	...
	ДобавитьПодпись(Результат.ОписаниеДанных.Подписи);
	ОбновитьСтатус();
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьПодпись(СвойстваПодписи, Отказ = Ложь)
	...
	Если ТипЗнч(СвойстваПодписи) = Тип("Массив") Тогда
		Для Каждого Подпись Из СвойстваПодписи Цикл
			...
			Если Результат <> Истина Тогда
				ОбщегоНазначения.СообщитьПользователю(Результат,,,,Отказ);
			КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьСформироватьФайлДоверенности(Отказ)
	...
	Если ЭлектронныеПодписи.Количество() = 0 Тогда
		...
		Если Объект.РегистрироватьВРеестре И Не ЗначениеЗаполнено(Объект.НомерДоверенности) Тогда
			...
			Иначе
				КонтекстОшибки = МашиночитаемыеДоверенностиФНССлужебныйКлиент.КонтекстДляОбработкиОшибкиРР();
				КонтекстОшибки.ЗаголовокПредупреждения = НСтр("ru = 'Не удалось получить номер доверенности'");
				КонтекстОшибки.Форма = ЭтотОбъект;
				МашиночитаемыеДоверенностиФНССлужебныйКлиент.ОбработатьОшибкуВзаимодействияРР(Результат.Ошибка, КонтекстОшибки);
				Отказ = Истина;
				Возврат;
			КонецЕсли;
		ИначеЕсли Не ЗначениеЗаполнено(Объект.НомерДоверенности) Тогда
			Объект.НомерДоверенности = Строка(Новый УникальныйИдентификатор);
			Модифицированность = Истина;
		КонецЕсли; 
		
		СформироватьФайлДоверенности(Отказ);
		ОповеститьОЗаписи();
...
&НаСервере
Процедура СформироватьФайлДоверенности(Отказ)
...
Процедура УстановитьВидимостьИДоступность(Форма, Условие = Неопределено)
	...
	Если Условие = Неопределено Или Условие = "Подписание" Тогда
		...
		Просрочена = Форма.Просрочена;
		
		КоличествоПодписей = Форма.ЭлектронныеПодписи.Количество();
		
		Форма.Элементы.ФормаОтправить.Видимость = Ложь;
		Форма.Элементы.ФормаЗакрыть.Видимость = Ложь;
		...
		РеквизитыНедоступныДляРедактирования = КоличествоПодписей > 0; 
		...
		Форма.Элементы.ГруппаНомерВРеестре.ТолькоПросмотр = РеквизитыНедоступныДляРедактирования;
		...
		Если КоличествоПодписей > 0 Тогда
			...
			Иначе
				Форма.Элементы.ГруппаЭлектронныеПодписи.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Подписи доверителей (%1)'"), КоличествоПодписей);
			КонецЕсли;
			
			Если Форма.Подписана Тогда
				Если Не Форма.Объект.РегистрироватьВРеестре Тогда
					Форма.Элементы.ФормаЗакрыть.Видимость = Истина;
					Форма.Элементы.ФормаЗакрыть.КнопкаПоУмолчанию = Истина;
				Иначе 
					Форма.Элементы.ФормаОтправить.Видимость = Не Просрочена
						И Форма.Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыМЧД.Черновик");
					Форма.Элементы.ФормаОтправить.КнопкаПоУмолчанию = Не Просрочена И Форма.Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыМЧД.Черновик");
				КонецЕсли;
				Форма.Элементы.ЭлектронныеПодписиДобавитьПодпись.Видимость = Ложь;
				Форма.Элементы.ФормаДобавитьПодпись.Видимость = Ложь;
			Иначе
				Форма.Элементы.ФормаПодписать.Видимость = Не Просрочена;
				Форма.Элементы.ЭлектронныеПодписиДобавитьПодпись.Видимость = Истина;
				Форма.Элементы.ФормаПодписать.КнопкаПоУмолчанию = Не Просрочена;
		...
		Если РеквизитыНедоступныДляРедактирования Тогда
			...
			Иначе
				Форма.Элементы.ПредставительТипДекорация.Заголовок = ?(Форма.ПредставительРеквизиты.ЭтоФизическоеЛицо,
...
Процедура ОтобразитьДекорацииСтатуса(Форма)
	Если Форма.ТребуетсяОбновлениеДекораций Тогда
		Форма.Элементы.ДекорацияПросрочена.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Просрочено"];
		Форма.Элементы.ДекорацияСоздание.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Создание"];
		Форма.Элементы.ДекорацияПодписана.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Подписан"];
		Форма.Элементы.ДекорацияГотово.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Активна"];
		Форма.Элементы.ДекорацияЗарегистрирована.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Зарегистрировано"]
			Или Форма.ТехническийСтатус = Форма["ТехническийСтатус_ДатаНачалаДействияНеНаступила"];
		Форма.Элементы.ДекорацияПодписание.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Подписание"];
		Форма.Элементы.ДекорацияРегистрация.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Регистрация"];
		Форма.Элементы.ДекорацияОтменена.Видимость = Форма.ТехническийСтатус = Форма["ТехническийСтатус_Отменена"]
			Или Форма.ТехническийСтатус = Форма["ТехническийСтатус_ОшибкаРегистрации"];
...
&НаСервере
Процедура ЗаполнитьТипОрганизации(ИмяЭлементаОрганизация, ТипСтрока)
...
&НаСервере
Процедура ПриИзмененииОрганизацииНаСервере(ИмяЭлементаОрганизация, Загрузка = Ложь)
	
	ЗаполнитьРеквизитыОрганизации(ИмяЭлементаОрганизация, Загрузка);
	
	Если ИмяЭлементаОрганизация = "Доверитель" И ДоверительТипСтрока <> "ФизическоеЛицо" Тогда
		Если ДоверительРеквизиты.ЭтоИндивидуальныйПредприниматель Тогда
			ЛицоБезДоверенностиНесколько = Ложь;
		КонецЕсли;
		
	ИначеЕсли ИмяЭлементаОрганизация = "Представитель" И ПредставительТипСтрока <> "ФизическоеЛицо" Тогда
		
		Если ПредставительРеквизиты.ЭтоИндивидуальныйПредприниматель Тогда
			ПредставительФЛНесколько = Ложь;
		КонецЕсли;
...
&НаСервере
Процедура ОбновитьСтрокуДоверителейПредставителей(ИмяЭлементаОрганизация) 
...
&НаСервере
Процедура ОчисткаДоверителяПредставителя(ИмяЭлементаОрганизация)
...
Процедура ЗаполнитьРеквизитыРуководителя(ИмяЭлементаОрганизация, Загрузка = Ложь)
	...
	Если ЭтотОбъект[ИмяЭлементаОрганизация + "Реквизиты"].Свойство("ЛицоБезДоверенности")
		И ЗначениеЗаполнено(ЭтотОбъект[ИмяЭлементаОрганизация + "Реквизиты"].ЛицоБезДоверенности) Тогда
...
Функция ЗаголовокДекорацииТаблицы(Реквизиты, ИмяЭлементаОрганизация = Неопределено, ТолькоОбязательные = Ложь)
	...
	Возврат МашиночитаемыеДоверенностиФНССлужебный.ЗаголовокДекорации(Реквизиты, ИмяЭлементаОрганизация, ТолькоОбязательные, Истина);
...
&НаСервере
Процедура ЗаполнитьРеквизитыОрганизации(ИмяЭлементаОрганизация, Загрузка = Ложь)
...
&НаСервере
Процедура УстановитьОтображениеРеквизитовЗависимыхОтОрганизации(ИмяЭлементаОрганизация)
...
Процедура ДобавитьВТаблицуПредставителя(Результат, Контекст) Экспорт
	...
	Ссылка = Неопределено;
	Результат.Свойство("Ссылка", Ссылка);
	Представители = Неопределено;
	Результат.Свойство("Представители", Представители);
	...
	НоваяСтрока.Представитель = Ссылка;
	...
	Если ЗначениеЗаполнено(Представители) Тогда
		Для Каждого ТекущийПредставитель Из Представители Цикл
...
&НаСервере
Процедура ОчиститьЭлементыФормы(ИмяЭлементаОрганизация, ОчищатьИдентификатор = Ложь)
...
&НаСервере
Процедура УдалитьПодписьНаСервере(ПорядковыйНомер)
	
	ЭлектроннаяПодпись.УдалитьПодпись(Объект.ФайлДоверенности, ПорядковыйНомер, УникальныйИдентификатор);
	
	МашиночитаемыеДоверенностиФНССлужебный.УстановитьПризнакПодписана(Объект.Ссылка, Ложь);
	МашиночитаемыеДоверенностиФНССлужебный.РассчитатьИЗаписатьТехническийСтатус(Объект.Ссылка);
...
Процедура ОбновитьСтатус(НаправитьЗапросВРР = Ложь)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) И Объект.РегистрироватьВРеестре Тогда
		...
		ДанныеСостояния = ОбновитьСтатусИПрочитатьСостояние(Объект.Ссылка, НаправитьЗапросВРР);
		...
		Если ДанныеСостояния.ОтображатьСтатусВДокументе Тогда
	...
	ОтобразитьДекорацииСтатуса(ЭтотОбъект);
	Если ТехническийСтатус = ЭтотОбъект.ТехническийСтатус_Регистрация Тогда
...
&НаСервереБезКонтекста
Функция ОбновитьСтатусИПрочитатьСостояние(Ссылка, НаправитьЗапросВРР)
...
&НаСервереБезКонтекста
Процедура УстановитьСтатусРегистрации(Доверенность, ИдентификаторТранзакции = Неопределено)
