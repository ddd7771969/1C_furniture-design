...
Процедура ИзменитьНумерациюНаСервере(Сделка, НачальноеЗначениеНомерПоПорядку, ТекущиеЗначение, ИзделиеЗаказчика)
	...
	Запрос.Текст = 
		...
		|	ИзделияЭлемент.Проект.Префикс КАК ПроектПрефикс,
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ Выборка.НомерПоПорядку = НачальныйНомер Тогда
			...
			обИзделиеЭлемент.Артикул = омИзделиеЭлементНаКлиентеСервере.СобратьАртикулИзДанных(Выборка.ПроектПрефикс, Выборка.НомерПомещения, обИзделиеЭлемент.НомерПоПорядку);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если ЗначениеЗаполнено(ТекущийОбъект.Проект) Тогда
		ПроектПрефикс = ТекущийОбъект.Проект.Префикс;	
	Иначе
		ПроектПрефикс = "";
	КонецЕсли;
	...
	ТекущийОбъект.Артикул = омИзделиеЭлементНаКлиентеСервере.СобратьАртикулИзДанных(ПроектПрефикс, НомерПомещения, ТекущийОбъект.НомерПоПорядку);
