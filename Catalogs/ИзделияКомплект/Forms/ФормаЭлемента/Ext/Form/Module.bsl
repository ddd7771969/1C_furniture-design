...
&НаКлиенте
Перем НормаВремениПроизводстваИзменено, НормаВремениМонтажаИзменено, ПутьКФайлуПДМ, ПутьКФайлуТЗ, НачальноеКоличествоСогласований;

&НаКлиенте
Перем НормаВремениПКДРКД, НачальныйКонструктор, НачальнаяСтадия, стПраваВыгрузкаВСРМ, НачальныйБезГабаритногоЧертежа;
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ФормаГруппаPDM.Видимость = ЗначениеЗаполнено(Код_в_PDM);
	
	НачальныйБезГабаритногоЧертежа = БезГабаритногоЧертежа;
	...
	НачальноеКоличествоСогласований	= КоличествоСогласований;
...
Процедура ЗаписатьЗначениеВРегистНаСервере(Изменить)

	МенеджерЗаписи = РегистрыСведений.ДатыКомплекта.СоздатьМенеджерЗаписи();
	ЗаполнитьЗначенияСвойств(МенеджерЗаписи, Изменить, , "БезГабаритногоЧертежа");
	МенеджерЗаписи.Прочитать();
	
	Если Изменить.БезГабаритногоЧертежа = МенеджерЗаписи.БезГабаритногоЧертежа Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(МенеджерЗаписи, Изменить);
	
	МенеджерЗаписи.Записать();

КонецПроцедуры // ЗаписатьЗначениеВРегистНаСервере(Изменить)()
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НЕ (НачальныйКонструктор = Конструктор И НачальнаяСтадия = ТекущаяСтадия И НачальноеКоличествоСогласований = КоличествоСогласований) Тогда
		ЗаписатьКонструктора(Объект.Ссылка, Конструктор, ТекущаяСтадия, НачальныйКонструктор = Конструктор, КоличествоСогласований);
	...
	Если НормаВремениПКДРКД Тогда
		...
		стДанные = Новый Структура("НормаВремени_ПКД, НормаВремени_РКД", );
		ЗаполнитьЗначенияСвойств(стДанные, ЭтаФорма);
		
		ИзменитьНормыВремениНаСервере(Объект.Ссылка, Объект.Проект, стДанные);
		
	КонецЕсли; 
	
    Если НЕ	НачальныйБезГабаритногоЧертежа = БезГабаритногоЧертежа Тогда
		
		Изменить = Новый Структура("Комплект, Проект, БезГабаритногоЧертежа",
									Объект.Ссылка,
									Объект.Проект,
									БезГабаритногоЧертежа);
		
		ЗаписатьЗначениеВРегистНаСервере(Изменить);	
...
&НаСервереБезКонтекста
Функция ИзменитьНормыВремениНаСервере(Комплект, Проект, стДанные)
	...
	Если ЗначениеЗаполнено(МенеджерЗаписей.Комплект) Тогда
		...
		Если НЕ МенеджерЗаписей.НормаВремени_ПКД = стДанные.НормаВремени_ПКД Тогда
		...
		Если НЕ МенеджерЗаписей.НормаВремени_РКД = стДанные.НормаВремени_РКД Тогда
...
&НаСервереБезКонтекста
Процедура ЗаписатьКонструктора(Комплект, Конструктор, ТекущаяСтадия, КонструкторНеИзменен, КоличествоСогласований)
	
	стПараметры = Новый Структура("Комплект, Конструктор, ТекущаяСтадия, КоличествоСогласований", Комплект, Конструктор, ТекущаяСтадия, КоличествоСогласований);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Ключ.Пустая() Тогда
		...
		ЗаполнитьЗначенияСвойств(ЭтаФорма, омСотрудники.СотрудникиОбъекта(Объект.Ссылка, Ложь));

		ЗаполнитьЗначенияСвойств(ЭтаФорма, ПолучитьДанныеИзДатыКомплекта(Объект.Ссылка));
		
	КонецЕсли;
	
	ДоступныПраваДляВидимостиКнопкиТЗ = Истина;
	
	Если НЕ РольДоступна("ПолныеПрава") Тогда
		
		Элементы.ФормаИзменитСтатус.Видимость 			= Ложь;
		Элементы.УстановитьКодPDM.Видимость 			= Ложь;
		Элементы.ГруппаНормы.ТолькоПросмотр				= Истина;
		Элементы.КоличествоСогласований.ТолькоПросмотр	= Истина;
		...
		Если РольДоступна("Дизайнер") Тогда
			...
			Элементы.Группа7.ТолькоПросмотр 		= Истина;
			Элементы.ГруппаИзделия.ТолькоПросмотр 	= Истина;
			Элементы.ГруппаНормы.ТолькоПросмотр 	= Истина;
			Элементы.ГруппаЗадачи.ТолькоПросмотр 	= Истина;
			Элементы.Группа12.ТолькоПросмотр 		= Истина;
			Элементы.ГруппаКоды.Видимость 			= Ложь;
			
		Иначе
			Элементы.ФормаЗагрузитьТЗ.Видимость 	= Ложь;
			ДоступныПраваДляВидимостиКнопкиТЗ 		= Ложь;
		КонецЕсли;
		
	КонецЕсли;   
	
	Если ДоступныПраваДляВидимостиКнопкиТЗ Тогда
	...
	ИсторияТЗ.Загрузить(омИстория.ПолучитьИсториюТЗ(Объект.Ссылка));
	
КонецПроцедуры 

Функция ПолучитьДанныеИзДатыКомплекта(Комплект)

	Данные = Новый Структура("БезГабаритногоЧертежа", Ложь);
...
КонецФункции // ПолучитьДанныеИзДатыКомплекта()
...
Процедура ЗаполнитьТекущиеСостояние()
	...
	|	ДанныеКомплекта.КоличествоСогласований КАК КоличествоСогласований
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		ТекущаяСтадияПН 		= Выборка.ТекущаяСтадияПолноеНаименование;
...
Процедура ЗаполнитьТаблицуЗадач()
	...
	СтруктураПараметров.ОбъектАдресации = Объект.Ссылка;
	СтруктураПараметров.НеВыполненые 	= Ложь;
	СтруктураПараметров.Исполнитель 	= Справочники.Пользователи.ПустаяСсылка();
	ТаблицаОбщихЗадач.Загрузить(омЗадачаОбщаяНаСервере.СформироватьТаблицуОбщихЗадач(СтруктураПараметров));
	
	КоличествоЗадач = ТаблицаОбщихЗадач.Количество();
	
	Если КоличествоЗадач = 0 Тогда
		Элементы.ГруппаЗадачи.Видимость = Ложь; 
	Иначе
		Элементы.ГруппаЗадачи.Заголовок = "Задачи (" + КоличествоЗадач + ")";
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ИзменилсяСтатусКомплекта" Тогда
		Если Параметр.Комплект = Объект.Ссылка Тогда
...
Процедура ЗаписатьКод_в_PDM_НаСервере(Код_в_PDM, КомплектСсылка)
	...
	ПроектСсылка = КомплектСсылка.Проект;
...
Процедура ЗаполнитьКомпозиции()
	...
	Элементы.ДекорацияЗадачаПКД.Видимость = Ложь;
	Элементы.ДекорацияЗадачаРКД.Видимость = Ложь;	
...
&НаКлиенте
Процедура ИзменитСтатус(Команда)
	СтруктураПараметр = Новый Структура("Комплект", Объект.Ссылка);
	ОповещениеОЗакрытии = Новый ОписаниеОповещения("ПослеЗакрытияОкнаСменыЭтапа", ЭтаФорма);
	ОткрытьФорму("ОбщаяФорма.ФормаПереносНаЭтап", СтруктураПараметр, ЭтаФорма, , , , ОповещениеОЗакрытии,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры 

&НаКлиенте
Процедура ПослеЗакрытияОкнаСменыЭтапа(Значение, ДополнительныеПараметры) Экспорт

	

КонецПроцедуры // ПослеЗакрытияОкнаСменыЭтапа()

&НаКлиенте
Процедура СоздатьЗадачу(Команда)
	ОписаниеПослеЗакрытия = Новый ОписаниеОповещения("ПослеЗакрытияНовойЗадачи", ЭтаФорма);
	ОткрытьФорму("Задача.ЗадачиОбщие.ФормаОбъекта", Новый Структура("ОбъектАдресации", Объект.Ссылка), , , , , ОписаниеПослеЗакрытия);
КонецПроцедуры  

&НаКлиенте
Процедура ПослеЗакрытияНовойЗадачи(Данные, ДополнительныеПараметры) Экспорт
	ЗаполнитьТаблицуЗадач();
КонецПроцедуры // ПослеЗакрытияНовойЗадачи()

&НаКлиенте
Процедура ТаблицаОбщихЗадачПередУдалением(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаОбщихЗадачПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаОбщихЗадачПередНачаломИзменения(Элемент, Отказ)
	Отказ = Истина;
...
&НаКлиенте
Процедура ИсторияТЗПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИсторияТЗПередНачаломИзменения(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ИсторияТЗПередУдалением(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры
...
Процедура ДанныеИзPDM(Команда)
	ОткрытьФорму("ОбщаяФорма.ФормаДанныхОбъектаPDM", Новый Структура("Комплект", Объект.Ссылка), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура КонтрольСтруктурыПапок(Команда)
	Сообщить(омPDMНаСервере.ПроверитьСтруктуруКаталогИзделияКомплект(Объект.Ссылка));
КонецПроцедуры

&НаКлиенте
Процедура СтруктураКаталога(Команда)
	ОткрытьФорму("ОбщаяФорма.ФормаСтруктураКаталоговPDM", Новый Структура("Комплект", Объект.Ссылка), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры

&НаКлиенте
Процедура ЗаблокированныеФайлы(Команда)
	ОткрытьФорму("ОбщаяФорма.ФормаРазрегестрированыеФайлы", Новый Структура("Комплект, НеЗакрывать", Объект.Ссылка, Истина), ЭтаФорма, , , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
