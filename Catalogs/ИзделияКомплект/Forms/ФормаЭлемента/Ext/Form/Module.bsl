...
&НаКлиенте
Перем НормаВремениПроизводстваИзменено, НормаВремениМонтажаИзменено, ПутьКФайлуПДМ;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если НЕ НачальныйКонструктор = Конструктор ИЛИ НачальнаяСтадия = ТекущаяСтадия Тогда
	...
	Если НормаВремениПКДРКД Тогда
		...
		стДанные = Новый Структура("НормаВремени_ПКД, НормаВремени_РКД, ДнейПКД, ДнейРКД, ДнейТЗ, НачалоТЗ", );
...
Процедура ЗаписатьКонструктора(Комплект, Конструктор, ТекущаяСтадия, КонструкторНеИзменен)
	...
	Если НЕ КонструкторНеИзменен Тогда
		...
		Запрос.УстановитьПараметр("ОбъектТЗ", Комплект);
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.Задача) Тогда
				...
				Если НЕ мТипыЗадачКонструктора.Найти(ОбъектЗадача.ТипЗадачи) = Неопределено Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ЗадачаВРаботе) Тогда
		...
		Элементы.ФормаИзменитьКонструктораВPDM.Доступность 	= Ложь;	
