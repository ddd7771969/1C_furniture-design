...
Процедура ЗаполнитьКомплекты()
	...
	Для каждого х Из стКомплекты.мИзделияКомплект Цикл
		СписокКомплектов.Добавить(х.ИзделияКомплект, х.Представление);
	КонецЦикла; 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		СписокКомплектов.Добавить(Договор, "Договор " + Представление);
		ЗаполнитьКомплекты();
		ПолучитьданныеИзделия();	
	Иначе
		...
		Если Параметры.Свойство("ЗначенияЗаполнения") Тогда
			Если Параметры.ЗначенияЗаполнения.Свойство("Родитель") Тогда
				Если ЗначениеЗаполнено(Параметры.ЗначенияЗаполнения.Родитель) Тогда
					Объект.Проект = Параметры.ЗначенияЗаполнения.Родитель.Проект;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Объект.ЕдиницаИзмерения = омЕдиницыИзмерения.ПолучитьЕдиницуПоУмолчанию(ОбъектНастройки.ПолучитьНаименованиеРеквизитаЕдиницы(Перечисления.ТипыНоменклатуры.Штучная));
		
	КонецЕсли;
	
	ВидимостьЭлементов(); 
	
	ЗаполнитьВыполненыеОбразцы();
	
КонецПроцедуры 

Процедура ЗаполнитьВыполненыеОбразцы()

    ТипОбразец = Тип("СправочникСсылка.Образец");
	
	МассивПредварительныхОбразцов = Новый Массив;
	
	Для каждого х Из Объект.Образцы Цикл
	
		Если ТипЗнч(х.Образец) = ТипОбразец Тогда
			х.Выполнен = Истина;	
		Иначе
			х.Выполнен = Ложь;	
		    МассивПредварительныхОбразцов.Добавить(х.Образец);
		КонецЕсли;	
	
	КонецЦикла;
	
	Если МассивПредварительныхОбразцов.Количество() > 0 Тогда
		
		Отбор = Новый Структура("Образец", );
	
		Для каждого х Из омОбразцы.ПолучитьДанныеОбразцовНаОснованииПредварительных(МассивПредварительныхОбразцов) Цикл
			
			Отбор.Образец = х.Образец;
			
			НайденыеСтрока = Объект.Образцы.НайтиСтроки(Отбор);
			
			Для каждого НайденаяСтрока Из НайденыеСтрока Цикл
			
				НайденаяСтрока.Выполнен = Истина;	
