&НаКлиенте
Перем НормаВремениПКДРКД, ИзмененКонструктор, ПутьКФайлуПДМ;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		Если Параметры.Свойство("ЗначенияЗаполнения", ЗначенияЗаполнения) Тогда
			...
			Если ЗначенияЗаполнения.Свойство("Родитель", Родитель) Тогда
				
				Запрос = Новый Запрос;
				Запрос.Текст = 
					"ВЫБРАТЬ
					|	Композиции.Проект КАК Проект
					|ИЗ
					|	Справочник.Композиции КАК Композиции
					|ГДЕ
					|	Композиции.Ссылка = &Родитель";
