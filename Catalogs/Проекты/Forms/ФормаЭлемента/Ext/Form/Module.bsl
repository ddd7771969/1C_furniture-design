...
Процедура ЗаполнитьИзДоговораНаСервере()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПриложенияКДоговорам.СрокОкончанияМонтажа КАК СрокОкончанияМонтажа
		|ПОМЕСТИТЬ втМонтаж
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПриложенияКДоговорам.СрокОкончанияПроизводства КАК СрокОкончанияПроизводства
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПриложенияКДоговорам.СрокОкончанияПКД КАК СрокОкончанияПКД
		|ПОМЕСТИТЬ втПКД
		|ИЗ
		|	Справочник.ПриложенияКДоговорам КАК ПриложенияКДоговорам
		|ГДЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(МИНИМУМ(втПКД.СрокОкончанияПКД), ДАТАВРЕМЯ(1, 1, 1)) КАК КрайПКД,
		|	ЕСТЬNULL(МИНИМУМ(втМонтаж.СрокОкончанияМонтажа), ДАТАВРЕМЯ(1, 1, 1)) КАК КрайМонтажа,
		|	ЕСТЬNULL(МИНИМУМ(втПроизводство.СрокОкончанияПроизводства), ДАТАВРЕМЯ(1, 1, 1)) КАК КрайПроизводства
		|ИЗ
		|	втМонтаж КАК втМонтаж,
