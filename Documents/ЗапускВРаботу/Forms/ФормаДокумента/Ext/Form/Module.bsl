...
Функция ПолучитьДанныеКомплекта(Комплект)
	...
	Если тзДанныеКомплека.Количество() = 0 Тогда
		стрВозврата = Новый Структура("Комплект, ДнейПКД, ДнейРКД, НормаВремени_ПКД, НормаДоработки_ПКД, НормаВремени_РКД, НормаДоработки_РКД, ГабаритныйЧертеж, Композиция, Изменен, НачалоПроизводства"
		,Комплект,
		,0
		,0
		,0
...
Процедура ЗаписатьВРегистрНаСервере(мИзменений, Проект, ДатаНачала)
	...
	Для каждого х Из мИзменений Цикл
		...
		Если ЗначениеЗаполнено(х.Композиция) Тогда
			...
			МенеджерЗаписейКомпозиция = РегистрыСведений.ДатыКомплекта.СоздатьМенеджерЗаписи();
			МенеджерЗаписейКомпозиция.Комплект = х.Композиция;
			МенеджерЗаписейКомпозиция.Прочитать();
			
			Если МенеджерЗаписейКомпозиция.НачалоТЗ = ПустаяДата Тогда
				
				КрайТЗ = омДаты.ДобавитьДниСУчетомВыходных(ДатаНачала, ДнейТЗ);
				
				МенеджерЗаписейКомпозиция.Комплект 	= х.Композиция;
				МенеджерЗаписейКомпозиция.Проект 	= Проект;
				МенеджерЗаписейКомпозиция.ДнейТЗ 	= ДнейТЗ;	
				МенеджерЗаписейКомпозиция.НачалоТЗ 	= ДатаНачала;
				МенеджерЗаписейКомпозиция.КрайТЗ	= КрайТЗ;
				
				МенеджерЗаписейКомпозиция.Записать();
				
				ТребуетсяРасчетПКД_РКД = Истина;
				
			КонецЕсли;
			
		ИначеЕсли МенеджерЗаписей.НачалоТЗ = ПустаяДата Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Для каждого х Из Объект.Комплекты Цикл
		...
		Если х.Изменен Тогда
			...
			стСтрока = Новый Структура("Комплект, ДнейПКД, ДнейРКД, НормаВремени_ПКД, НормаВремени_РКД, БезЗамера, БезГабаритногоЧертежа, НормаДоработки_ПКД, НормаДоработки_РКД, Композиция", );
