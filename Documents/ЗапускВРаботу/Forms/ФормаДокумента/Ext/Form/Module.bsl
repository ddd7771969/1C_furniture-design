&НаКлиенте
Перем мИзменений, мИзмененийМатериалы, СтруктураСдвигНормаДни, МассивДанныехДляВыгрузки, ИндексВыгрузки, ВсеОбъектыВыгружены;
...
Функция ЗаполнитьНачальноеЗначениеТЧ(мКомплекты = Неопределено)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ДатыКомплекта.НормаВремени_ПКД, 2) КАК НормаВремени_ПКД,
		|	ЕСТЬNULL(ДатыКомплекта.НормаВремени_РКД, 0.5) КАК НормаВремени_РКД,
		|	ЛОЖЬ КАК Изменен,
		|	ДатыКомплекта.БезЗамера КАК БезЗамера,
		|	втКомплекты.Комплект.ГабаритныйЧертеж КАК ГабаритныйЧертеж,
		|	Композиции.Ссылка КАК Композиция,
		|	ДатыКомплекта.БезГабаритногоЧертежа КАК БезГабаритногоЧертежа,
		|	ДатыКомплекта.Код_в_PDM КАК КодPDM,
...
Функция ПолучитьДанныеКомплекта(Комплект)
	...
	Если тзДанныеКомплека.Количество() = 0 Тогда
		стрВозврата = Новый Структура("Комплект, ДнейПКД, ДнейРКД, НормаВремени_ПКД,  НормаВремени_РКД, ГабаритныйЧертеж, Композиция, Изменен, НачалоПроизводства, ГОСТ, КатегорияКонструктора, ТипКомплекта"
		,Комплект,
		,0
		,0
		,0
...
Функция КонтрольВыгрузкиВPMНаСервере(Ссылка)
	...
	Запрос.Текст = 
		...
		|	Документ.ЗапускВРаботу.Комплекты КАК ЗапускВРаботуКомплекты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыКомплекта КАК ДатыКомплекта
		|		ПО ЗапускВРаботуКомплекты.Комплект.Родитель.Композиция = ДатыКомплекта.Комплект
		|ГДЕ
		|	ЗапускВРаботуКомплекты.Ссылка = &Ссылка
		|	И ЕСТЬNULL(ДатыКомплекта.Код_в_PDM, 0) = 0
		|	И НЕ ЗапускВРаботуКомплекты.Комплект.Родитель.Композиция = ЗНАЧЕНИЕ(Справочник.Композиции.ПустаяСсылка)
		|	И НЕ ЗапускВРаботуКомплекты.Комплект.Родитель = ЗНАЧЕНИЕ(Справочник.ИзделияКомплект.ПустаяСсылка)";
	...
	Запрос.Текст = 
		...
		|	Документ.ЗапускВРаботу.Комплекты КАК ЗапускВРаботуКомплекты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДатыКомплекта КАК ДатыКомплекта
		|		ПО ЗапускВРаботуКомплекты.Комплект = ДатыКомплекта.Комплект
		|ГДЕ
		|	ЗапускВРаботуКомплекты.Ссылка = &Ссылка
		|	И ЕСТЬNULL(ДатыКомплекта.Код_в_PDM, 0) = 0";
...
Процедура ЗаписатьВРегистрНаСервере(мИзменений, Проект, ДатаНачала)
	...
	Для каждого х Из мИзменений Цикл
		...
		Если НЕ ЗначениеЗаполнено(х.Комплект) Тогда
			Возврат;
		КонецЕсли;
...
Процедура ПроверитьУдалениеКомплекта(мКомплекты, Проект)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если мКомплекты.Найти(Выборка.Комплект) = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(Выборка.Комплект) Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Для каждого х Из Объект.Комплекты Цикл
		...
		Если х.Изменен Тогда
			...
			стСтрока = Новый Структура("Комплект, ДнейПКД, ДнейРКД, НормаВремени_ПКД, НормаВремени_РКД, БезЗамера, БезГабаритногоЧертежа, Композиция, КатегорияКонструктора, ТипКомплекта, ГОСТ", );
		...
		Для каждого ИмяКолонкиМатериал Из мИменаКолонкиМатериала Цикл
			...
			Если х.Свойство(ИмяКолонкиМатериал) Тогда
...
Процедура СоздатьГЧ(ТекущийКомплект, мПомещения, Наименование, РодительГЧ)
	...
	Если ТипЗнч(ТекущийКомплект) = Тип("СправочникСсылка.Композиции") Тогда
		...
		|	И ИзделияКомплект.ГабаритныйЧертеж = ЗНАЧЕНИЕ(Справочник.ГабаритныеЧертежи.ПустаяСсылка)";
		
		Запрос.УстановитьПараметр("Композиция", ТекущийКомплект);
		...
		Пока Выборка.Следующий() Цикл
			...
			об.ГабаритныйЧертеж = НайденыйГЧ;
			об.Записать();
