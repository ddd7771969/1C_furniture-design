...
Функция ЗаполнитьНачальноеЗначениеТЧ(мКомплекты = Неопределено)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА втКомплекты.Комплект.ГабаритныйЧертеж = ЗНАЧЕНИЕ(Справочник.ГабаритныеЧертежи.ПустаяСсылка)
		|			ТОГДА втКомплекты.Комплект.Родитель.Композиция.ГабаритныйЧертеж
		|		ИНАЧЕ втКомплекты.Комплект.ГабаритныйЧертеж
