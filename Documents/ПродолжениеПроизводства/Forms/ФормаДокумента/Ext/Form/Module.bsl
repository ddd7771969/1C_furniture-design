...
Процедура ИзделияКомплектПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если ЗначениеЗаполнено(Объект.Проект) Тогда
		...
		стПараметры.Вставить("НеПоказыватьВВыборе", ЗаполнитьнеПоказыват(НеПоказыватьИзделиеКомплект, Объект.Проект));
...
&НаСервереБезКонтекста
Функция ЗаполнитьнеПоказыват(НеПоказыватьИзделиеКомплект, Проект)
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГлабольныеСостояниеКомплекта.СрезПоследних КАК ГлабольныеСостояниеКомплектаСрезПоследних
		|		ПО (ГлабольныеСостояниеКомплектаСрезПоследних.Комплект = ИзделияКомплект.Ссылка)
		...
		|	И ГлабольныеСостояниеКомплектаСрезПоследних.Комплект ЕСТЬ NULL
		|	И НЕ ИзделияКомплект.Ссылка В (&НеПоказыватьИзделиеКомплект)";
	
	Запрос.УстановитьПараметр("Проект", Проект);
	Запрос.УстановитьПараметр("НеПоказыватьИзделиеКомплект", НеПоказыватьИзделиеКомплект);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		НеПоказыватьИзделиеКомплект.Добавить(Выборка.Ссылка);
	КонецЦикла;
	
	Возврат НеПоказыватьИзделиеКомплект;	

КонецФункции // ЗаполнитьнеПоказыват()
