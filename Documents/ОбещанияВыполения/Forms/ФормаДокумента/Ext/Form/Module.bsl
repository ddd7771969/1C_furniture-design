...
Процедура СделкаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	Иначе
		...
		ПоказатьВопрос(ОписаниеОповещения,"При смене Проекты будет очищена табличная часть. Продолжить?", РежимДиалогаВопрос.ДаНет);
...
Функция ПолучитьМассивСделокОжидания()
...
КонецФункции // ПолучитьМассивСделокОжидания()
...
Процедура ОткрытьФормуВыбораСделки()
	...
	ПараметрыФормы.Вставить("МасиивСделкиПоказывать",	ПолучитьМассивСделокОжидания());
...
&НаКлиенте
Процедура ПослеВыборкаСделки(Значение, ДополнительныйПараметр) Экспорт
