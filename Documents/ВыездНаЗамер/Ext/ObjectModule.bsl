...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Запрос.Текст = 
		...
		|			И ВыездНаЗамерГабаритныеЧертежи.ГабаритныйЧертеж = ПроверитьГабаритныеЧертежи.ОбъектТЗ
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		Задачи.ПроверитьГабаритныеЧертежи.СоздатьЗадачуПроверитьГабаритныеЧертежи(Выборка.ГабаритныйЧертеж, ИсполнительЗадачи, Выборка.Проект, Ссылка);
	КонецЦикла; 
