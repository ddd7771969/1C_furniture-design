Процедура ПолучитьДанныеЛегенды()
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Приоритеты.СтатусПриоритета.ВсегоЗадач КАК ВсегоЗадач,
		|	Приоритеты.СтатусПриоритета.КоличествоЗадачУИсполнителя КАК КоличествоЗадачУИсполнителя,
	...
	Пока Выборка.Следующий() Цикл
		...
		ВебЗначениеЦветаФона = омЦветНаСервереПовтор.ПолучитьВебПредставленияЦвета(Выборка.ЦветФона, Ложь);
		ВебЗначениеЦветТекст = омЦветНаСервереПовтор.ПолучитьВебПредставленияЦвета(Выборка.ЦветТекста);
		...
		НС.ЦветФона 	= омЦветНаСервереПовтор.ПолучитьЦветИзСтрокиСПрефиксом(Выборка.ЦветФона);
		НС.ЦветТекста 	= омЦветНаСервереПовтор.ПолучитьЦветИзСтрокиСПрефиксом(Выборка.ЦветТекста);
...
Процедура ВывестиТаблицу()
	...
	Приоритеты.Сортировать("НомерПоПорядку, ПриоритетНаименование, КомплектНаименование");
	ВывестиПоКомплектам();
