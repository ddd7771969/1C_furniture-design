&НаКлиенте
Перем мИзменения, мСписокВыбораЦвета, ТаблицаВыгрузкиИзменена, ИзменененыРасширенияФаловКартинок, ИзменененыСтруктураПапокИзделия;

Процедура СохранитьНаСервере(мИзменения, ТаблицаВыгрузкиИзменена, ИзменененыРасширенияФаловКартинок, ИзменененыСтруктураПапокИзделия)
	...
	Если ИзменененыРасширенияФаловКартинок Тогда
	
		ИмяНастройки = "РасширенияФаловКартинок";
		СохранениеСписка(ИмяНастройки, РасширениеФайловКартинк ,ПустойПользователь);
		
	КонецЕсли;
	
	Если ИзменененыСтруктураПапокИзделия Тогда
		
		ИмяНастройки = "СтруктураПапокИзделия";
		СохранениеСписка(ИмяНастройки, СтруктураПапокИзделия, ПустойПользователь);
...
Процедура СохранениеСписка(ИмяНастройки, СписокЗначений, ПустойПользователь)
	...
	Для х = 0 По КоличествоЗначений - 1 Цикл
		
		МенеджерЗаписи = РегистрыСведений.НастройкиПроекта.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.ИмяНастройки 	= ИмяНастройки;
		МенеджерЗаписи.НомерНастройки 	= х;
		МенеджерЗаписи.Пользователь 	= ПустойПользователь;
		МенеджерЗаписи.Значение 		= СписокЗначений.Получить(х);
		МенеджерЗаписи.Записать();
...
&НаКлиенте
Процедура СтруктураПапокИзделияПриИзменении(Элемент)
	ИзменененыСтруктураПапокИзделия = Истина;
КонецПроцедуры

#Область РасширениеФаловКартинки

&НаКлиенте
 Процедура РасширениеФайловКартинкПриИзменении(Элемент)
	 ИзменененыРасширенияФаловКартинок = Истина;
 КонецПроцедуры
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    ЗаполнитьВыгрузку();
    ЗаполнитьСписокЗначений("РасширенияФаловКартинок", 	РасширениеФайловКартинк);
    ЗаполнитьСписокЗначений("СтруктураПапокИзделия",	СтруктураПапокИзделия);
...
Процедура ПриОткрытии(Отказ)
	...
	ИзменененыРасширенияФаловКартинок 	= Ложь;
    ИзменененыСтруктураПапокИзделия		= Ложь;
КонецПроцедуры

Процедура ЗаполнитьСписокЗначений(ИмяНастройки, СписокЗначений)
	...
	Пока Выборка.Следующий() Цикл
		СписокЗначений.Добавить(Выборка.Значение);
	КонецЦикла;
...
Процедура ТаблицаЦветовПередУдалением(Элемент, Отказ)
	...
	//СписокУдаленныхЦветов.Добавить(ТД.НаименованиеЭтапа);
КонецПроцедуры 
...
Процедура ПослеЗакрытияОкнаРедактированияВыгрузка(ПараметрыДанных, ДопПараметры) Экспорт
    ...
    стПраметры.Вставить("ПутьКФайлуEXEРаздатьПраваКомпозиции", Ложь);
    стПраметры.Вставить("ПутьКФайлуEXEЗабратьПраваКомпозиции", Ложь);
	...
	Для каждого х Из тзВыгрузка_в_PDM Цикл 
		...
		ИначеЕсли х.ИмяНастройки = "ПутьКФайлуEXEРаздатьПраваКомпозиции" Тогда
			х.В_PDM = ПараметрыДанных.ПутьКФайлуEXEРаздатьПраваКомпозиции; 
			стПраметры.ПутьКФайлуEXEРаздатьПраваКомпозиции = Истина;
		ИначеЕсли х.ИмяНастройки = "ПутьКФайлуEXEЗабратьПраваКомпозиции" Тогда
			х.В_PDM = ПараметрыДанных.ПутьКФайлуEXEЗабратьПраваКомпозиции; 
			стПраметры.ПутьКФайлуEXEЗабратьПраваКомпозиции = Истина;
		КонецЕсли;
	...
	Если НЕ стПраметры.ПутьКФайлуEXEРаздатьПраваКомпозиции Тогда
		НС = тзВыгрузка_в_PDM.Добавить();
		НС.Пользователь = ПараметрыДанных.Пользователь;
		НС.ИмяНастройки = "ПутьКФайлуEXEРаздатьПраваКомпозиции";
		НС.В_PDM 		= ПараметрыДанных.ПутьКФайлуEXEРаздатьПраваКомпозиции;                
	КонецЕсли;

	Если НЕ стПраметры.ПутьКФайлуEXEЗабратьПраваКомпозиции Тогда
		НС = тзВыгрузка_в_PDM.Добавить();
		НС.Пользователь = ПараметрыДанных.Пользователь;
		НС.ИмяНастройки = "ПутьКФайлуEXEЗабратьПраваКомпозиции";
		НС.В_PDM 		= ПараметрыДанных.ПутьКФайлуEXEЗабратьПраваКомпозиции;                
	КонецЕсли;
...
Функция ПолучитьСтруктуруДанныхПользователя(Изменить)
	...
	Если Изменить Тогда
		стДанные = Новый Структура("Пользователь, ПутьКФайлуEXEЕстьРазрегестрированные, ПутьКФайлуEXE, ПутьКФайлуЗагрузки, ПутьКФайлуEXEПроекта, ПутьКФайлуEXEКонструктор, ПутьКФайлуEXEКомпозиция, ПутьКФайлуEXEГабаритныйЧерчеж, ПутьКФайлуEXEИзменениеСтатусаГЧ, ПутьКФайлуEXEЗагрузкаТЗ, ПутьКФайлуEXEРаздатьПрава, ПутьКФайлуEXEЗабратьПрава, ПутьКФайлуEXEРаздатьПраваКомпозиции, ПутьКФайлуEXEЗабратьПраваКомпозиции"
		, текПользователь);
	Иначе	
		стДанные = Новый Структура("ПутьКФайлуEXE, ПутьКФайлуEXEЕстьРазрегестрированные, ПутьКФайлуЗагрузки, ПутьКФайлуEXEПроекта, ПутьКФайлуEXEКонструктор, ПутьКФайлуEXEКомпозиция, ПутьКФайлуEXEГабаритныйЧерчеж, ПутьКФайлуEXEИзменениеСтатусаГЧ, ПутьКФайлуEXEЗагрузкаТЗ, ПутьКФайлуEXEРаздатьПрава, ПутьКФайлуEXEЗабратьПрава, ПутьКФайлуEXEРаздатьПраваКомпозиции, ПутьКФайлуEXEЗабратьПраваКомпозиции");
	КонецЕсли;
...
Процедура Сохранить(Команда)
	СохранитьНаСервере(мИзменения, ТаблицаВыгрузкиИзменена, ИзменененыРасширенияФаловКартинок, ИзменененыСтруктураПапокИзделия);
	ЭтаФорма.Закрыть();
КонецПроцедуры
...
&НаСервереБезКонтекста
 Процедура ПересчитатьДатыОкрнчанияНаСервере()
	 РегистрыСведений.ПлановоеЗначениеПроектирования.ПересчитатьДатуОкончания();
 КонецПроцедуры

&НаКлиенте
 Процедура ПересчитатьДатыОкрнчания(Команда)
	 ПересчитатьДатыОкрнчанияНаСервере();
 КонецПроцедуры
