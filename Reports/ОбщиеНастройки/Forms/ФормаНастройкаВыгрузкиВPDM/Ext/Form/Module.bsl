...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	 ...
	 Параметры.Свойство("ПутьКФайлуEXEРаздатьПрава", 			ПутьКФайлуEXEРаздатьПрава);
	 Параметры.Свойство("ПутьКФайлуEXEЗабратьПрава", 			ПутьКФайлуEXEЗабратьПрава);
	 Параметры.Свойство("ПутьКФайлуEXEРаздатьПраваКомпозиции",	ПутьКФайлуEXEРаздатьПраваКомпозиции);
	 Параметры.Свойство("ПутьКФайлуEXEЗабратьПраваКомпозиции",	ПутьКФайлуEXEЗабратьПраваКомпозиции);
	 Параметры.Свойство("ПутьКФайлуEXEЕстьРазрегестрированные", ПутьКФайлуEXEЕстьРазрегестрированные);
КонецПроцедуры  
...
&НаКлиенте
Процедура ПутьКФайлуEXEРаздатьПраваНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Режим = РежимДиалогаВыбораФайла.ВыборКаталога; 
	Заголовок  	= "Выберите каталог";
	Оповещение = Новый ОписаниеОповещения("ВыборКаталога_Завершение", ЭтаФорма,Новый Структура("ЭлементВыбора","ПутьКФайлуEXEРаздатьПрава"));
	ПолучитьДиалогВыбораФайла(ЭтаФорма.ПутьКФайлуЗагрузки, Оповещение, Режим, Заголовок);
КонецПроцедуры


&НаКлиенте
Процедура ПутьКФайлуEXEЗабратьПраваНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Режим = РежимДиалогаВыбораФайла.ВыборКаталога; 
	Заголовок  	= "Выберите каталог";
	Оповещение = Новый ОписаниеОповещения("ВыборКаталога_Завершение", ЭтаФорма,Новый Структура("ЭлементВыбора","ПутьКФайлуEXEЗабратьПрава"));
	ПолучитьДиалогВыбораФайла(ЭтаФорма.ПутьКФайлуЗагрузки, Оповещение, Режим, Заголовок);
КонецПроцедуры
...
Процедура Перенести(Команда)
    ...
    стДанные.Вставить("ПутьКФайлуEXEРаздатьПрава", );
    стДанные.Вставить("ПутьКФайлуEXEЗабратьПрава", );
    стДанные.Вставить("ПутьКФайлуEXEРаздатьПраваКомпозиции", );
    стДанные.Вставить("ПутьКФайлуEXEЗабратьПраваКомпозиции", );
    стДанные.Вставить("ПутьКФайлуEXEЕстьРазрегестрированные", );
...
&НаКлиенте
Процедура ПутьКФайлуEXEЕстьРазрегестрированныеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Режим 		= РежимДиалогаВыбораФайла.Открытие;
	Заголовок  	= "Выберите файл для проверки разрегестрированных";
	Оповещение 	= Новый ОписаниеОповещения("ВыборКаталога_Завершение", ЭтаФорма,Новый Структура("ЭлементВыбора",Элемент.Имя));
	ПолучитьДиалогВыбораФайла(ЭтаФорма.ПутьКФайлуEXEИзменениеСтатусаГЧ, Оповещение, Режим, Заголовок);
КонецПроцедуры

&НаКлиенте
Процедура ПутьКФайлуEXEРаздатьПраваКомпозицииНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Режим 		= РежимДиалогаВыбораФайла.Открытие;
	Заголовок  	= "Выберите файл для назначить права композиции";
	Оповещение 	= Новый ОписаниеОповещения("ВыборКаталога_Завершение", ЭтаФорма,Новый Структура("ЭлементВыбора",Элемент.Имя));
	ПолучитьДиалогВыбораФайла(ЭтаФорма.ПутьКФайлуEXEРаздатьПраваКомпозиции, Оповещение, Режим, Заголовок);
КонецПроцедуры

&НаКлиенте
Процедура ПутьКФайлуEXEЗабратьПраваКомпозицииНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	Режим 		= РежимДиалогаВыбораФайла.Открытие;
	Заголовок  	= "Выберите файл для забрать права композиции";
	Оповещение 	= Новый ОписаниеОповещения("ВыборКаталога_Завершение", ЭтаФорма,Новый Структура("ЭлементВыбора",Элемент.Имя));
	ПолучитьДиалогВыбораФайла(ЭтаФорма.ПутьКФайлуEXEЗабратьПраваКомпозиции, Оповещение, Режим, Заголовок);
КонецПроцедуры
