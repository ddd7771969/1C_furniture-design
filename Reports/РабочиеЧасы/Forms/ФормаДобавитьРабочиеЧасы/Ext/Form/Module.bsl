&НаКлиенте
Перем КоличествоДнейВМесяце, НомерМесяца, МожноРедактировать;
...
Процедура ПриОткрытии(Отказ)
	...
	НомерМесяца 			= 0;
	МожноРедактировать		= омРолиНаСервере.ДоступностьКадровогоУчета(); 
...
Процедура тдКалендарьОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка, ДополнительныеПараметры)
	...
	Если Тип("Структура") = ТипЗнч(Расшифровка) И МожноРедактировать Тогда
...
Процедура РасчитатьКоличествоДнейЧасов()
	...
	Для текДень = 1 По КоличествоДнейВМесяце  Цикл
		текДата = Дата(Год, НомерМесяца, текДень);
		
		КоличествоРабочихЧасов = 8;
		
		Для каждого текВыходной Из ВыходныеДни Цикл
...
Процедура ПолучитьКоличествоДнейВМесяце()
	...
	Если ЗначениеЗаполнено(ЭтаФорма.Месяц) И ЭтаФорма.Год > 0 Тогда
		КоличествоДнейВМесяце = омМесяц.КоличествоДнейВМесяце(Месяц, Год, НомерМесяца);
	Иначе
		КоличествоДнейВМесяце = 0;
	КонецЕсли;
	ПолучитьКоличествоЧасов(); 
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("НачалоМесяца") Тогда
		
		ТекущийМесяц = НачалоМесяца(Параметры.НачалоМесяца);
		ЗаполнитьНаСервере();
