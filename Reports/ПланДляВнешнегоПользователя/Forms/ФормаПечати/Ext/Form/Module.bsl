...
Процедура ЗаполнитьТЗЗадачи(МассивЗадачи, ПустаяДата)
	...
	ТЗЗадачи.Очистить();
	тзПомещения.Очистить();
	тзКомплекты.Очистить();
	
	Для каждого х Из МассивЗадачи Цикл
		
		Если ПолнаяПечать Тогда
			Если НЕ ЗначениеЗаполнено(х.Композиция) Тогда
				Если НЕ х.НачалоТЗ = ПустаяДата И НЕ х.ОкончаниеТЗ = ПустаяДата  Тогда
					
					НС 							= ТЗЗадачи.Добавить();
					НС.Комплект 				= х.Комплект;
					НС.НаименованиеКомплект 	= х.НаименованиеКомплект;
					НС.Этап 					= "ТЗ";
					НС.ДатаНачала 				= х.НачалоТЗ;
					НС.ДатаОкончания 			= х.ОкончаниеТЗ; 
			...
			Если НЕ ТипКомпозиция = ТипЗнч(х.Комплект) Тогда
				
				Если НЕ х.НачалоРКД = ПустаяДата И НЕ х.ОкончаниеРКД = ПустаяДата  Тогда
					
					НС 							= ТЗЗадачи.Добавить();
					НС.Комплект 				= х.Комплект;
					НС.НаименованиеКомплект 	= х.НаименованиеКомплект;
					НС.Этап 					= "РКД";
					НС.ДатаНачала 				= х.НачалоРКД;
					НС.ДатаОкончания 			= х.ОкончаниеРКД; 
					НС.ПродолжительностьВДнях   = Окр((НС.ДатаОкончания - НС.ДатаНачала) / СекундВДне, 0);
					НС.Помещение 				= х.Помещение; 
					НС.ПомещениеНаименование	= х.ПомещениеНаименование; 
					НС.Подряд					= х.ПодрядРКД; 
				...
				Если НЕ х.НачалоПроизводства = ПустаяДата И НЕ х.ОкончаниеПроизводства = ПустаяДата  Тогда
					...
					НС.Помещение 				= х.Помещение; 
					НС.ПомещениеНаименование	= х.ПомещениеНаименование; 
					НС.Подряд					= х.ПодрядПроизводство; 
		...
		Иначе
			...
			Если НЕ ЗначениеЗаполнено(х.Композиция) Тогда
				Если НЕ х.НачалоТЗ = ПустаяДата И НЕ х.ОкончаниеПроектирования = ПустаяДата  Тогда
	...
	тзКомплекты.Сортировать("Помещение, Композиция, НаименованиеКомплект");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПолнаяПечать 	= Параметры.ПолнаяПечать;
...
Процедура СоздатьТаблицуПечати()
	...
	ЗаполнитьТЗЗадачи(омГантНаСервере.ПолучитьДатыПроекта(Проект), ПустаяДата);
...
Процедура ВывестиДанные() 
	...
	СтрктураОтборДатаГотовности = Новый Структура("Помещение", );
	СтрктураОтборДатаГотовности.Вставить("Этап", ?(ПолнаяПечать, "РКД", "Производство"));
	ПустаяДата = Дата(1, 1, 1);
	
	Для каждого ТекущиеПомещение Из тзПомещения Цикл
	
		СтрктураОтборДатаГотовности.Помещение = ТекущиеПомещение.Помещение;
		
		ДатаГотовности = ПустаяДата;
		
		Для каждого х Из ТЗЗадачи.НайтиСтроки(СтрктураОтборДатаГотовности) Цикл
		
			Если ДатаГотовности = ПустаяДата Тогда
				ДатаГотовности = х.ДатаНачала;
			Иначе	
				ДатаГотовности = Мин(х.ДатаНачала, ДатаГотовности);
			КонецЕсли;	
		
		КонецЦикла;
		
		ТекущиеПомещение.ДатаГотовности = ДатаГотовности;	
	...
	стЦвета = Новый Структура("Производство, Монтаж, Пересечение, Согласования, Конструирование, Помещение, Композиция",
			ЦветБелый, ЦветБелый, ЦветБелый, ЦветБелый, ЦветБелый, ЦветБелый, ЦветБелый);
	
	
	Если ПолнаяПечать Тогда
		
		стЦвета.Вставить("ТЗ", 						ЦветБелый);
		стЦвета.Вставить("РКД", 					ЦветБелый);
		стЦвета.Вставить("ПодрядКонструирование",	ЦветБелый);
		стЦвета.Вставить("ПодрядРКД", 				ЦветБелый);
		стЦвета.Вставить("ПодрядПроизводство", 		ЦветБелый);
	...
	Для каждого х Из тзЦвета Цикл
		...
		ИначеЕсли х.НаименованиеЭтапа = "Композиция" Тогда
			стЦвета.Композиция 			= х.Цвет;
		КонецЕсли; 
		
		Если ПолнаяПечать Тогда
			
			Если х.НаименованиеЭтапа = "ПодрядКонструирование" Тогда
				стЦвета.ПодрядКонструирование 	= х.Цвет;
			ИначеЕсли х.НаименованиеЭтапа = "ПодрядРКД" Тогда
				стЦвета.ПодрядРКД 			= х.Цвет;
			ИначеЕсли х.НаименованиеЭтапа = "ПодрядПроизводство" Тогда
				стЦвета.ПодрядПроизводство 	= х.Цвет;
	...
	стОтборПомещение	= Новый Структура("Композиция, Помещение", ПредопределенноеЗначение("Справочник.Композиции.ПустаяСсылка"));
	...
	Для каждого текПомещение Из тзПомещения Цикл 
		...
		стПараметровОбласти.Текст 					= Формат(текПомещение.ДатаГотовности, "ДФ=dd.MM.yyyy");
		стПараметровОбласти.ЦветФона 				= стЦвета.Помещение;
		...
		стПараметровОбласти.Текст = "";
 		ВыводОбъекта(НайденыеСтроки, текПомещение, КешЦвета, стПараметровОбласти, смещениеКолонок, стЦвета,,,ВыведенныеНаименования);
		...
		Для каждого текКомплект Из НайденыеСтроки Цикл
			...
			ЭтоКомпозиция = ТипКомпозиция = ТипЗнч(текКомплект.Комплект);
			
			Если ЭтоКомпозиция Тогда
				стПараметровОбластиКомплект.ЦветФона = ПолучитьЦветЭтапа("Композиция", стЦвета);
			Иначе
				стПараметровОбластиКомплект.ЦветФона = ЦветБелый;
			КонецЕсли;
			...
			НайденыеСтрокиКомпозиции = тзВывод.НайтиСтроки(стОтбор);
			
			ВыводОбъекта(НайденыеСтрокиКомпозиции, текКомплект, КешЦвета, стПараметровОбласти, смещениеКолонок, стЦвета, ЭтоКомпозиция,, ВыведенныеНаименования);
			
			Если ЭтоКомпозиция Тогда
				
				стОтборКомпозиция.Композиция = текКомплект.Комплект;
				КомплектыКомпозиции = тзКомплекты.НайтиСтроки(стОтборКомпозиция);
				Для каждого текКомплектКомпозиции Из КомплектыКомпозиции Цикл
					
					стПараметровОбластиКомплект.ЦветФона = ЦветБелый;
					стПараметровОбластиКомплект.Расшифровка	= текКомплектКомпозиции.Комплект;
					стПараметровОбластиКомплект.НомерСтроки = текКомплектКомпозиции.НомерСтроки;
					стПараметровОбластиКомплект.Текст 		= "    " + текКомплектКомпозиции.НаименованиеКомплект;
					омТабличныйДокументКлиентСервер.СформироватьОбластьТабличногоДокумента(ПолеВывода, стПараметровОбластиКомплект);
					
					стПараметровОбласти.Текст = "";
					стОтбор.ОбъектДанных = текКомплектКомпозиции.Комплект;
					НайденыеСтрокиКомплекта = тзВывод.НайтиСтроки(стОтбор);
					
					КэшЦвета = Неопределено;
					ВыводОбъекта(НайденыеСтрокиКомплекта, текКомплектКомпозиции, КешЦвета, стПараметровОбласти, смещениеКолонок, стЦвета, ЭтоКомпозиция, Истина, ВыведенныеНаименования);
	...
	Для каждого текДата Из НайденыеСтроки Цикл
		...
		Иначе
			...
			Иначе
				...
				Подряд = текДата.СтруктураДанных[0].Значение.Подряд;
				стПараметровОбласти.ЦветФона 	= ПолучитьЦветЭтапа(?(Подряд И ПолнаяПечать, "Подряд", "") + ВидОбъекта, стЦвета);
			КонецЕсли;
			стПараметровОбласти.НомерСтроки 	= текКомплект.НомерСтроки;	
			стПараметровОбласти.НомерКолонки 	= текДата.НомерДаты + смещениеКолонок;
			
			ТекстЯчейки = ?(Подряд И ПолнаяПечать, "П_", "") + Сред(ВидОбъекта, 1, 3);
...
Процедура ВывестиШапку()
	...
	стПараметровОбласти.ВысотаОбласти 	= 2;
	стПараметровОбласти.Полужирный 		= Истина;
	стПараметровОбласти.Текст 			= "Готовность помещения";
	...
	Если строка_тзЗадач.ДатаОкончания > Дата(1, 1, 1) Тогда
		...
		Пока датаНачала <= строка_тзЗадач.ДатаОкончания Цикл
			...
			Если НЕ ДанныеУжеЗаполнены Тогда
				стДанных = Новый Структура("ВидОбъекта, ЗначениеОбъекта, Подряд", 
					строка_тзЗадач.Этап, строка_тзЗадач.Комплект, строка_тзЗадач.Подряд);
