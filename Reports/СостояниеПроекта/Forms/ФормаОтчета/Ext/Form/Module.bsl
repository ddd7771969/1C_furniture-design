...
Функция РассчитатьПоля(тзДанныеЗапрос) 
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(МАКСИМУМ(ДанныеКомплекта.Период), ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаСогласования,
		...
		|	ЕСТЬNULL(МАКСИМУМ(СостояниеЗамера.Период), ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПринятияПомещения
		|ИЗ
