...
Процедура ДоЗаполнитьПредставления()
	...
	Пока Выборка.Следующий() Цикл
		стОтбор.Сделка = Выборка.Сделка;
		
		НайденыеСтроки = тзКомплекты.НайтиСтроки(стОтбор);
		
		стрСделка = Формат(Выборка.Номер,"ЧЦ=2; ЧВН=") + "(" + Выборка.Префикс + ") " + Выборка.Наименование;
		
		Для каждого х Из НайденыеСтроки Цикл
			х.СделкаПредставление = стрСделка;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	тзКомплекты.Сортировать("СделкаПредставление, Родитель,КомплектПредставление");
	...
	Для каждого х Из тзКомплекты Цикл
		Если НЕ текСделка = х.Проект Тогда
			х.СтрокаСделка = НомерСтроки;
	...
	//ЗаполнитьИтоги(ДатаНачала,ДатаОкончания);
...
Процедура ВывестиДанные()
	...
	стПараметровОбластиСделка = омТабличныйДокументКлиентСервер.ПолучитьСтруктуруПараметров();
	стПараметровОбластиСделка.НомерКолонки 				= 2;
	стПараметровОбластиСделка.ЦветРамки 				= "210210210";
	стПараметровОбластиСделка.ЦветФона 					= ЦветСделки;
	стПараметровОбластиСделка.ГоризонтальноеПоложение 	= ГоризонтальноеПоложение.Лево;
	...
	Для каждого текКомплект Из тзКомплекты Цикл 
		...
		Если текКомплект.СтрокаСделка > 0  Тогда
			КешЦвета 				= Неопределено; 
			стПараметровОбластиСделка.Текст 					= Строка(текКомплект.СделкаПредставление);
			стПараметровОбластиСделка.Расшифровка				= текКомплект.Проект;
			стПараметровОбластиСделка.НомерСтроки 				= текКомплект.СтрокаСделка;
			омТабличныйДокументКлиентСервер.СформироватьОбластьТабличногоДокумента(ПолеВывода, стПараметровОбластиСделка, КешЦвета);
			
			стПараметровОбластиКонстр.Текст 					= "";
			стПараметровОбластиКонстр.Расшифровка				= Неопределено;
			стПараметровОбластиКонстр.НомерСтроки 				= текКомплект.СтрокаСделка;
			стПараметровОбластиКонстр.ЦветФона 					= ЦветСделки;
			...
			стПараметровОбласти.ЦветФона 				= ЦветСделки;
		...
		Если НайденыеСтроки.Количество() > 0 Тогда
			...
			//стОтборДата.Дата = НайденыеСтроки[0].ОкончаниеМонтаж;
...
Процедура ВыводОбъекта(НайденыеСтроки, текКомплект, КешЦвета, стПараметровОбласти, смещениеКолонок, стЦвета = Неопределено)
	Для каждого текДата Из найденыеСтроки Цикл
		Если текКомплект.СтрокаСделка > 0 И ТипЗнч(текДата.СтруктураДанных) = Тип("Структура") Тогда
			стПараметровОбласти.НомерСтроки 	= текКомплект.СтрокаСделка;	
...
Функция ПолучитьВысотуТаблицы()
	...
	Для каждого х Из тзКомплекты Цикл
		Если мСделки.Найти(х.Проект) = Неопределено Тогда
			мСделки.Добавить(х.Проект);
		КонецЕсли;
    ...
    Возврат тзКомплекты.Количество() + мСделки.Количество() + мРодитель.Количество() + 2;
...
Процедура ЗаполнитьТаблицуВывода()
	...
	Для каждого строка_тзЗадач Из ТЗЗадачи Цикл
		ЗаполнитьСделку(строка_тзЗадач, стОтбор, стОтборДата, КоличествоСекундВДне, мКешСделок);
		ЗаполнитьКомплект(строка_тзЗадач, стОтбор, стОтборДата, КоличествоСекундВДне);	
	КонецЦикла;
	
	ДоЗаполнитьСделки(мКешСделок, стОтборДата, стОтбор);
	
КонецПроцедуры // ЗаполнитьТаблицуВывода()

&НаСервере
Процедура ДоЗаполнитьСделки(мКешСделок, стОтборДата, стОтбор)
...
КонецПроцедуры // ДоЗаполнитьСделки(мКешСделок)
...
КонецПроцедуры // ЗаполнитьСделку(строка_тзЗадач, стОтбор, стОтборДата, КоличествоСекундВДне)

&НаСервере
Процедура ЗаполнитьСделку(строка_тзЗадач, стОтбор, стОтборДата, КоличествоСекундВДне, мКешСделок)
	...
	Если НЕ КэшНайден Тогда
		ЭлементМассива 	= Новый Структура("Проект, ДатаМинимум, ДатаМаксимум",стОтбор.ОбъектДанных, Неопределено, Дата(1,1,1));
	...
	ИначеЕсли ЭлементМассива.ДатаМинимум > датаНачала Тогда	
		ЭлементМассива.ДатаМинимум = датаНачала;
	КонецЕсли;
...
КонецПроцедуры // ЗаполнитьСделку(строка_тзЗадач, стОтбор, стОтборДата, КоличествоСекундВДне, мКешСделок)
