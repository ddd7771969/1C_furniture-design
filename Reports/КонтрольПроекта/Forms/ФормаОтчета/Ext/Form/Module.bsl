...
Процедура ЗаполнитьЗадачиДоговора(спСделки)   	
	...
	|	ДатыКомпозиций.КрайТЗ КАК КрайТЗКомпозиций,
	|	ДатыКомплекта.КрайТЗ КАК КрайТЗ,
	|	ДатыКомпозиций.НачалоТЗ КАК НачалоТЗКомпозиций,
	...
	Пока Выборка.Следующий() Цикл
		Если НЕ (Выборка.КрайТЗ = ПустаяДата ИЛИ Выборка.НачалоТЗ = ПустаяДата) Тогда
			...
			Если ЗначениеЗаполнено(Выборка.Композиция) Тогда
				НоваяСтрока.ДатаНачалаПлан 			= Выборка.НачалоТЗКомпозиций;
				НоваяСтрока.ДатаОкончанияПлан 		= Выборка.КрайТЗКомпозиций;
			Иначе	
