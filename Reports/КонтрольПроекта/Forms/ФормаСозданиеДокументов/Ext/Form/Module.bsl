...
Функция СоздатьИзменитьЗадачу(Выборка, Исполнитель, ЗадачаСсылка)
	...
	Если Выборка.Задача = Неопределено Тогда
		...
		КрайняяДатаИсполнения = омДаты.ДобавитьДниСУчетомВыходных(ТекущаяДата(), 2);		
		омЗадачи.СоздатьЗадачуНаСозданиеГабаритногоЧеритежа(Проект, Выборка.ГабаритныйЧертеж, КрайняяДатаИсполнения, Исполнитель);		
