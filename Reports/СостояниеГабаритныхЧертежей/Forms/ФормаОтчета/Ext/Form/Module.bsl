...
Процедура СделкаПриИзмененииНаСервере()
	...
	Если ТипОтчета = 0 Тогда
		...
		Запрос.Текст = 
			...
			|	ЕСТЬNULL(СоздатьМодельГабаритногоЧертежа.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаМодели,
			|	ВЫБОР
			|		КОГДА СостояниеЗамераСрезПоследних.СтатусЗамера = ЗНАЧЕНИЕ(Перечисление.СтатусыЗамера.Рабочий)
			|			ТОГДА СостояниеЗамераСрезПоследних.Период
			...
			|		ПО СостояниеЗамераСрезПоследних.ОбъектЗамера = СоздатьМодельГабаритногоЧертежа.ОбъектТЗ
			...
			|	СоздатьМодельГабаритногоЧертежа.Дата,
