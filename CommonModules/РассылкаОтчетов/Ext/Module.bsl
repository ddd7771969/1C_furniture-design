...
Функция ПараметрыСообщенияДляПодсистемыВзаимодействия(ПараметрыДоставки, ПараметрыПисьма) Экспорт

	МодульВзаимодействия = ОбщегоНазначения.ОбщийМодуль("Взаимодействия");
	Сообщение1 = МодульВзаимодействия.ПараметрыПисьма();
	Сообщение1.Тема  = ПараметрыПисьма.Тема;
	Сообщение1.Текст = ПараметрыПисьма.Тело;
	Сообщение1.Важность = ПараметрыПисьма.Важность; 
	ПараметрыПисьма.Свойство("УведомитьОДоставке", Сообщение1.ДополнительныеПараметры.УведомитьОДоставке);
	ПараметрыПисьма.Свойство("УведомитьОПрочтении", Сообщение1.ДополнительныеПараметры.УведомитьОПрочтении);

	Если ПараметрыПисьма.ТипТекста = "ПростойТекст" ИЛИ ПараметрыПисьма.ТипТекста = ТипТекстаПочтовогоСообщения.ПростойТекст Тогда
		Сообщение1.ДополнительныеПараметры.ФорматПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.ОбычныйТекст;
	Иначе
		Сообщение1.ДополнительныеПараметры.ФорматПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПараметрыДоставки.Получатель) Тогда
		...
		Для Каждого Кому Из АдресаПолучателя Цикл
			НоваяСтрока = Сообщение1[ИмяТаблицыПолучателей].Добавить();
	...
	Если ЗначениеЗаполнено(ПараметрыПисьма.АдресОтвета) Тогда
		НоваяСтрока = Сообщение1.ПолучателиОтвета.Добавить();
