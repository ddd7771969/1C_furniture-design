&НаКлиенте
Функция ПереместитьФайлНаСервер(ИмяФайла, УИД_Вызова = Неопределено, Адрес = Неопределено, УдалитьФайлПослеЗагрузки = Ложь, Параметры = Неопределено) Экспорт 
	
	стЗавершениеОбратныйВызов = Новый Структура("УИД_Вызова, УдалитьФайлПослеЗагрузки, ИмяФайла, Параметры"
												, УИД_Вызова, УдалитьФайлПослеЗагрузки, ИмяФайла, Параметры);
...
Процедура ЗавершениеОбратныйВызов(ОписаниеПомещенногоФайла, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры.УИД_Вызова) Тогда
		...
		Если ЗначениеЗаполнено(ДополнительныеПараметры.Параметры) Тогда
			
			СтрокаКоманды = СтрШаблон(ДополнительныеПараметры.Параметры.ПослеПеремещенияФайла, ИнформацияОФайле.АдресФайла, ИнформацияОФайле.Ошибка); 
...
&НаКлиенте
Функция ПолучитьИмяФайлаСУчетомСущемтвующих(Знач ИмяФайла) Экспорт

	Суфикс = 0;
	ТекущийФайл = Новый Файл(ИмяФайла);
	
	Пока ТекущийФайл.Существует() Цикл
	
		Суфикс = Суфикс + 1;
		
		ИмяФайла = ТекущийФайл.Путь + ТекущийФайл.ИмяБезРасширения + "_" + Суфикс + ТекущийФайл.Расширение;
...
КонецФункции // ПолучитьИмяФайлаСУчетомСущемтвующих()
