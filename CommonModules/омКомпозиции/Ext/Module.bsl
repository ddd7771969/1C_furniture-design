...
Функция ПолучитьКомплектыКомпозиции(мКомпозиции) Экспорт
	...
	Запрос.Текст = 
		...
		|	ДанныеКомплектаСрезПоследних.Конструктор КАК Конструктор,
		|	ДанныеКомплектаСрезПоследних.ТекущаяСтадия КАК ТекущаяСтадия,
		|	ДанныеКомплектаСрезПоследних.Комплект КАК Композиция,
		|	ЗНАЧЕНИЕ(Справочник.ИзделияКомплект.ПустаяСсылка) КАК Комплект
		|ИЗ
