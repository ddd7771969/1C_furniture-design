...
Функция ЗаполнитьКоличествоДнейКомпозиции(мКомпозиции) Экспорт
	...
	Для каждого ТекущаяКомпозиция Из мКомпозиции Цикл
		...
		Если ТребуетсяИзменения ИЛИ ТребуетсяИзмененияГант Тогда 
			...
			стДанные = Новый Структура("Проект, Комплект, ДнейПКД, НормаВремени_ПКД, НачалоПКД"
				, Проект, текДанные.Композиция, ДнейПКД, НормаВремени_ПКД, НачалоПКД);
...
Процедура ЗаписатьВДатыКомплектаБезПерерасчета(стДанные) Экспорт
	...
	НаборЗаписей.Отбор.Проект.Установить(стДанные.Проект);
	...
	НоваяЗапись.Проект 				= стДанные.Проект;
...
Функция ПолучитьКомплектыКомпозиции(Композиция) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ДанныеКомплектаСрезПоследних.Комплект = &Композиция
		|
		|УПОРЯДОЧИТЬ ПО
	...
	тзДанные = Результат[0].Выгрузить();
	
	Выборка = Результат[1].Выбрать();
	ДанныеКомпозиции = Новый Структура("Конструктор, ТекущаяСтадия", );
	Пока Выборка.Следующий() Цикл

		ЗаполнитьЗначенияСвойств(ДанныеКомпозиции, Выборка);		
		
	КонецЦикла;
	
	Возврат Новый Структура("ДанныеКомплектов, ДанныеКомпозиции", тзДанные, ДанныеКомпозиции);
	
КонецФункции // ПолучитьКомплектыКомпозиции()

Функция ПолучитьКомпозициюПоКомплекту(Комплект) Экспорт

	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ИзделияКомплект.Родитель.Композиция КАК Композиция,
		|	ДанныеКомплектаСрезПоследних.Конструктор КАК КонструкторКомпозиции,
		|	ДанныеКомплектаСрезПоследних.ТекущаяСтадия КАК СтадияКомпозиции
		|ИЗ
		|	Справочник.ИзделияКомплект КАК ИзделияКомплект
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеКомплекта.СрезПоследних КАК ДанныеКомплектаСрезПоследних
		|		ПО (ДанныеКомплектаСрезПоследних.Комплект = ИзделияКомплект.Ссылка)
	...
	Данные = Новый Структура("Композиция, КонструкторКомпозиции, СтадияКомпозиции", );
...
КонецФункции // ПолучитьКомпозициюПоКомплекту()
