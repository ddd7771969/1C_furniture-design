...
Функция ПолучитьПредставление(ТекущиеДанные, ТекущаяГруппировка)
	...
	ИначеЕсли ТекущаяГруппировка = "Проект" Тогда
		Возврат Формат(ТекущиеДанные.СделкаНомер, "ЧЦ=3; ЧВН=") + " (" + ТекущиеДанные.ПроектПрефикс + ") " + ТекущиеДанные.СделкаНаименование;
...
Функция ПолучитьТаблицуДляДиаграммыПланРаботы(стПериоды, стФильтры, Конструктор = Ложь)
	...
	Запрос.Текст = 
		...
		|	ИзделияКомплектИзделияЭлемент.Ссылка.Проект.Префикс КАК ПроектПрефикс,
...
Функция ТекстЗапросаОбъеденятьВКомпозицию()
    ...
    Возврат "ВЫБРАТЬ
            ...
            |	ИзделияКомплект.ОсновноеПомещение.Наименование КАК ОсновноеПомещениеНаименование
            ...
            |	ИзделияКомплект.Родитель.Композиция.ОсновноеПомещение,
            |	ИзделияКомплект.Родитель.Композиция.ОсновноеПомещение.Наименование
            |ИЗ
            |	Справочник.ИзделияКомплект КАК ИзделияКомплект
            |ГДЕ
            |	ИзделияКомплект.Проект = &Проект
            ...
            |	ЕСТЬNULL(ДанныеКомплекта.ВнешняяДатаНачалоТЗ, ДАТАВРЕМЯ(1, 1, 1)) КАК НачалоТЗ,
            |	ЕСТЬNULL(ДанныеКомплекта.ВнешняяДатаНачалоРКД, ДАТАВРЕМЯ(1, 1, 1)) КАК НачалоРКД,
            |	ЕСТЬNULL(ДанныеКомплекта.ВнешняяДатаОкончаниеТЗ, ДАТАВРЕМЯ(1, 1, 1)) КАК ОкончаниеТЗ,
            |	ЕСТЬNULL(ДанныеКомплекта.ВнешняяДатаОкончаниеРКД, ДАТАВРЕМЯ(1, 1, 1)) КАК ОкончаниеРКД,
            ...
            |	ИзделияКомплект.Комплект КАК Комплект,
            |	ЕСТЬNULL(ИзделияКомплект.Комплект.Родитель.Композиция, ЗНАЧЕНИЕ(Справочник.Композиции.ПустаяСсылка)) КАК Композиция,
            |	ЕСТЬNULL(НазначениеПодрядчикаКомплекты.ПКД, ЛОЖЬ) КАК ПодрядПКД,
            |	ЕСТЬNULL(НазначениеПодрядчикаКомплекты.РКД, ЛОЖЬ) КАК ПодрядРКД,
            |	ЕСТЬNULL(НазначениеПодрядчикаКомплекты.Производство, ЛОЖЬ) КАК ПодрядПроизводство,
            |	ДанныеКомплекта.ТекущаяСтадия.ПолноеНаименование КАК СтадияПолноеНаименование
            |ИЗ
            ...
            |		ПО (ДанныеКомплекта.Комплект = ИзделияКомплект.Комплект)
            |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.НазначениеПодрядчика.Комплекты КАК НазначениеПодрядчикаКомплекты
            |		ПО ИзделияКомплект.Комплект = НазначениеПодрядчикаКомплекты.Комплект
            |ГДЕ
            |	ВЫБОР
...
Функция ТекстЗапросаТолькоКомплекты()
    ...
    Возврат "ВЫБРАТЬ
            ...
            |	ЕСТЬNULL(ИзделияКомплект.Родитель.Композиция.Наименование, """") КАК КомпозицияНаименование
            |ИЗ
            |	Справочник.ИзделияКомплект КАК ИзделияКомплект
            |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеКомплекта.СрезПоследних(, ) КАК ДанныеКомплекта
...
Функция ПолучитьИнтервалыПомещения(тзДатыПроекта) Экспорт
	...
	тзИнтервалыПомещения.Колонки.Добавить("НачалоТЗ", 					Новый ОписаниеТипов("Дата"));
	тзИнтервалыПомещения.Колонки.Добавить("ОкончаниеТЗ", 				Новый ОписаниеТипов("Дата"));
	тзИнтервалыПомещения.Колонки.Добавить("НачалоРКД", 					Новый ОписаниеТипов("Дата"));
	тзИнтервалыПомещения.Колонки.Добавить("ОкончаниеРКД", 				Новый ОписаниеТипов("Дата"));
	...
	Для каждого х Из тзДатыПроекта Цикл
		...
		АнализДатыНачала("НачалоТЗ", 					ПустаяДата, х, НайденаяСтрока);
		...
		АнализДатыНачала("НачалоРКД", 					ПустаяДата, х, НайденаяСтрока);
...
Процедура ВключитьИнтервалНеделяНаГанте(Диаграмма) Экспорт

	Если Диаграмма.ОбластьПостроения.ШкалаВремени.Элементы.Количество() = 0 Тогда
		ЭлементШкалыНеделя = Диаграмма.ОбластьПостроения.ШкалаВремени.Элементы.Добавить();
	Иначе	
		ЭлементШкалыНеделя = Диаграмма.ОбластьПостроения.ШкалаВремени.Элементы[0];
	КонецЕсли;
	ЭлементШкалыНеделя.Единица = ТипЕдиницыШкалыВремени.Неделя;
	ЭлементШкалыНеделя.Формат = "ДФ=dd.MM";

КонецПроцедуры
 
Процедура ДобавитьТекущуюДатуНаДиаграмму(Диаграмма) Экспорт

	Если Диаграмма.ОбластьПостроения.ШкалаВремени.Элементы.Количество() < 2 Тогда 
		
		Диаграмма.ОбластьПостроения.ШкалаВремени.Элементы.Добавить(); 
		
	КонецЕсли;
	
	ЭлементМеток = Диаграмма.ОбластьПостроения.ШкалаВремени.Элементы[1]; 
	ЭлементМеток.ОтображатьПериодическиеМетки = Ложь;
	
	ЭлементМеток.Метки.Очистить();
	
	Метка = ЭлементМеток.Метки.Добавить(ТекущаяДата()); 
	Метка.Текст = "Текущая дата " + Формат(ТекущаяДата(), "ДФ=dd.MM.yy"); 
