...
Функция ОбновитьГантНаСервере(стПараметры) Экспорт
	...
	ТаблицаДат = Справочники.ПриложенияКДоговорам.ПолучитьДатыКомплектовПриложенияКДоговору(СтруктураДанныеГантаВр.тзДанныеГанта.ВыгрузитьКолонку("ОбъектТЗ"));
	...
	стПараметры.Вставить("тзИсполнитель", 	СтруктураДанныеГантаВр.тзИсполнитель);
	...
	ЗаполнитьВыходные(Диаграмма, стПромежуточныеДанные, тзЦвета);
	
	омГантНаСервере.ДобавитьТекущуюДатуНаДиаграмму(Диаграмма);
	
	ПостроитьДиаграммуГанта(стПараметры, стПромежуточныеДанные, тзЦвета, ТаблицаДат);
...
Процедура ЗаполнитьВыходные(Диаграмма, стПромежуточныеДанные, тзЦвета)

	НайденаяСтрока = тзЦвета.Найти("Выходной", "НаименованиеЭтапа");
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ВыходныеДни.КоличествоРабочихЧасов = 0
		|	И ВыходныеДни.Дата >= &ДатаНачала
		|	И ВыходныеДни.Дата <= &ДатаОкончания";
	
	Запрос.УстановитьПараметр("ДатаНачала", стПромежуточныеДанные.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", стПромежуточныеДанные.ДатаОкончания);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл 
		
		ИнтФ		= Диаграмма.ИнтервалыФона.Добавить(Выборка.Дата, КонецДня(Выборка.Дата));
		ИнтФ.Цвет 	= ЦветФона;
...
Функция ВывестиПроект(стПараметры, текПроект, ДанныеТочки = Неопределено, тзЦвета = Неопределено, стОтборПроект = Неопределено) Экспорт
	...
	ДанныеТочки.ДатаНачала 			= НачалоДня(стДаты.ДатаНачала);
	ДанныеТочки.ДатаОкончания	 	= КонецДня(стДаты.ДатаОкончания - 1);
...
Функция ВывестиИсполнитель(стПараметры, текИсполнитель, ИсполнительНаименование, ДанныеТочки = Неопределено, тзЦвета = Неопределено, стОтборИсполнитель = Неопределено) Экспорт
	...
	Если стОтборИсполнитель = Неопределено Тогда
		...
		НайденыеСтроки					= стПараметры.тзДанныеГанта;
	Иначе
		НайденыеСтроки 					= стПараметры.тзДанныеГанта.НайтиСтроки(стОтборИсполнитель);
	КонецЕсли;
	...
	Если Обновить Тогда
		ДанныеТочки 				= ПолучитьДанныеТочки(Диаграмма);
		ДанныеТочки.СсылкаЗначение 	= ИсполнительНаименование;
		тзЦвета 					= омДиаграммаГанта.ПолучитьТаблицуЦветов();
	КонецЕсли;
	...
	ДанныеТочки.ДатаНачала 			= НачалоДня(стДаты.ДатаНачала);
	ДанныеТочки.ДатаОкончания	 	= КонецДня(стДаты.ДатаОкончания - 1);
	//ДанныеТочки.Продолжительность 	= Окр((стДаты.ДатаОкончания - стДаты.ДатаНачала) / (24 * 3600), 0);
	ДанныеТочки.НаименованиеТочки 	= ИсполнительНаименование;
	ДанныеТочки.Этап 				= "";
...
Функция ПолучитьТаблицуДанных() Экспорт
	...
	тзДанныеГанта.Колонки.Добавить("СтадияНаименованиеШаблон", 	ТипСтрока);
	...
	тзИсполнитель.Колонки.Добавить("РежимРаботы",				Новый ОписаниеТипов("ПеречислениеСсылка.РежимыРаботы"));
...
Процедура ПостроитьДиаграммуГанта(стПараметры, стПромежуточныеДанные, тзЦвета, ТаблицаДат)
	...
	Если Сортировка = 0 Тогда        
		...
		Для каждого СтрокаПроекты Из стПараметры.тзПроекты Цикл
			...
			Для каждого СтрокаИсполнителя Из стПараметры.тзИсполнитель Цикл
				...
				ИсполнительНаименование = ?(ПустаяСтрока(СтрокаИсполнителя.ИсполнительНаименование), "Без конструктора", СтрокаИсполнителя.ИсполнительНаименование); 
				
				ДанныеТочки.СсылкаЗначение 		= ИсполнительНаименование + СтрокаПроекты.ПроектНаименование;
				ДанныеТочки.РодительДиаграммы 	= СтрокаПроекты.ПроектНаименование;
				ДанныеТочки.РежимРаботы 		= СтрокаИсполнителя.РежимРаботы;
				
				стОтборИсполнитель.Исполнитель	= текИсполнитель; 
				
				Если ВывестиИсполнитель(стПараметры, текИсполнитель, ИсполнительНаименование, ДанныеТочки, тзЦвета, стОтборИсполнитель) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
				
				ДанныеТочки.РодительДиаграммы 	= ИсполнительНаименование + СтрокаПроекты.ПроектНаименование;
				стОтборОбъектТЗ.Исполнитель 	= текИсполнитель; 
				
				Для каждого текОбъектТЗ Из стПромежуточныеДанные.мОбъектТЗ Цикл
					ВывестиОбъектТЗ(Диаграмма, тзДанныеГанта, текОбъектТЗ, стОтборОбъектТЗ, тзЦвета, ДанныеТочки, ТаблицаДат, стПромежуточныеДанные);
				КонецЦикла;
	...
	ИначеЕсли Сортировка = 1 Тогда
		...
		Для каждого СтрокаИсполнителя Из стПараметры.тзИсполнитель Цикл
			...
			ДанныеТочки.СсылкаЗначение 	= СтрокаИсполнителя.ИсполнительНаименование;
			ДанныеТочки.РодительДиаграммы 	= Неопределено;
			ДанныеТочки.РежимРаботы 		= СтрокаИсполнителя.РежимРаботы;
			стОтборИсполнитель.Исполнитель	= текИсполнитель; 
			
			Если ВывестиИсполнитель(стПараметры, текИсполнитель, СтрокаИсполнителя.ИсполнительНаименование, ДанныеТочки, тзЦвета, стОтборИсполнитель) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ДанныеТочки.РодительДиаграммы = СтрокаИсполнителя.ИсполнительНаименование;
...
Процедура ВывестиОбъектТЗ(Диаграмма, тзДанныеГанта, текОбъектТЗ, стОтборОбъектТЗ, тзЦвета, ДанныеТочки, ТаблицаДат, стПромежуточныеДанные)
	...
	//НайденаяСтрокаДаты = ТаблицаДат.Найти(текОбъектТЗ, "КомплектыКомпозиции");
	//
	//Если НЕ НайденаяСтрокаДаты = Неопределено И НайденаяСтрокаДаты.СрокОкончанияПКД > ПустаяДата Тогда
	//	
	//	ЗначениеКрайненйДаты = НайденаяСтрокаДаты.СрокОкончанияПКД;
	//	
	//	СекундВСуткахВЧетырехНеделях = 28 * 24 * 3600;
	//	Если стПромежуточныеДанные.ДатаНачала - СекундВСуткахВЧетырехНеделях <= ЗначениеКрайненйДаты 
	//		И стПромежуточныеДанные.ДатаОкончания + СекундВСуткахВЧетырехНеделях >= ЗначениеКрайненйДаты Тогда
	//		
	//		ДанныеТочки.ДатаОкончания	= ЗначениеКрайненйДаты;
	//		ДанныеТочки.ДатаНачала 		= ЗначениеКрайненйДаты;
	//		ДанныеТочки.Этап			= "";
	//		
	//		ВывестиТочку(Диаграмма, ДанныеТочки, тзЦвета, WebЦвета.ТемноФиолетовый);
	//		
	//	КонецЕсли;
	//	
	//КонецЕсли;
	...
	Для каждого НайденаяСтрока Из НайденыеСтроки Цикл
		...
		ДанныеТочки.ДатаОкончания	  = КонецДня(НайденаяСтрока.ДатаОкончания - 1);
		//ДанныеТочки.Продолжительность = Окр(НайденаяСтрока.Продолжительность, 0);
...
Функция ПолучитьДанныеТочки(Диаграмма)
	...
	ДанныеТочки.Вставить("РежимРаботы", 			Перечисления.РежимыРаботы.ПустаяСсылка()); 
...
Функция ПолучитьДатаНачалаОкончания(НайденыеСтроки)
	...
	Для каждого НайденаяСтрока Из НайденыеСтроки Цикл
		...
		ИспользоватьДату 		= НЕ НайденаяСтрока.ДатаОкончания 		= ПустаяДата;	
		ИспользоватьДатуПлан 	= НЕ НайденаяСтрока.ДатаОкончанияПлан 	= ПустаяДата;	
		
		Если стДаты.ДатаНачала = ПустаяДата Тогда
			Если НайденаяСтрока.ЭтоПлан Тогда
		...
		Если НайденаяСтрока.ПланРазработкиВДнях = 0 Тогда
			ДатаОкончанияСВыходными = НайденаяСтрока.ДатаНачала + СекундВСутках
		Иначе
			//ДатаОкончанияСВыходными = омДаты.ДобавитьДниСУчетомВыходных(НайденаяСтрока.ДатаНачала, НайденаяСтрока.ПланРазработкиВДнях);
			ДатаОкончанияСВыходными = НайденаяСтрока.ДатаНачала + НайденаяСтрока.ПланРазработкиВДнях * СекундВСутках;
		КонецЕсли;
		...
		Если НЕ НайденаяСтрока.ЭтоПлан Тогда
			Если ТД > стДаты.ДатаОкончания  Тогда
			
				стДаты.ДатаОкончания = ТД;	
...
Функция ВывестиТочку(Диаграмма, ДанныеТочки, тзЦвета, ТекущийЦвет = Неопределено, Обновить = Ложь)
	...
	Интервал.Конец 	= ДанныеТочки.ДатаОкончания;
	...
	Если ДанныеТочки.ЕстьРасшифровка Тогда
		Интервал.Расшифровка 	= Новый Структура("Проект, Этап, СсылкаКомплект, Исполнитель, Состояние, РежимРаботы"
		, ДанныеТочки.Проект, ДанныеТочки.Этап, ДанныеТочки.СсылкаЗначение
		, ДанныеТочки.Исполнитель, ДанныеТочки.Состояние, ДанныеТочки.РежимРаботы);
	Иначе
...
Функция ПолучитьПромежуточныеДанные(тзДанныеГанта)
	...
	Для каждого х Из тзДанныеГанта Цикл 
		...
		Если х.ДатаНачала > ПустаяДата И НЕ х.ПродленныйЭтап И х.ДатаОкончания = ПустаяДата Тогда
			х.ДатаОкончания = омДаты.ДобавитьДниСУчетомВыходных(х.ДатаНачала, х.ПланРазработкиВДнях); 			
		КонецЕсли;
...
Функция ПолучтьДанныеОПаузеКомплекта(Задача, ПриостановкаКонструктором, ТекущаяСтрокаТаблицы)
	...
	Если Выборка.Количество() = 2 Тогда
		...
		Если Состояние0 = ПриостановкаКонструктором Тогда
			...
			//ТекущаяСтрокаТаблицы.Продолжительность 	= Окр((ТД - Период1) / СекундВДне, 0);
			ТекущаяСтрокаТаблицы.ДатаОкончания 		= ТД;
			
		Иначе
			
			ТекущаяСтрокаТаблицы.ДатаОкончания = омДаты.ДобавитьДниСУчетомВыходных(ТекущаяСтрокаТаблицы.ДатаНачала, ТекущаяСтрокаТаблицы.ПланРазработкиВДнях); 
...
Процедура ПолучитьДанныеДиаграммы(Конструктор, Подразделение, Проект, СтруктураДанныеГанта)
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ПодразделенияСрезПоследних.Подразделение, ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка)) КАК Подразделение,
		...
		|	ЕСТЬNULL(ТаймингЗадачСрезПоследних.ПереходРедакция, 0) + ЕСТЬNULL(ТаймингЗадачСрезПоследних.ПереходРекламация, 0) + 1 КАК Переходы,
		|	ТаймингЗадачСрезПоследних.Период КАК Период,
		|	ЕСТЬNULL(ДанныеКомплекта.ТекущаяСтадия.ОграниченныйПоказ, ЛОЖЬ) КАК ТекущаяСтадияОграниченныйПоказ,
		|	ДанныеКомплекта.ТекущаяСтадия.ШаблонНаименования КАК СтадияНаименованиеШаблон
		|ПОМЕСТИТЬ втТайминг
		|ИЗ
		...
		|			ПО ТаймингЗадачСрезПоследних.УИД = ТаймингЗадачКомментарии.УИД
		|		ПО втЗадачи.Задача = ТаймингЗадачСрезПоследних.Задача
		...
		|ГДЕ
		|	НЕ ДанныеКомплекта.ТекущаяСтадия.ОграниченныйПоказ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	втЗадачи.ОбъектТЗ КАК ОбъектТЗ,
		|	втЗадачи.Задача.Исполнитель КАК Исполнитель,
		...
		|	втТайминг.Период КАК ДатаНачала,
		|	ВЫБОР
		...
		|	втЗадачи.Задача.Исполнитель.Наименование КАК ИсполнительНаименование,
		...
		|	втТайминг.СтадияНаименованиеШаблон КАК СтадияНаименованиеШаблон,
		|	втТайминг.Переходы КАК Переходы,
		...
		|		ПО втЗадачи.Задача.Исполнитель.Сотрудник = ДанныеСотрудниковСрезПоследних.Сотрудник
		|ГДЕ
		|	(ВЫБОР
		|				КОГДА втЗадачи.Этап = ""ПКД""
		...
		|	И НЕ втТайминг.ТекущаяСтадияОграниченныйПоказ";
