...
Функция ВывестиИсполнитель(стПараметры, текИсполнитель, ДанныеТочки = Неопределено, тзЦвета = Неопределено, стОтборИсполнитель = Неопределено) Экспорт
	...
	Если Обновить Тогда
		ДанныеТочки = ПолучитьДанныеТочки(Диаграмма);
		ДанныеТочки.СсылкаЗначение 			= Строка(текИсполнитель);
		тзЦвета 	= омДиаграммаГанта.ПолучитьТаблицуЦветов();
	КонецЕсли;
	...
	ДанныеТочки.Продолжительность 	= Окр((стДаты.ДатаОкончания - стДаты.ДатаНачала) / (24 * 3600), 0);
	ДанныеТочки.НаименованиеТочки 	= Строка(текИсполнитель);
	ДанныеТочки.Этап 				= "";
	...
	Если Сортировка = 0 Тогда        
		...
		Для каждого СтрокаПроекты Из стПараметры.тзПроекты Цикл
			...
			Для каждого СтрокаИсполнителя Из стПараметры.тзИсполнитель Цикл
				...
				Если ВывестиИсполнитель(стПараметры, текИсполнитель, ДанныеТочки, тзЦвета, стОтборИсполнитель) = Неопределено Тогда
					Продолжить;
				КонецЕсли;
