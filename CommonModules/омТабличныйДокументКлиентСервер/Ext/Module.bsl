Функция ПолучитьСтруктуруПараметров(НетГраници = Ложь) Экспорт
	...
	ЛинияГраницы = ?(НетГраници, Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.НетЛинии, 1), Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1)); 
	стПараметры.Вставить("ГраницаСлева",	ЛинияГраницы);
	стПараметры.Вставить("ГраницаСправа",	ЛинияГраницы);
	стПараметры.Вставить("ГраницаСверху",	ЛинияГраницы);
	стПараметры.Вставить("ГраницаСнизу",	ЛинияГраницы);
...
Функция СформироватьОбластьТабличногоДокумента(ТабДок, стПараметры, КэшЦвета = Неопределено) Экспорт
	...
	Если КэшЦвета = Неопределено Тогда
		ЦветФона 	= ?(ТипЗнч(стПараметры.ЦветФона) 	= Тип("Цвет"), 	стПараметры.ЦветФона, 	омЦветНаСервере.ПолучитьЦветИзСтроки(стПараметры.ЦветФона));
		ЦветТекста 	= ?(ТипЗнч(стПараметры.ЦветТекста) 	= Тип("Цвет"), 	стПараметры.ЦветТекста, омЦветНаСервере.ПолучитьЦветИзСтроки(стПараметры.ЦветТекста));
		ЦветРамки   = ?(ТипЗнч(стПараметры.ЦветРамки) 	= Тип("Цвет"), 	стПараметры.ЦветРамки, 	омЦветНаСервере.ПолучитьЦветИзСтроки(стПараметры.ЦветРамки));
...
Функция ВывестиЛинию(ТабДок, стПараметры) Экспорт
	...
	Линия.ЦветЛинии = омЦветНаСервере.ПолучитьЦветИзСтроки(стПараметры.ЦветЛинииСтрокой);
...
Процедура ОбвестиОбласть(ТабДок, Линия  = Неопределено, ЛинииВнутриОбласти = Ложь, ИмяОбластиПараметр = Неопределено) Экспорт

	ЛинияГраницы = ?(Линия  = Неопределено, Новый Линия(ТипЛинииЯчейкиТабличногоДокумента.Сплошная, 1), Линия); 
	
	ВысотаСтрокой = ЧислоБезРазделителей(ТабДок.ВысотаТаблицы + 1);
	ШиринаСтрокой = ЧислоБезРазделителей(ТабДок.ШиринаТаблицы + 1);
	
	Если ЛинииВнутриОбласти Тогда
		
		Если ИмяОбластиПараметр = Неопределено Тогда
			ИмяОбласти = "R1C1:R" + ВысотаСтрокой + "C" + ШиринаСтрокой;
			Область = ТабДок.Область(ИмяОбласти);
		Иначе
			Область = ТабДок.Область(ИмяОбластиПараметр);
		КонецЕсли;
	...
	Иначе
		...
		ИмяОбласти 					= "R1C1:R1C" + ШиринаСтрокой;
		Область 					= ТабДок.Область(ИмяОбласти);
		Область.ГраницаСверху      	= ЛинияГраницы; 
	
		ИмяОбласти 					= "R1C1:R" + ВысотаСтрокой + "C1";
		Область 					= ТабДок.Область(ИмяОбласти);
		Область.ГраницаСлева      	= ЛинияГраницы; 
	
		ИмяОбласти 					= "R" + ВысотаСтрокой + "C1:R" + ВысотаСтрокой + "C" + ШиринаСтрокой;
		Область 					= ТабДок.Область(ИмяОбласти);
		Область.ГраницаСнизу      	= ЛинияГраницы; 
	
		ИмяОбласти 					= "R1C" + ШиринаСтрокой + ":R" + ВысотаСтрокой + "C" + ШиринаСтрокой;
		Область 					= ТабДок.Область(ИмяОбласти);
		Область.ГраницаСправа      	= ЛинияГраницы; 
