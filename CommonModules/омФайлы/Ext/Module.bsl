Функция ПолучитьПутьКФайлуПДМ(Объект, Проект = Неопределено, НаименованиКомплекта = Неопределено) Экспорт
	...
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Композиции") Тогда
		...
		ПутьКФайлу = ПолучитьПутьКФайлуКомпозиции(Объект, Проект);
...
Функция ПолучитьПутьКФайлуКомпозиции(Композиция, Проект)

	Если ЗначениеЗаполнено(Проект) Тогда
	
		Возврат омПроектыСерверПовтор.ПолучитьПутьКИменамПапокПроекта(Проект).ПроектИмяПапкиКомпозицииВPDM;	
...
Функция ПолучитьПутьКФайлуГабаритныеЧертежи(ГЧ, Проект)
	...
	Если ЗначениеЗаполнено(Проект) Тогда
		...
		Возврат омПроектыСерверПовтор.ПолучитьПутьКИменамПапокПроекта(Проект).ПроектИмяПапкиГЧВPDM;	
...
Функция ПолучитьПутьКФайлуКомплект(Комплект, Проект, НаименованиКомплекта)
	...
	Если ЗначениеЗаполнено(Проект) И ЗначениеЗаполнено(НаименованиКомплекта) Тогда
		...
		ПроектИмяКорневойПапкиВPDM = омПроектыСерверПовтор.ПолучитьПутьКИменамПапокПроекта(Проект).ПроектИмяПапкиКомпозицииВPDM;
