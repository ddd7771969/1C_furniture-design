...
Функция СоеденитьИмяКаталогаИмяФайла(ИмяКаталга, ИмяФайла) Экспорт
	
	Если Прав(ИмяКаталга, 1) = "\" Тогда
		Если Лев(ИмяФайла, 1) = "\" Тогда
			ПолныйПуть = ИмяКаталга + Сред(ИмяФайла, 2); 
		Иначе	
			ПолныйПуть = ИмяКаталга + ИмяФайла; 
		КонецЕсли;
	Иначе	
		Если Лев(ИмяФайла, 1) = "\" Тогда
			ПолныйПуть = ИмяКаталга + ИмяФайла; 
...
КонецФункции // СоеденистьИмяКаталогаИмяФайла()
