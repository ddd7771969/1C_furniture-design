...
Функция ЗаполнитьКомплектыКонструктора(Проект, Конструктор, НепоказыватьОжиданиеГЧ = Ложь) Экспорт
	...
	Запрос.Текст = 
		...
		|	И НЕ Задачи_ПКД.ТипЗадачи В (&ИсключениеТиповЗадач)
		|	И ВЫБОР
		...
		|	И НЕ Задачи_РКД.ТипЗадачи В (&ИсключениеТиповЗадач)
		|	И ВЫБОР
		|			КОГДА &Проект = ЗНАЧЕНИЕ(Справочник.Проекты.ПустаяСсылка)
		|				ТОГДА ИСТИНА
	...
	ИсключениеТиповЗадач = Новый Массив;
	ИсключениеТиповЗадач.Добавить(Перечисления.ТипыЗадачКонструирования.Конструктор);	
	...
	Запрос.УстановитьПараметр("ИсключениеТиповЗадач", 	ИсключениеТиповЗадач);
