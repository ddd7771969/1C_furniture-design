...
Функция ПолучитьДанныеДляКода(ОсновноеИзделие, Проект) Экспорт
	...
	стВозврата.НомерПроекта = Формат(стВозврата.НомерПроекта, "ЧЦ=2; ЧВН=; ЧГ=0");
...
Процедура УстановитьКодПДМКодКомплекта(КодКомплект, Код_в_PDM) Экспорт
	...
	Если ЗначениеЗаполнено(ИзделиеКомплект) И Код_в_PDM > 0 Тогда
...
Функция КомплектЧастьКомпозиции(ИзделиеКомплект) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Возврат Выборка.Композиция;
	КонецЦикла;
	
	Возврат Справочники.Композиции.ПустаяСсылка();

КонецФункции // КомплектЧастьКомпозиции()
