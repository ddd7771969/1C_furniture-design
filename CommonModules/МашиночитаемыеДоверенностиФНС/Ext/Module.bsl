...
Функция ДоверенностиСОтбором(Отбор, ВыбранныеПоля, Знач НаДату = Неопределено) Экспорт
	...
	Если ВыбранныеПоля = Неопределено Тогда
		...
		ВыбранныеПоля.Добавить("Представление");
	...
	|	МашиночитаемыеДоверенности.Представление КАК Представление,
...
Функция ДобавитьПодписьКФайлуДоверенности(ФайлДоверенности, Знач Подпись) Экспорт
	...
	Результат = МашиночитаемыеДоверенностиФНССлужебный.ПроверитьСертификатДоверителя(Доверенность, ФайлДоверенности, Подпись.Сертификат);
	
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		Возврат Результат;
	КонецЕсли;
	
	Подпись.ТребуетсяПроверка = Истина;
	ЭлектроннаяПодпись.ДобавитьПодпись(ФайлДоверенности, Подпись);
	
	Если Результат = Истина Тогда
		МашиночитаемыеДоверенностиФНССлужебный.УстановитьПризнакПодписана(Доверенность, Истина);
		МашиночитаемыеДоверенностиФНССлужебный.РассчитатьИЗаписатьТехническийСтатус(Доверенность);
	КонецЕсли;
