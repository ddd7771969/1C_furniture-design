...
//Изменяет порядок элементов массива.
//
// Параметры:
//  Массив - Массив - массив произвольных значений.
//
// Возвращаемое значение:
//  Массив - 0 элемент равен последнему элементу входящего массива.
//
&НаСервере
Функция ИзменитьПорядокМассива(Массив) Экспорт
	...
	мВозврата = Новый Массив(РазмерМассива);

	ОбратныйИндекс = РазмерМассива - 1;
	
	Для х = 0 По ОбратныйИндекс Цикл
		мВозврата[х] = Массив[ОбратныйИндекс - х];
	КонецЦикла;
	
	Возврат мВозврата;
КонецФункции // ИзменитьПорядок(мЭтапы)
