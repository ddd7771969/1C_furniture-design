Процедура УправлениеКонтроляВыгрузкиВPDM(Запуск = Ложь, Интервал = 20, КоличествоПопыток = 7) Экспорт
...
Процедура РазобратьИмяФайла(Файл)
	...
	мПрефиксовДляАнализаОшибки = Новый Массив(4);
	...
	ИначеЕсли КоличествоЭлементов = 3 Тогда 
		...
		ИначеЕсли ИмяПрефикса = "Task" Тогда
...
Процедура ПослеПереносаТЗ(Параметры) Экспорт
	...
	Если ПрефиксКода = "K" Тогда //Комплект
		
		ТипСправочника = "комплекта ";
		стДанные = омPDMНаСервере.ПолучитьКомплектПоКоду(Параметры.КодЭлемента);
		
	Иначе //Композиция
		
		ТипСправочника = "композиции ";
		стДанные = омPDMНаСервере.ПолучитьКомпозициюПоКоду(Параметры.КодЭлемента);
		
	КонецЕсли;
	
	Если Параметры.ЗначениеСуфикса = "0" Тогда
	
		ТекстСообщения = " Не удалось перенести файл ТЗ для " + ТипСправочника + " " + стДанные.Наименование;	
	
	Иначе
	
		ТекстСообщения = " Файл ТЗ перенесен для " + ТипСправочника + " " + стДанные.Наименование;
		
		омКанбанТЗ.ПереместитьРеквизитВОжидание(стДанные.Ссылка);	
	КонецЕсли;
