Процедура УправлениеКонтроляВыгрузкиВPDM(Запуск = Ложь, Интервал = 2, КоличествоПопыток = 300) Экспорт
...
Процедура РазобратьИмяФайла(Файл)
	...
	ИначеЕсли КоличествоЭлементов = 3 Тогда 
		...
		ИначеЕсли ИмяПрефикса = "MP" Тогда
			...
			ПараметрыЧтения = Новый Структура("ИмяСправочника, ТекстДок, ЗначениеСуфикса, КодЭлемента, Файл", ИмяСправочника, ТекстДок, ЗначениеСуфикса, КодЭлемента, Файл);
		...
		ИначеЕсли ИмяПрефикса = "MPC" Тогда
			...
			ПараметрыЧтения = Новый Структура("ИмяСправочника, ЗначениеСуфикса, КодЭлемента, Файл", ИмяСправочника, ЗначениеСуфикса, КодЭлемента, Файл);
			ОписаниеОповещения = Новый ОписаниеОповещения("ПослеЗабратьПрава", ом_ПДМ_на_клиенте, ПараметрыЧтения); 
...
Процедура ПослеЧтенияФайлаКомпозиции(Параметры) Экспорт
	...
	Если НЕ(КодPDM = "0" ИЛИ ПустаяСтрока(КодPDM)) Тогда
		...
		омКомпозицииСервер.УстановитьКодПДМДляКомпозиции(Параметры.КодЭлемента, Число(КодPDM)); 
	КонецЕсли;
...
Процедура ПослеЧтенияФайлаПроекты(Параметры) Экспорт
	...
	Если НЕ(КодЭлемента = "0" ИЛИ ПустаяСтрока(КодЭлемента)) Тогда
		омПроекты.УстановитьКодПДМДляПроекта(Параметры.КодСделки, КодЭлемента); 
	КонецЕсли;
...
Процедура ПослеЧтенияФайлаКомплект(Параметры) Экспорт
	...
	Если НЕ(КодКомплектаПДМ = "0" ИЛИ ПустаяСтрока(КодКомплектаПДМ)) Тогда
		...
		Попытка
			...
			КодКомплектаПДМЧисло = Число(КодКомплектаПДМ);
			
		Исключение
			КодКомплектаПДМЧисло = 0;
			
		КонецПопытки;
		
		Если КодКомплектаПДМЧисло = 0 Тогда
			
			Возврат;	
		
		КонецЕсли;
		
		омИзделиеКомплект.УстановитьКодПДМКодКомплекта(Параметры.КодКомплект, КодКомплектаПДМЧисло); 
		Оповестить("ИзменилсяКодКомплектаПДМ", Новый Структура("КодКомплект", КодКомплектаПДМЧисло));
	КонецЕсли;
...
Процедура ПослеЗабратьПрава(Параметры) Экспорт
	...
	омВыгрузкаCRMНаСевере.УстановитьСчетчикПоискаФайлаОтветаВыгрузкиPDM(0);
	Оповестить("ИзменилсяНаборПравPDM", Новый Структура("ИмяСправочника, КодЭлемента, ЗначениеСуфикса", Параметры.ИмяСправочника, Параметры.КодЭлемента, Параметры.ЗначениеСуфикса));
...
Функция ПолучитьПутьКВыгузке(ТипВыгрузки, стДанныеВыгрузки) Экспорт
	...
	ИначеЕсли ТипВыгрузки = "РаздатьПраваКомпозиции" Тогда
		ПутьКФайлуEXE = стДанныеВыгрузки.ПутьКФайлуEXEРаздатьПраваКомпозиции;
	ИначеЕсли ТипВыгрузки = "ЗабратьПрава" Тогда
		ПутьКФайлуEXE = стДанныеВыгрузки.ПутьКФайлуEXEЗабратьПрава;
	ИначеЕсли ТипВыгрузки = "ЗабратьПраваКомпозиции" Тогда
		ПутьКФайлуEXE = стДанныеВыгрузки.ПутьКФайлуEXEЗабратьПраваКомпозиции;
