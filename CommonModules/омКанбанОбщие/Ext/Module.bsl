...
Функция ПолучитьТайминКарточки(Знач УИДКарточка, УИДДоски) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	УИДКолонок = омКанбанДоскиНаСервереПовтор.ПолучитьУИДКолонокДоски(УИДДоски, Истина);
	Если ТипЗнч(УИДКарточка) = Тип("СправочникСсылка.ИзделияКомплект") Тогда
		УИДКарточка = омКанбанДоскиНаСервере.ПолучитьУИДКарточки(УИДДоски, УИДКарточка);		
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	КанбанПереходКарточки.НомерВКолонке КАК НомерВКолонке,
		|	КанбанПереходКарточки.Окончание КАК Окончание,
		|	КанбанПереходКарточки.Пользователь КАК Пользователь,
		|	КанбанПереходКарточки.УИДКудаДорожка КАК УИДКудаДорожка,
		|	КанбанПереходКарточки.УИДКудаКолонка КАК УИДКудаКолонка,
		|	КанбанДополнительныеДанныеПерехода.ТипДополнения КАК ТипДополнения,
		|	КанбанДополнительныеДанныеПерехода.Дополнение КАК Дополнение,
		|	КанбанДополнительныеДанныеПерехода.Дополнение.Наименование КАК ДополнениеНаименование,
		|	КанбанДополнительныеДанныеПерехода.Комментарий КАК Комментарий,
		|	КанбанКолонки.Наименование КАК НаименованиеКолонки,
		|	КанбанДорожки.Наименование КАК НаименованиеДорожки,
		|	КанбанКолонки.ВРаботе КАК ВРаботе,
		|	КанбанКолонки.Статус КАК Статус,
		|	СтадииКомплектаРеквизитыСтадии.Ссылка КАК Стадия,
		|	СтадииКомплектаРеквизитыСтадии.Ссылка.ПолноеНаименование КАК СтадияПолноеНаименование
		|ИЗ
		|	РегистрСведений.КанбанПереходКарточки КАК КанбанПереходКарточки
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КанбанДополнительныеДанныеПерехода КАК КанбанДополнительныеДанныеПерехода
		|		ПО КанбанПереходКарточки.УИДПерехода = КанбанДополнительныеДанныеПерехода.УИДПерехода
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КанбанДорожки КАК КанбанДорожки
		|		ПО КанбанПереходКарточки.УИДКудаДорожка = КанбанДорожки.УИД
		...
		|ГДЕ
		|	КанбанПереходКарточки.УИДКарточка = &УИДКарточка
		|	И КанбанПереходКарточки.УИДКудаКолонка В(&УИДКолонок)
		|
		|УПОРЯДОЧИТЬ ПО
		|	КанбанПереходКарточки.Период,
		|	УИДКудаКолонка";
	
	Запрос.УстановитьПараметр("УИДКарточка", 	УИДКарточка);
	Запрос.УстановитьПараметр("УИДКолонок", 	УИДКолонок);
	
	Возврат Новый Структура("УИДКарточка, ТаблицаДанных", УИДКарточка, Запрос.Выполнить().Выгрузить());
	
КонецФункции // ПолучитьТайминКарточки()

Функция ПредыдущиеСостояниеКанбана(УИДКарточка, УИДДоски) Экспорт
	...
	Запрос.Текст = 
		...
		|	И КанбанПереходКарточки.УИДКарточка = &УИДКарточка
		|
		|УПОРЯДОЧИТЬ ПО
		|	КанбанПереходКарточки.Период";
	...
	Если Выборка.Количество() = 2 Тогда
		Пока Выборка.Следующий() Цикл
			Возврат Выборка.УИДКудаКолонка;
		КонецЦикла;
	КонецЕсли;
	
	Возврат "";
	
КонецФункции // ПредыдущиеСостояниеКанбана()
