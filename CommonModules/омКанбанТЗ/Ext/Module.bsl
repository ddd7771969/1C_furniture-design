...
Функция ПолучитьHTMLКанбанТЗ(стПараметры = Неопределено, стНастройки = Неопределено) Экспорт
    ...
    Если стПараметры = Неопределено Тогда
		стПараметры = омКанбанОбщие.ПолучитьСтруктураКанбана("НаименованиеДоскиДляТЗ");		
	КонецЕсли; 
...
Функция ПолучитьТекущиеЗначениеКолонках(Сотрудник, стПараметры)
	...
	стКоличествоКарточекПоКолонкам = Новый Структура("ДоступноРедактирование", ЗначениеЗаполнено(Сотрудник));
...
Процедура ПолучитьКрайниеДаты(ДанныеКарточек)
	...
	Запрос.Текст = 
		...
		|	ДанныеКомплекта.ВнешняяДатаОкончаниеТЗ КАК КрайТЗ
		|ИЗ
...
Процедура УстановитьРазделительСотрудникТЗ(КомплектКомпозиция, Сотрудник) Экспорт

	УИДРесурс = ПолучитУИДРесурса(КомплектКомпозиция);
	
	МенеджерЗапись = РегистрыСведений.КанбанРазделители.СоздатьМенеджерЗаписи();
	МенеджерЗапись.УИДКарточка = УИДРесурс;
	МенеджерЗапись.Прочитать();
	
	МенеджерЗапись.УИДКарточка 	= УИДРесурс;
	МенеджерЗапись.Сотрудник 	= Сотрудник;
	
	МенеджерЗапись.Записать();
