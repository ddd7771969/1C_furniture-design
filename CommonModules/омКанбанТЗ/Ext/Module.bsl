...
Функция ПолучитьHTMLКанбанТЗ(стПараметры = Неопределено, стНастройки = Неопределено, Сотрудник) Экспорт
	...
	Возврат Новый Структура("КодHTML, мДанныеДляПоиска", СформироватьИнтерфейс(стДанныеДляВывода, стПараметры, стНастройки, ЗначениеЗаполнено(Сотрудник)), мДанныеДляПоиска);
	
КонецФункции // ПолучитьHTMLКанбанТЗ()

Функция СформироватьИнтерфейс(стДанныеДляВывода, стПараметры, стНастройки, ДоступноРедактирование)

	Шаблон = ПолучитьШаблонHTML();
	
	Kanban = СформироватьКанбан(стДанныеДляВывода, стПараметры, стНастройки, ДоступноРедактирование);
...
Функция СформироватьКанбан(стДанныеДляВывода, стПараметры, стНастройки, ДоступноРедактирование)
	...
	Для каждого ТекущаяКолонка Из стПараметры.мКолонки Цикл   //УИД, ИмяСобытия, ЛимитКарточек, ЛимитЧасов, Наименование, Цвет, ЗапрещенИнтерактивныйПереход, ЗапрещенИнтерактивныйУход
		...
		текШаблонЭлемента = ?(ТекущаяКолонка.ЗапрещенИнтерактивныйУход ИЛИ НЕ ДоступноРедактирование, ШаблонЭлементаБезПеремещения, ШаблонЭлемента);	
		...
		текШаблонКатегории = ?(ТекущаяКолонка.ЗапрещенИнтерактивныйПереход ИЛИ НЕ ДоступноРедактирование, ШаблонКатегорииБезПеремещения, ШаблонКатегории);	
