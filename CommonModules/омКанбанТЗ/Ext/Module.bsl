...
Функция СформироватьИнтерфейс(стДанныеДляВывода, стПараметры, стНастройки)
	...
	Kanban = СформироватьКанбан(стДанныеДляВывода, стПараметры);
...
Функция СформироватьКанбан(стДанныеДляВывода, стПараметры)
	...
	Для каждого ТекущаяКолонка Из стПараметры.мКолонки Цикл   //УИД, ИмяСобытия, ЛимитКарточек, ЛимитЧасов, Наименование, Цвет
		
		стОтбор.Колонка = ТекущаяКолонка.УИД;
		стДанныеКатегории = СформироватьЗадачи(стОтбор,  стДанныеДляВывода, стПараметры, ШаблонЭлемента);	
		Категории.Добавить(СтрШаблон(ШаблонКатегории, ТекущаяКолонка.УИД, ТекущаяКолонка.Наименование, стДанныеКатегории.СтрокаHTML 
...
Функция СформироватьЗадачи(стОтбор, стДанныеДляВывода, стПараметры, ШаблонЭлемента)
...
Функция ПолучитьСтруктураКанбана() 
	...
	СтруктураДоски = Новый Структура("УИДДоски, Наименование, НомерВерсии, ПутьКФайлуКартинкиКорзины, ПерваяЗагрузка", "", "", 0, "", Истина);
	...
	Пока Выборка.Следующий() Цикл  
		СтруктураКолонки = Новый Структура("УИД, ИмяСобытия, ЛимитКарточек, ЛимитЧасов, Наименование, Цвет", );
...
Функция ПолучитьШаблонЭлемента() Экспорт
	...
	ШаблонHTML = "
      |<div class=""item"" draggable=""true"" data-id=""%1"" data-track=""%2"" ondragstart=""onDragStart(event)"" ondragend=""onDragEnd(event)"" onclick=""onClickItem(event)"" ondblclick=""onDbClickItem(event)"">
