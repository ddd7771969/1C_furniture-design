...
Функция ПолучитьHTMLКанбанТЗ(стПараметры = Неопределено, стНастройки = Неопределено) Экспорт
	...
	Возврат Новый Структура("КодHTML, мДанныеДляПоиска"
			...
			, стПараметры
			, стНастройки
			, ПолучитьТекущиеЗначениеКолонках(?(стНастройки = Неопределено, Справочники.Сотрудники.ПустаяСсылка(), стНастройки.Сотрудник)))
	, мДанныеДляПоиска);
	
КонецФункции // ПолучитьHTMLКанбанТЗ() 

Функция ПолучитьТекущиеЗначениеКолонках(Сотрудник)
	
	стМаксПоКолонкам = Новый Структура("ДоступноРедактирование",ЗначениеЗаполнено(Сотрудник));

	Возврат стМаксПоКолонкам;

КонецФункции // ПолучитьМаксВКолонках()


Функция СформироватьИнтерфейс(стДанныеДляВывода, стПараметры, стНастройки, стМаксПоКолонкам)

	Шаблон = ПолучитьШаблонHTML();
	
	Kanban = СформироватьКанбан(стДанныеДляВывода, стПараметры, стНастройки, стМаксПоКолонкам);
...
Функция СформироватьКанбан(стДанныеДляВывода, стПараметры, стНастройки, стМаксПоКолонкам)
	...
	Для каждого ТекущаяКолонка Из стПараметры.мКолонки Цикл   //УИД, ИмяСобытия, ЛимитКарточек, ЛимитЧасов, Наименование, Цвет, ЗапрещенИнтерактивныйПереход, ЗапрещенИнтерактивныйУход
		...
		текШаблонЭлемента = ?(ТекущаяКолонка.ЗапрещенИнтерактивныйУход ИЛИ НЕ стМаксПоКолонкам.ДоступноРедактирование, ШаблонЭлементаБезПеремещения, ШаблонЭлемента);	
		...
		текШаблонКатегории = ?(ТекущаяКолонка.ЗапрещенИнтерактивныйПереход ИЛИ НЕ стМаксПоКолонкам.ДоступноРедактирование, ШаблонКатегорииБезПеремещения, ШаблонКатегории);	
		
		Если стДанныеКатегории.ВсегоЗадач >= ТекущаяКолонка.ЛимитКарточек И ТекущаяКолонка.ЛимитКарточек > 0 Тогда
		...
		Категории.Добавить(СтрШаблон(текШаблонКатегории, ТекущаяКолонка.УИД, ТекущаяКолонка.Наименование, стДанныеКатегории.СтрокаHTML 
						, "", "149, 18, 18" , стДанныеКатегории.ВсегоЗадач, ?(ТекущаяКолонка.ЛимитКарточек = 0, "", "Max " + ТекущаяКолонка.ЛимитКарточек)));
...
Функция ПолучитьДанныеДляВывода(стСтруктураКанбана, стНастройки)
	...
	Запрос.Текст = 
		...
		|				НЕ(УИДКудаДорожка = """"
		|						ИЛИ УИДКудаКолонка = """"
		|						ИЛИ УИДКудаКолонка = ""100"")) КАК КанбанПереходКарточкиСрезПоследних
	...
	Запрос.УстановитьПараметр("ИспользоватьПроект", ?(стНастройки.Проект.Количество() = 0, Ложь, Истина)); 
	...
	Если ЗначениеЗаполнено(стНастройки.Сотрудник) ИЛИ НЕ стНастройки.Проект = Неопределено Тогда
