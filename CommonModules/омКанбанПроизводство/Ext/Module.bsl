...
Функция ПолучитьHTMLПроизводство(стПараметры = Неопределено, стНастройки = Неопределено) Экспорт

    Если стПараметры = Неопределено Тогда
		стПараметры = омКанбанОбщие.ПолучитьСтруктураКанбана("НаименованиеДоскиПроизводство");		
	КонецЕсли; 
	...
	Если стНастройки = Неопределено Тогда
		...
		стНастройки.Вставить("Проект", 					Новый СписокЗначений);
	...
	Возврат Новый Структура("КодHTML, мДанныеДляПоиска"
			, СформироватьИнтерфейс(стДанныеДляВывода
			, стПараметры
			, стНастройки
			, ПолучитьТекущиеЗначениеКолонках(?(стНастройки = Неопределено, Справочники.Сотрудники.ПустаяСсылка(), стНастройки.Сотрудник), стПараметры))
	, мДанныеДляПоиска);
	
КонецФункции // ПолучитьHTMLКанбанТЗ() 

Функция ПолучитьТекущиеЗначениеКолонках(Сотрудник, стПараметры)
	
	стКоличествоКарточекПоКолонкам = Новый Структура("ДоступноРедактирование", Истина);
	
	Если ЗначениеЗаполнено(Сотрудник) Тогда
		
		УИДКудаКолонка = Новый Массив;
		...
		Для каждого ТекущаяКолонка Из стПараметры.мКолонки Цикл   //УИД, ИмяСобытия, ЛимитКарточек, ЛимитЧасов, Наименование, Цвет, ЗапрещенИнтерактивныйПереход, ЗапрещенИнтерактивныйУход
			УИДКудаКолонка.Добавить(ТекущаяКолонка.УИД);
		КонецЦикла;
		...
		Запрос.Текст = 
			...
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ КанбанРазделители.УИДКарточка) КАК КоличествоЗадач
			|ИЗ
			...
			|		ПО КанбанПереходКарточкиСрезПоследних.УИДКарточка = КанбанРазделители.УИДКарточка
			|ГДЕ
			|	КанбанПереходКарточкиСрезПоследних.УИДКудаКолонка В(&УИДКудаКолонка)
			|	И КанбанРазделители.Сотрудник = &Сотрудник
			|
			|СГРУППИРОВАТЬ ПО
			|	КанбанПереходКарточкиСрезПоследних.УИДКудаКолонка";
		
		Запрос.УстановитьПараметр("Сотрудник", Сотрудник);
		Запрос.УстановитьПараметр("УИДКудаКолонка", УИДКудаКолонка);
		
		стКоличествоКарточекПоКолонкам.Вставить("КоличествоКарточекПоКолонкам", Запрос.Выполнить().Выгрузить());
		
	Иначе
		
		стКоличествоКарточекПоКолонкам.Вставить("КоличествоКарточекПоКолонкам", Новый ТаблицаЗначений);
		
	КонецЕсли;
	
	Возврат стКоличествоКарточекПоКолонкам;
	
КонецФункции // ПолучитьМаксВКолонках()

Функция СформироватьИнтерфейс(стДанныеДляВывода, стПараметры, стНастройки, стКоличествоКарточекПоКолонкам)

	Шаблон = ПолучитьШаблонHTML();
	
	Kanban = СформироватьКанбан(стДанныеДляВывода, стПараметры, стНастройки, стКоличествоКарточекПоКолонкам);
	...
	css_item 				= ПолучитьМакет_css_item(стДанныеДляВывода);
	JS 						= ПолучитьМакет_js();
	...
	css_item 				= СтрЗаменить(css_item, 	"#16#", 	РазмерШрифта);
...
Функция СформироватьКанбан(стДанныеДляВывода, стПараметры, стНастройки, стКоличествоКарточекПоКолонкам)
	...
	КоличествоКарточекПоКолонкам	= стКоличествоКарточекПоКолонкам.КоличествоКарточекПоКолонкам;
	
	Для каждого ТекущаяКолонка Из стПараметры.мКолонки Цикл   //УИД, ИмяСобытия, ЛимитКарточек, ЛимитЧасов, Наименование, Цвет, ЗапрещенИнтерактивныйПереход, ЗапрещенИнтерактивныйУход
		
		текШаблонЭлемента = ?(ТекущаяКолонка.ЗапрещенИнтерактивныйУход ИЛИ НЕ стКоличествоКарточекПоКолонкам.ДоступноРедактирование, ШаблонЭлементаБезПеремещения, ШаблонЭлемента);	
		...
		текШаблонКатегории = ?(ТекущаяКолонка.ЗапрещенИнтерактивныйПереход ИЛИ НЕ стКоличествоКарточекПоКолонкам.ДоступноРедактирование, ШаблонКатегорииБезПеремещения, ШаблонКатегории);	
		
		Если КоличествоКарточекПоКолонкам.Количество() = 0 Тогда
			
			КоличествоЗадач = стДанныеКатегории.ВсегоЗадач;
			
		Иначе
			
			НайденаяСтрока = КоличествоКарточекПоКолонкам.Найти(ТекущаяКолонка.УИД, "УИДКудаКолонка");
			
			Если НайденаяСтрока = Неопределено Тогда
				КоличествоЗадач = стДанныеКатегории.ВсегоЗадач;
			Иначе	
				КоличествоЗадач = НайденаяСтрока.КоличествоЗадач;
			КонецЕсли;
			
		КонецЕсли;
		
		
		Если КоличествоЗадач >= ТекущаяКолонка.ЛимитКарточек И ТекущаяКолонка.ЛимитКарточек > 0 Тогда
			текШаблонКатегории = ШаблонКатегорииБезПеремещения;	
		КонецЕсли;
		...
		Категории.Добавить(СтрШаблон(текШаблонКатегории, ТекущаяКолонка.УИД, ТекущаяКолонка.Наименование, стДанныеКатегории.СтрокаHTML 
						, "", "149, 18, 18", КоличествоЗадач, ?(ТекущаяКолонка.ЛимитКарточек = 0, "", "Max " + ТекущаяКолонка.ЛимитКарточек)));
		
	КонецЦикла;
	
	Если РольДоступна("ПолныеПрава") И НЕ ПустаяСтрока(стПараметры.СтруктураДоски.ПутьКФайлуКартинкиКорзины) Тогда
...
Функция СформироватьЗадачи(стОтбор, стДанныеДляВывода, стПараметры, ШаблонЭлемента, стНастройки)
	...
	стОтборТега			= Новый Структура("УИД", );
	ВсегоЗадач			= 0;
	tabindex = 1;
	...
	Для каждого ТекущаяЯчейка Из тзДанныеДляВывода.НайтиСтроки(стОтбор) Цикл
		...
		стОтборТега.УИД = ТекущаяЯчейка.УИД;
		
		НайденыеСтрокиТег = тзДанныеТегов.НайтиСтроки(стОтборТега);
		
		Если НайденыеСтрокиТег.Количество() = 0 Тогда
		
			СтрокаТегов = "";	
		
		Иначе
		
			СтрокаТегов = "<table><tr>";
			
			Для каждого ТекущийТег Из НайденыеСтрокиТег Цикл
				СтрокаТегов = СтрокаТегов + "<td class=tag_number_" + ТекущийТег.КодТег + ">" 
						+ ?(ПустаяСтрока(ТекущийТег.ТегТекст), "&nbsp;&nbsp;", ТекущийТег.ТегТекст) + "</td>";
			КонецЦикла;
			
			СтрокаТегов = СтрокаТегов + "</tr></table>";
		
		КонецЕсли;
		
		ЗадачиКатегории.Добавить(СтрШаблон(ШаблонЭлемента
			, ТекущаяЯчейка.УИД, ТекущаяЯчейка.Дорожка, ТекущаяЯчейка.Наименование, Формат(ТекущаяЯчейка.КрайняяДата, "ДФ=dd.MM.yy")
			, tabindex, СтрокаТегов));
...
Функция ПолучитьДанныеДляВывода(стСтруктураКанбана, стНастройки)
	...
	Запрос.Текст = 
		...
		|	И ВЫБОР
		|			КОГДА &ИспользоватьПроект
		|				ТОГДА КанбанРазделители.Проект В (&Проект)
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|	И ВЫБОР
		...
		|			КОГДА &Отбор = """"
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ КанбанКарточки.Ресурс.Наименование = &Отбор
		|		КОНЕЦ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|ГДЕ
		|	НЕ(КанбанПереходКарточкиСрезПоследних.УИДКудаДорожка = """"
		|				ИЛИ КанбанПереходКарточкиСрезПоследних.УИДКудаКолонка = ""1""
		|				ИЛИ КанбанПереходКарточкиСрезПоследних.УИДКудаКолонка = ""100"")
		|
		|УПОРЯДОЧИТЬ ПО
		...
		|	КанбанТеги.Тег.Код КАК КодТег,
		|	КанбанТеги.Тег.ЦветТекста КАК ТегЦветТекста,
		|	КанбанТеги.Тег.ЦветФона КАК ТегЦветФона,
		|	КанбанТеги.Тег.Текст КАК ТегТекст
		|ИЗ
	...
	ИспользоватьПроект = ?(стНастройки.Проект.Количество() = 0, Ложь, Истина);
	...
	Запрос.УстановитьПараметр("ИспользоватьПроект", ИспользоватьПроект); 
	Запрос.УстановитьПараметр("Проект", 			стНастройки.Проект); 
	Запрос.УстановитьПараметр("Отбор", 				стНастройки.Отбор); 
	...
	Если ЗначениеЗаполнено(стНастройки.Сотрудник) ИЛИ ИспользоватьПроект Тогда
	...
	тзДанныеТегов = Результат[3].Выгрузить();
	тзДанныеТегов.Индексы.Добавить("УИД");
	
	Возврат Новый Структура("ДанныеКарточек, ДанныеТегов", ДанныеКарточек, тзДанныеТегов);
...
Процедура ПолучитьКрайниеДаты(ДанныеКарточек)
	...
	Запрос.Текст = 
		...
		|	ДанныеКомплекта.ВнешняяДатаНачалоПроизводства КАК ВнешняяДатаОкончаниеПроизводства
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ НайденаяСтрока = Неопределено Тогда
			НайденаяСтрока.КрайняяДата = Выборка.ВнешняяДатаОкончаниеПроизводства;	
		КонецЕсли;
...
Функция ПереместитьРеквизитВОжидание(Реквизит, Комментарий = "") Экспорт
	...
	УИДДоски 	= омКанбанДоскиНаСервереПовтор.ПолучитьУИДДоски("УИДДоскиПроизводство");
	УИДКудаКолонка 	= омКанбанДоскиНаСервереПовтор.ПолучитьУИДКолонки(УИДДоски, "_Ожидание_Производства");
	
	УИДКарточки = ПереместитьРеквизитВПереопределенное(Реквизит, УИДКудаКолонка, Комментарий);
	
КонецФункции // переместитьРеквизитВОжидание()

Функция ПереместитьРеквизитВПереопределенное(Реквизит, УИДКудаКолонка, Комментарий = "") Экспорт
...
Функция СоздатьКомментарий(УИДКарточка, Комментарий) Экспорт 
	
	МенеджерЗаписи = РегистрыСведений.КанбанКарточки.СоздатьМенеджерЗаписи();
    МенеджерЗаписи.УИД = УИДКарточка;
    ...
    ТегКомментарий = омТегиПовтор.ПолучитьТегКомментарий();
	
	Если ЗначениеЗаполнено(ТегКомментарий) Тогда
		...
		Запрос.УстановитьПараметр("УИД", УИДКарточка);
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.Тег = ТегКомментарий Тогда
				ТребуетсяЗапись = Ложь;
				Прервать;
			КонецЕсли;
			
			НомерТега = Макс(НомерТега, Выборка.НомерТега);
			
		КонецЦикла;
		
		Если ТребуетсяЗапись Тогда
			
			МенеджерЗаписи = РегистрыСведений.КанбанТеги.СоздатьМенеджерЗаписи();
			МенеджерЗаписи.УИД 			= УИДКарточка;
			МенеджерЗаписи.НомерТега 	= НомерТега;
			МенеджерЗаписи.Тег 			= ТегКомментарий;
...
КонецФункции // СоздатьКомментарий(УИДКарточка, Комментарий)
...
Функция ПеревестиРеквизитВСтоп(Реквизит, Комментарий = "", УИДКарточка = "") Экспорт
	
	УИДДоски 	= омКанбанДоскиНаСервереПовтор.ПолучитьУИДДоски("УИДДоскиПроизводство");
    омКанбанДоскиНаСервере.ПеревестиКарточкуВСтоп(УИДДоски, Реквизит, Комментарий, УИДКарточка); 

КонецФункции // ПеревестиРеквизитВЗавершенный()

Функция ПолучитьКартинкуКорзины(ПутьККорзине) Экспорт

	Возврат "<div class=""category"" data-category-id=""100"" ondragover=""event.preventDefault()"" ondrop=""onDrop(event)"">
...
Функция ПолучитьУИДКолонокПороизводства(ДобавитьСлужебныеКолонки = Ложь) Экспорт

	Возврат омКанбанДоскиНаСервереПовтор.ПолучитьУИДКолонокДоски(омКанбанДоскиНаСервереПовтор.ПолучитьУИДДоски("УИДДоскиПроизводство"), ДобавитьСлужебныеКолонки);	

КонецФункции // ПолучитьКолонкиПороизводства()

Функция РесурсВПроизводстве(Ресурс, ОписанияКолонки = "") Экспорт
	
	ТекущаяКолонкаУИД = ПолучитьТекущуюКолонкуРесурса(Ресурс);
	
	Если ПустаяСтрока(ТекущаяКолонкаУИД) Тогда
	
		ОписанияКолонки = "Не было в производстве";
		Возврат Ложь;
		
	КонецЕсли;
	
	Если ТекущаяКолонкаУИД = омКанбанДоскиНаСервереПовтор.ПолучитьУИДКолонки(, "_Приостановка_Производства") Тогда
		ОписанияКолонки = "В приостановке производства";
		Возврат Ложь;
	КонецЕсли;
	
	НайденоеЗначение = 	омКанбанДоскиНаСервереПовтор.ПолучитьСписокСлужебныхКолонок().НайтиПоЗначению(ТекущаяКолонкаУИД);

	Если НайденоеЗначение = Неопределено Тогда
		ОписанияКолонки = "В производстве";
		Возврат Истина;
	КонецЕсли;
	
	ОписанияКолонки = НайденоеЗначение.Представление;
	Возврат Ложь;
	
КонецФункции // РесурсВПроизводстве()

Процедура ПеревестиКарточкуВПриостановку(Ресурс, Комментарий = "", УИДКарточка = "", УИДКудаКолонка = "") Экспорт

	УИДДоски = омКанбанДоскиНаСервереПовтор.ПолучитьУИДДоски("УИДДоскиПроизводство");

	Если ПустаяСтрока(УИДКарточка) Тогда
		
		УИДКарточка = омКанбанДоскиНаСервере.ПолучитьУИДКарточки(УИДДоски, Ресурс); 
	...
	Если РесурсВОжидании(УИДКарточка) Тогда
		
		омКанбанДоскиНаСервере.ПеревестиКарточкуВВыполнено(УИДДоски, УИДКарточка, "Приостановка"); 
		
	Иначе
		
		стПараметрыПерехода = омКанбанНачальноеЗаполнение.ПолучитьСтруктуруДляЭлементовКанбана("ПереходКарточки", Комментарий, УИДКудаКолонка);
		
		стПараметрыПерехода.Доска = УИДДоски;

		Если ПустаяСтрока(УИДКудаКолонка) Тогда
			стПараметрыПерехода.УИДКудаКолонка 	= омКанбанДоскиНаСервереПовтор.ПолучитьУИДКолонки(УИДДоски, "_Приостановка_Производства");
		КонецЕсли;
...
Функция РесурсВОжидании(Ресурс) Экспорт
	
	ТекущаяКолонкаУИД = ПолучитьТекущуюКолонкуРесурса(Ресурс);
	
	Возврат ТекущаяКолонкаУИД = омКанбанДоскиНаСервереПовтор.ПолучитьУИДКолонки(, "_Ожидание_Производства");
	
КонецФункции // РесурсВОжидании()

Функция ПолучитьТекущуюКолонкуРесурса(Ресурс) Экспорт

	УИДДоски = омКанбанДоскиНаСервереПовтор.ПолучитьУИДДоски("УИДДоскиПроизводство");
	
	Запрос = Новый Запрос;
	Если ТипЗнч(Ресурс) = Тип("Строка") Тогда    //УИД карточки
		...
		|	И КанбанПереходКарточкиСрезПоследних.УИДКарточка = &Ресурс";
	...
	Иначе
		...
		|	И КанбанКолонки.Доска = &УИДДоски";
	...
	Пока Выборка.Следующий() Цикл
		ТекущаяКолонкаУИД = Выборка.УИДКудаКолонка;
	КонецЦикла;
	
	Возврат ТекущаяКолонкаУИД; 

КонецФункции // ПолучитьТекущуюКолнкуРесураса()
...
Функция ПолучитьШаблонКатегории() Экспорт
    ...
    |    <h3>%2</h3>
    |    <p class=""total-sum"">Всего задач: %6. %7</p>
    |  </div>
...
Функция ПолучитьШаблонЭлементаБезПеремещения()
	...
	Возврат СтрЗаменить(ПолучитьШаблонЭлемента(), "onDragStart", "offDragStart");
...
Функция ПолучитьМакет_css_item(стДанныеДляВывода)
	...
	Для каждого ПараметрТега Из стДанныеДляВывода.ДанныеТегов Цикл
	
		СтрокаСтильТегов = СтрокаСтильТегов + "
		|	.tag_number_" + ПараметрТега.КодТег + "{
		|      background: " + омЦветНаСервере.ПолучитьВебПредставленияЦвета(ПараметрТега.ТегЦветФона) + ";
		|      color: " + омЦветНаСервере.ПолучитьВебПредставленияЦвета(ПараметрТега.ТегЦветТекста) + ";
		|      font-size: #16#px;
		|}";
