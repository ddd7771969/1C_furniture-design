...
//
Функция СоздатьПисьмо(Сообщение1, УчетнаяЗапись, ОтправитьСразу = Истина) Экспорт
	
	РезультатОтправкиПисьма = РезультатОтправкиПисьма();
	
	ПисьмоHTML = (Сообщение1.ДополнительныеПараметры.ФорматПисьма = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML);
	...
	Попытка
		...
		Если Сообщение1.Важность = ВажностьИнтернетПочтовогоСообщения.Высокая Тогда
			Письмо.Важность = Перечисления.ВариантыВажностиВзаимодействия.Высокая;
		ИначеЕсли Сообщение1.Важность = ВажностьИнтернетПочтовогоСообщения.Низкая Тогда
		...
		Если ПисьмоHTML Тогда
			...
			Письмо.ТекстHTML = Сообщение1.Текст;
			Письмо.Текст     = ПолучитьОбычныйТекстИзHTML(Сообщение1.Текст);
			
		Иначе
			
			Письмо.Текст = Сообщение1.Текст;
			
		КонецЕсли;
		
		Письмо.Тема = Сообщение1.Тема;
		...
		НастройкиПользователя = ПолучитьПараметрыРаботыПользователяДляИсходящегоЭлектронногоПисьма(
		                           УчетнаяЗапись, Сообщение1.ДополнительныеПараметры.ФорматПисьма, Истина);
		ЗаполнитьЗначенияСвойств(Письмо, НастройкиПользователя);    
		
		Письмо.УведомитьОДоставке  = Макс(Сообщение1.ДополнительныеПараметры.УведомитьОДоставке, Письмо.УведомитьОДоставке);   
		Письмо.УведомитьОПрочтении = Макс(Сообщение1.ДополнительныеПараметры.УведомитьОПрочтении, Письмо.УведомитьОПрочтении);

		
		Письмо.УдалятьПослеОтправки = Ложь;
		
		Письмо.Комментарий = Сообщение1.ДополнительныеПараметры.Комментарий;
		
		Для Каждого ПолучательПисьма Из Сообщение1.Получатели Цикл
		...
		Для Каждого ПолучательОтвета Из Сообщение1.ПолучателиОтвета Цикл
		...
		Для Каждого ПолучательСкрытыхКопий Из Сообщение1.ПолучателиСкрытыхКопий Цикл
		...
		Письмо.ЕстьВложения = (Сообщение1.Вложения.Количество() > 0);
		РазмерВложений  = 0;
		РазмерыВложений = Новый Соответствие;
		Для Каждого Вложение Из Сообщение1.Вложения Цикл
		...
		Если ЗначениеЗаполнено(Сообщение1.ДополнительныеПараметры.Предмет) Тогда
			Предмет = Сообщение1.ДополнительныеПараметры.Предмет;
		Иначе
...
Функция ПараметрыПисьма() Экспорт
	...
	Сообщение1.Вставить("Тема", "");
	Сообщение1.Вставить("Текст", "");
	Сообщение1.Вставить("Важность", ВажностьИнтернетПочтовогоСообщения.Обычная);
	Сообщение1.Вставить("СообщенияПользователю", Новый ФиксированныйМассив(Новый Массив));
	...
	Сообщение1.Вставить("Получатели", Получатели);
	...
	Сообщение1.Вставить("ПолучателиОтвета", ПолучателиОтвета); 
	...
	Сообщение1.Вставить("ПолучателиСкрытыхКопий", ПолучателиСкрытыхКопий);
	...
	Сообщение1.Вставить("Вложения", Вложения);
	...
	Сообщение1.Вставить("ДополнительныеПараметры", ДополнительныеПараметры);
...
Функция ДанныеПрисоединенныхФайловПисьмаОбъект(ПисьмоОбъект) Экспорт
