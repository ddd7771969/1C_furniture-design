...
Функция ВыгрузитьВPDM(ЭлементВыгрузки, ТипВыгрузки = "Комплект", ПараметрыВыгрузки = Неопределено) Экспорт       
	...
	ТекстДок = СоздатьФайлВыгрузки(стДанныеЭлемента.мДанныеЭлемента[0], ТипВыгрузки);
	
	ЗапуститьИсполнение(ТекстДок, стВозврата, ТестКаталог, ПутьКФайлуEXE, НаименованиеЭлемента, ТипВыгрузки, ЭлементВыгрузки);
	
	Если КоличествоЭлементов > 1 И (ТипВыгрузки = "РаздатьПрава" ИЛИ ТипВыгрузки = "ЗабратьПрава") Тогда
		...
		Для х = 1 По КоличествоЭлементов - 1 Цикл
			...
			ТекстДок = СоздатьФайлВыгрузки(стДанныеЭлемента.мДанныеЭлемента[х], ТипВыгрузки);
			ЗапуститьИсполнение(ТекстДок, стВозврата, ТестКаталог, ПутьКФайлуEXE, НаименованиеЭлемента, ТипВыгрузки, ЭлементВыгрузки);	
...
Функция СоздатьФайлВыгрузки(мДанныеЭлемента, ТипВыгрузки)
	...
	ИначеЕсли ТипВыгрузки = "РаздатьПрава" ИЛИ ТипВыгрузки = "ЗабратьПрава" Тогда
		...
		ДанныеСтруктура = Тип("Структура") = ТипЗнч(мДанныеЭлемента);
		
		Если ДанныеСтруктура Тогда
			
			
			стрТекст = стрТекст + Символы.ВК 
			+ мДанныеЭлемента["ID_PDM"] + ";" 
			+ мДанныеЭлемента["UsrName"];
