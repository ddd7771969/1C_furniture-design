...
Функция ЗаписатПереход(Данные, УИДДоски, стДополнительныеДанные = Неопределено) Экспорт
	
	УИДКарточка 	= Данные.Получить("itemId");
	
	РезультатЗаписи = Новый Структура("ТребуетсяКомментарий, УИДПерехода", Ложь, "");
	...
	Запрос.Текст = 
		...
		|	КанбанКолонки.ТребуетсяКомментарий КАК ТребуетсяКомментарий
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ПустаяСтрока(Выборка.ДействиеПриПопадании) Тогда
		...
		РезультатЗаписи.ТребуетсяКомментарий	= Выборка.ТребуетсяКомментарий;
	...
	РезультатЗаписи.УИДПерехода = МенеджерЗаписиПереход.УИДПерехода;
	Возврат РезультатЗаписи;
	
КонецФункции // ЗаписатПереход()
...
Процедура СоздатьЗадачуКонтрольЗадачи(УИДКарточки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗадачаПринятьТЗ.Комплекты.Очистить();
		
		Если ТипЗнч(ЗадачаПринятьТЗ.ОбъектТЗ) = Тип("СправочникСсылка.Композиции") Тогда
		
			ЗадачаПринятьТЗ.Комплекты.Загрузить(ПолучитьКомплектыКомпозиции(ЗадачаПринятьТЗ.ОбъектТЗ));
...
Функция ПолучитьКомплектыКомпозиции(Композиция) Экспорт
