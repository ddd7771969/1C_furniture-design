...
Процедура ПеремещениеВОднойКолонкеИДорожке(УИДКудаКолонка, УИДКудаДорожка, новНомерВКолонке, текНомерВКолонке, УИДКарточка)
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.КанбанПереходКарточки.СрезПоследних(, УИДКудаКолонка = &УИДКудаКолонка) КАК КанбанПереходКарточкиСрезПоследних
		|ГДЕ
...
Процедура ПриПеремещенииВКолонкуПодтверждения(НомерВКолонке,УИДКудаКолонка,УИДКудаДорожка,УИДКарточки) Экспорт
	
	Если ЗначениеЗаполнено(УИДКарточки) Тогда
	
		СоздатьЗадачуКонтрольЗадачи(УИДКарточки);	
...
Процедура СоздатьЗадачуКонтрольЗадачи(УИДКарточки)
