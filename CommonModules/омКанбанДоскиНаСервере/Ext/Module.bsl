...
Процедура ПеревестиКарточкуВПриостановку(УИДДоски, Ресурс, Комментарий = "", УИДКарточка = "", УИДКудаКолонка = "") Экспорт
...
Функция ПолучитьУИДКарточки(УИДДоски = "", Ресурс) Экспорт
	...
	Запрос.Текст = 
		...
		|			КОГДА &Доска = """"
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ КанбанКарточки.Доска = &Доска
		|		КОНЕЦ
...
Функция ПолучитьКарточкиКанбанаСКолонкамиИТегами(УИДДоски, МассивРесурсов, МассивТегов = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	КанбанКарточки.Ресурс КАК Ресурс,
		|	ЕСТЬNULL(КанбанПереходКарточкиСрезПоследних.УИДКудаКолонка, """") КАК УИДКудаКолонка,
		|	ЕСТЬNULL(КанбанТеги.Тег, ЗНАЧЕНИЕ(Справочник.Теги.ПустаяСсылка)) КАК Тег
		|ИЗ
		|	РегистрСведений.КанбанКарточки КАК КанбанКарточки
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КанбанТеги КАК КанбанТеги
		|		ПО КанбанКарточки.УИД = КанбанТеги.УИД
		|ГДЕ
		|	КанбанКарточки.Доска = &Доска
		|	И КанбанКарточки.Ресурс В(&Ресурс)
		|	И ВЫБОР
		|			КОГДА &ИспользоватьМассивТегов
		|				ТОГДА КанбанТеги.Тег В (&МассивТегов)
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ";
	
	Запрос.УстановитьПараметр("Доска", 	УИДДоски);
	Запрос.УстановитьПараметр("Ресурс", МассивРесурсов);
	Запрос.УстановитьПараметр("МассивТегов", МассивТегов);
	Запрос.УстановитьПараметр("ИспользоватьМассивТегов", НЕ МассивТегов = Неопределено);
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции // ПолучитьКарточкиКанбанаСКолонками(УИДДоски, МассивРесурсов)
...
Функция ПолучитьКомплектыНаСтопе(УИДДоски) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	КанбанПереходКарточкиСрезПоследних.УИДКудаКолонка = ""200""
...
Процедура ЗаписатьТег(Тег, УИДКарточка) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	КанбанКарточки.Доска КАК УИДДоски
	               |ИЗ
	               |	РегистрСведений.КанбанТеги КАК КанбанТеги
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.КанбанКарточки КАК КанбанКарточки
	               |		ПО КанбанТеги.УИД = КанбанКарточки.УИД
	               |ГДЕ
	               |	КанбанТеги.УИД = &УИД";
	...
	Если ТребуетсяЗапись Тогда
		...
		МенеджерЗаписи.НомерТега 	= НомерТега + 1;
		...
		Запрос.Текст = 
			...
			|	РегистрСведений.КанбанКарточки КАК КанбанКарточки
			|ГДЕ
			|	КанбанКарточки.УИД = &УИДКарточка";
		...
		Пока Выборка.Следующий() Цикл
			ИнкриментВерсии(Выборка.УИДДоски);
		КонецЦикла;
		
	КонецЕсли;
	
КонецПроцедуры //ЗаписатьТег(Тег, УИДКарточка) 

Функция УдалитьТег(Тег, УИДКарточка) Экспорт
	...
	Пока Выборка.Следующий() Цикл
        ...
        МенеджерЗаписи = РегистрыСведений.КанбанТеги.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.УИД 			= УИДКарточка;
		МенеджерЗаписи.НомерТега 	= Выборка.НомерТега;
...
Функция ПроверитьНаличиеТега(РесурсИлиУИДКарточка, Тег, УИДДоски = "") Экспорт
	
	Если ТипЗнч(РесурсИлиУИДКарточка) = Тип("Строка") Тогда
		УИДКарточка = РесурсИлиУИДКарточка;
	Иначе
		УИДКарточка = ПолучитьУИДКарточки(УИДДоски, РесурсИлиУИДКарточка);
	КонецЕсли;
	...
	Возврат НЕ Запрос.Выполнить().Выбрать().Количество() = 0;
	
КонецФункции // ПроверитьНаличиеТега()

Функция МассивСПредопределеннымиКолонками() Экспорт

	МассивСПредопределеннымиКолонками = Новый Массив(3);
	МассивСПредопределеннымиКолонками[0] = "1";
	МассивСПредопределеннымиКолонками[1] = "100";
	МассивСПредопределеннымиКолонками[2] = "200";
	
    Возврат МассивСПредопределеннымиКолонками;
	
КонецФункции // МассивСПредопределеннымиКолонками()
...
Процедура СоздатьЗадачуКонтрольЗадачи(УИДКарточки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		Если ЗначениеЗаполнено(Выборка.ЗадачаПринятьТЗ) Тогда
			
			ЗадачаПринятьТЗ 			= Выборка.ЗадачаПринятьТЗ.ПолучитьОбъект();
			ЗадачаПринятьТЗ.Выполнена 	= Ложь;	
