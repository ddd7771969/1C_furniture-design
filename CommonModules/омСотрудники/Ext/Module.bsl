...
КонецФункции // ЗаполнитьТаблицуРабочееВремя()
 ...
 Функция ЗаполнитьСписокСотрудниковПоДолжностям(ТолькоРаботающие) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеСотрудника = Новый Структура("Сотрудник, Наименование, Фамилия", );
		ЗаполнитьЗначенияСвойств(ДанныеСотрудника, Выборка); 
		
		Если НЕ ПустаяСтрока(Выборка.Имя) Тогда
			ДанныеСотрудника.Фамилия = ДанныеСотрудника.Фамилия + " " + Лев(СокрЛ(Выборка.Имя), 1) + "."; 	
		КонецЕсли;
		
		Если НЕ ПустаяСтрока(Выборка.Отчество) Тогда
			ДанныеСотрудника.Фамилия = ДанныеСотрудника.Фамилия + " " + Лев(СокрЛ(Выборка.Отчество), 1) + "."; 	
		КонецЕсли;
