			...
			Если ИменаПолейССуффиксом.Свойство("Серия") Тогда
				...
				Если ТипЗнч(ИмяПоляСерия) = Тип("Массив") Тогда
					Для Каждого Имя Из ИмяПоляСерия Цикл
						МассивНепроверяемыхРеквизитов.Добавить(ПараметрыУказанияСерий.ИмяТЧТовары + "." + Имя);
					КонецЦикла;
				Иначе
					МассивНепроверяемыхРеквизитов.Добавить(ПараметрыУказанияСерий.ИмяТЧТовары + "." + ИмяПоляСерия);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ПараметрыУказанияСерий.ИмяТЧТовары = ПараметрыУказанияСерий.ИмяТЧСерии Тогда
		...
		Если ПараметрыПроверки.ВыдаватьСообщения Тогда
			...
			ПредставлениеТЧ = ?(ПустаяСтрока(ПараметрыПроверки.ПредставлениеТЧ),
								МетаданныеДокумента.ТабличныеЧасти[ПараметрыУказанияСерий.ИмяТЧТовары].Синоним,
								ПараметрыПроверки.ПредставлениеТЧ);
			...
			Пока Выборка.Следующий() Цикл
				Если Выборка.СерияНеЗаполнена Тогда
					...
					ИмяРеквизитаСерия = ?(ПустаяСтрока(ПараметрыПроверки.ИмяРеквизитаСерия),
											МетаданныеДокумента.ТабличныеЧасти[ПараметрыУказанияСерий.ИмяТЧТовары].Реквизиты[Выборка.ИмяПоляСерия].Синоним,
				...
				Если Выборка.ОшибкаУпаковки Тогда
					ТекстСообщения = НСтр("ru = 'Политика учета серий товара ""%НоменклатураПредставление%"" предусматривает, что серии указываются для каждого экземпляра. Укажите единичную упаковку в строке %НомерСтроки% списка ""%ПредставлениеТЧ%"".'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НоменклатураПредставление%",	Выборка.НоменклатураПредставление);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%",				Строка(Выборка.НомерСтроки));
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредставлениеТЧ%",			ПредставлениеТЧ);
...
// 
// Возвращаемое значение:
//  Булево - Округление количества выполнено корректно
Функция ОкруглениеКоличестваВыполненоКорректно(Объект, ПараметрыПроверки = Неопределено) Экспорт
