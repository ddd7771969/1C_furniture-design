...
Функция Комплектующие(Параметры, ДополнительныеПараметры) Экспорт
	...
	Если РаспределениеЗапасов.ЭтоПроизводительныйРежим() Тогда
		ТекстЗапроса =
			...
			|				ТОГДА
			|					ДоступностьТоваров.ВНаличииОстаток - ДоступностьТоваров.РезервироватьНаСкладеОстаток
			|			ИНАЧЕ
			|				ВЫБОР
			|					КОГДА
			|						ДоступностьТоваров.ВНаличииОстаток
			...
			|		И Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) КАК ДоступностьТоваров
			|ГДЕ
			|	&ФильтрСклады
			|ИНДЕКСИРОВАТЬ ПО
	...
	Иначе
		ТекстЗапроса =
			...
			|		И (ДоступностьТоваров.Номенклатура, ДоступностьТоваров.Характеристика) В(
			|			ВЫБРАТЬ
			|				ВариантыКомплектацииНоменклатурыТовары.Номенклатура КАК Номенклатура,
			|				ВариантыКомплектацииНоменклатурыТовары.Характеристика КАК Характеристика
			|			ИЗ
			...
			|	ДоступностьТоваров.Характеристика
			|ИНДЕКСИРОВАТЬ ПО
			|	Номенклатура, Характеристика";
	КонецЕсли;
