...
Функция УстановкаПараметровСеанса(ИменаПараметровСеанса) Экспорт
	...
	ПараметрыСеанса.СчетчикПоискаФайлаОтветаВыгрузкиPDM = 0;
...
Процедура ПередЗапускомПрограммы()
	...
	ПоддерживаемаяВерсияПлатформы = "8.3.22";
	
	Если ТекущийРежим = Режимы.НеИспользовать Тогда
		ПоддерживаемыеВерсииПлатформы = СтрРазделить("8.3.22", ", ", Ложь);
	...
	Иначе
		...
		Если СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_22") Тогда
