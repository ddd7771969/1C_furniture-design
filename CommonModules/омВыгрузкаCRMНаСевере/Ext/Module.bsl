...
Функция ПолучитьНаименованиеНастроек() Экспорт 
	...
	мНаименованиеНастроек = Новый Массив(8);
...
Функция ПолучитьДанныеЭлемента(ЭлементВыгрузки, ТипВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	ИначеЕсли ТипВыгрузки = "Комплект" Тогда
		...
		|ГДЕ
		|	ИзделияКомплект.Ссылка = &ИзделиеКомплект";
		
		Запрос.УстановитьПараметр("ИзделиеКомплект", ЭлементВыгрузки);
	...
	ИначеЕсли ТипВыгрузки = "ЗагрузкаТЗ" Тогда
		...
		НаименованиеЭлемента = "ProjectTask";
			
	КонецЕсли; 
		
	Возврат Новый Структура("НаименованиеЭлемента, мДанныеЭлемента", НаименованиеЭлемента, ОбщегоНазначения.ТаблицаЗначенийВМассив(тзДанные));
