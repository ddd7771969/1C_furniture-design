...
Функция ПолучитьДанныеЭлемента(ЭлементВыгрузки, ТипВыгрузки, ПараметрыВыгрузки = Неопределено) Экспорт
	...
	Если ТипВыгрузки = "Проект" Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			НС.НомерПроекта 		= Формат(Выборка.НомерПроекта, "ЧЦ=2; ЧВН=; ЧГ=0");
	...
	ИначеЕсли ТипВыгрузки = "Комплект" Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			НомерПроекта 			= Формат(Выборка.НомерПроекта, "ЧЦ=2; ЧВН=; ЧГ=0");
	...
	ИначеЕсли ТипВыгрузки = "РаздатьПрава" ИЛИ ТипВыгрузки = "ЗабратьПрава" 
		...
		Если ТипЗнч(ОбъектТЗ) = Тип("СправочникСсылка.Композиции") Тогда
			...
			Если ТипВыгрузки = "ЗабратьПрава" Тогда
				...
				НеСниматьПраваРКД = Новый Массив;
				...
				НеСниматьПраваРКД = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Код_в_PDM");
				
				
				Пока Выборка.Следующий() Цикл
					
					Если НеСниматьПраваРКД.Найти(Выборка.Код_в_PDM) = Неопределено Тогда
		...
		Иначе
			...
			Если ТипЗнч(ЭлементВыгрузки) = Тип("ЗадачаСсылка.Задачи_РКД") Тогда
		...
		НаименованиеЭлемента = ?(ТипВыгрузки = "ЗабратьПрава", "MovePermissions", "UpdatePermissions");
