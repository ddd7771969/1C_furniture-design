...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("РезультатАвторизацииНаСервереМЧДРРСКО", "МашиночитаемыеДоверенностиФНССлужебный.УстановитьПараметрСеансаРезультатАвторизацииНаСервереМЧДРРСКО");
...
Функция ПроверитьСертификатДоверителя(Доверенность, ФайлДоверенности, Сертификат) Экспорт
	...
	ДанныеДоверенности = Запрос.Выполнить().Выгрузить();
...
Функция ЗаголовокДекорации(Реквизиты, ИмяЭлементаОрганизация = Неопределено, ТолькоОбязательные = Ложь, ДляТаблицы = Ложь) Экспорт
	...
	Если Не ДляТаблицы И ЗаголовокДекорацииОшибки.Количество() > 0 Тогда
		ЗаголовокДекорации.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не заполнены: %1'"),
			СтрСоединить(ЗаголовокДекорацииОшибки, ", ")));
	КонецЕсли;
	
	ЗаголовокДекорацииСтрока = СтрСоединить(ЗаголовокДекорации, Символы.ПС);
	
	Если ДляТаблицы Тогда
		...
		Структура.НеЗаполненыРеквизиты = СтроковыеФункции.ФорматированнаяСтрока(СтрСоединить(ЗаголовокДекорацииОшибки, ", "));
		Возврат Структура;
	Иначе
...
Функция ПолучитьИЗаписатьОшибкуМЧДРР(ПараметрыОшибки)
	...
	Если ОтветHTTP <> Неопределено И ОтветHTTP.КодСостояния = СтрокаОтвета Тогда 
...
Процедура ДобавитьФайлДляРасследования(Файлы, ДанныеФайла, ТипФайла)
	...
	КонецЕсли;
	СведенияОФайле.Вставить("Имя", ТипФайла + ".txt");
...
Функция ФайлыДляРасследования(ПараметрыФайловДляРасследования)
	...
	Если Запрос <> Неопределено Тогда
		...
		СтруктураЗначения.Вставить("АдресРесурса", АдресРесурса);
	...
	Если ИмяФайлаЗапроса <> "" Тогда
		ДобавитьФайлДляРасследования(ДополнительныеФайлы, ИмяФайлаЗапроса, НСтр("ru='Файл запроса'"));
	КонецЕсли;
	
	Если ИмяФайлаОтвета <> "" Тогда
		ДобавитьФайлДляРасследования(ДополнительныеФайлы, ИмяФайлаОтвета, НСтр("ru='Файл ответа'"));
	КонецЕсли;
...
Процедура РассчитатьИЗаписатьТехническийСтатус(Документ) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ВнешниеСтатусы = Новый Соответствие;
	ВнешниеСтатусы.Вставить(Перечисления.ТехническиеСтатусыМЧД.Регистрация, Истина);
	ВнешниеСтатусы.Вставить(Перечисления.ТехническиеСтатусыМЧД.Зарегистрировано, Истина);
	ВнешниеСтатусы.Вставить(Перечисления.ТехническиеСтатусыМЧД.РегистрацияОтмены, Истина);
	ВнешниеСтатусы.Вставить(Перечисления.ТехническиеСтатусыМЧД.Отменена, Истина);
	ВнешниеСтатусы.Вставить(Перечисления.ТехническиеСтатусыМЧД.ОшибкаРегистрации, Истина);
...
Функция АвторизоватьсяНаСервереМЧДРР() Экспорт
	...
	Исключение
		...
		ПараметрыФайловДляРасследования.ИмяФайлаОтвета = ИмяФайлаОтвета;
	...
	Исключение
		ПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ПараметрыФайловДляРасследования = ПараметрыФайловДляРасследования();
		ПараметрыФайловДляРасследования.ИмяФайлаЗапроса = ИмяФайлаЗапроса;
		ПараметрыФайловДляРасследования.ИмяФайлаОтвета = ИмяФайлаОтвета;
		ПараметрыФайловДляРасследования.Запрос = ЗапросHTTP;
		ПараметрыФайловДляРасследования.Ответ = ОтветHTTP;
		ДополнительныеФайлы = ФайлыДляРасследования(ПараметрыФайловДляРасследования);
...
Функция ЗарегистрироватьМЧДРР(
	...
	ИмяВременногоФайлаДоверенности = ПолучитьИмяВременногоФайла(); // АПК:441 Временные файлы удаляются по МассивИменФайлов
	ТекстВыгрузки = ПолучитьСтрокуИзДвоичныхДанных(ДанныеВыгрузки);
	ПолучитьДвоичныеДанныеИзСтроки(ТекстВыгрузки).Записать(ИмяВременногоФайлаДоверенности);
	...
	СодержимоеФайла = Символы.ПС + "--My1cV8bNdr"
		...
		+ Символы.ПС + "Content-Type: text/xml"//"Content-Type: application/octet-stream"
		+ Символы.ПС
		+ Символы.ПС;
	...
	ИмяВременногоФайлаПодписи = ПолучитьИмяВременногоФайла(); // АПК:441 Временные файлы удаляются по МассивИменФайлов
...
Процедура УстановитьПризнакПодписана(Доверенность, Подписана) Экспорт
	ЗаписьСтатуса = РегистрыСведений.МашиночитаемыеДоверенностиСтатусы.СоздатьМенеджерЗаписи();
	ЗаписьСтатуса.МашиночитаемаяДоверенность = Доверенность;
	ЗаписьСтатуса.Прочитать();
	ЗаписьСтатуса.Подписана = Подписана;
	ЗаписьСтатуса.Записать();
КонецПроцедуры
