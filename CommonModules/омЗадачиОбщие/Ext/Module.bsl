...
Функция ПолучитьЗадачуПоОбъекту(ОбъектЗадачи) Экспорт
	...
	Запрос.Текст = 
		...
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК Задача_ПКД_у_Конструктора,
		...
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК Задача_РКД_у_Конструктора
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Задачи_РКД) Тогда
			...
			Возврат Новый Структура("Задача_РКД, РаботаКонструктора", Выборка.Задачи_РКД, Выборка.Задача_РКД_у_Конструктора);	
		
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Выборка.Задачи_ПКД) Тогда
		
			Возврат Новый Структура("Задача_РКД, РаботаКонструктора", Выборка.Задачи_ПКД, Выборка.Задача_ПКД_у_Конструктора);	
