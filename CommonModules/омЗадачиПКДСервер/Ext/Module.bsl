...
Функция ПолучитьЗадчиВРаботе(Конструктор, ОбщееКоличество = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ЗадачиВРаботе.ПКД.Добавить(Выборка.Ссылка);
	КонецЦикла;
	
	Выборка = РезультатЗапроса[1].Выбрать();
	
	Пока Выборка.Следующий() Цикл
		ЗадачиВРаботе.РКД.Добавить(Выборка.Ссылка);
	КонецЦикла;
	
	Если ОбщееКоличество Тогда
		
		Возврат ЗадачиВРаботе.ПКД.Количество() + ЗадачиВРаботе.РКД.Количество();
...
Функция ВозвможностьПеревестиЗадачуВРаботу(Конструктор) Экспорт
	...
	КоличествоВРаботе = омЗадачиПКДСервер.ПолучитьЗадчиВРаботе(Конструктор, Истина);
