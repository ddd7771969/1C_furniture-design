...
Функция ПолучитьДанныеПроектаДляВнешнегоПлана(Проект) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ДанныеКомплектаСрезПоследних.ВнешняяПродолжительностьПроизводства, 0) КАК ПродолжительностьПроизводства,
		|	ДлительностьЭтаповДляВнешнегоПользователяКомплекты.Комплект.Родитель.Композиция КАК Композиция
		|ИЗ
		...
		|ГДЕ
		|	ДлительностьЭтаповДляВнешнегоПользователяКомплекты.Ссылка.Проект = &Проект
		|	И ДлительностьЭтаповДляВнешнегоПользователяКомплекты.Ссылка.Проведен
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ЕСТЬNULL(ДанныеКомплектаСрезПоследних.ВнешняяПродолжительностьПроизводства, 0)";
	
	Запрос.УстановитьПараметр("Проект", Проект);
	
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции // ПолучитьДанныеПроектаДляВнешнегоПлана()
