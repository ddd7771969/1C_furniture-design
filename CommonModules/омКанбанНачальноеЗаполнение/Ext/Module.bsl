Функция ЗаполнитьНачальноеЗаполнениеКанбана(ИмяКанбана, стРазделители, мРесурсы, Комментарий, УИД_существующей_карточки = "") Экспорт
	...
	Для каждого текущийРесурс Из мРесурсы Цикл
		...
		Если ПустаяСтрока(УИД_существующей_карточки) Тогда
			УИДКарточка = СоздатьКарточкиКанбан(стПараметрыКарточки);
		Иначе
			УИДКарточка = УИД_существующей_карточки;
		КонецЕсли;
		...
		МассивУИД.Добавить(Новый Структура("Ресурс, УИДКарточка", текущийРесурс, УИДКарточка));
...
Функция ИзДесятичногоВСтроковуюДвоичную(Знач ДесятичноеЧисло)
	...
	Если СтрДлина(стрВозврата) = 1 Тогда
	
		стрВозврата = "0" + стрВозврата;	
...
Функция ПолучитьСтруктуруДляЭлементовКанбана(ТипЭлемента, Комментарий = "", УИДКудаКолонка = "") Экспорт
	...
	ИначеЕсли ТипЭлемента = "ПереходКарточки" Тогда
		...
		стПараметры.Вставить("УИДКудаКолонка",	УИДКудаКолонка);
	...
	ИначеЕсли ТипЭлемента = "ДополнительныеДанные" Тогда
		...
		стПараметры.Вставить("Дополнение", 		"");
		стПараметры.Вставить("Комментарий", 	Комментарий);
